...
probability of a hypothesis of interest can be overestimated or
underestimated. The posterior probability should reflect all the
evidence bearing on the hypothesis, but should not be confused with a
measure of the probative value or strength of the evidence. The Federal
Rule of Evidence, rule 401, underscores that an item evidence has
probative value if it `has any tendency to make a fact more or less
probable than it would be without the evidence'. The emphasis here is
not so much on the posterior probability of the hypothesis given the
evidence, \(\pr{H \vert E}\), but rather, on the extent to which the
presentation of the evidence should change---or has any tendency to
change---the probability of the hypothesis. This distinction is crucial.

Suppose the prior probability of a given hypothesis \(H\) is low, say
\(\pr{H}=.001\), but taking evidence \(E\) into account brings this
probability up to \(.35\), that is, \(\pr{H \vert E}=.35\). This is a
dramatic upward shift. Even though the posterior probability of \(H\)
given \(E\) is not very high, \(E\) has probative value in favor of
\(H\). Conversely, suppose the prior probability of \(H\) is extremely
high, say \(\pr{H}=.999\), but taking evidence \(E\) into account brings
this probability down to \(.75\), that is, \(\pr{H \vert E}=.75\). This
is a dramatic downward shift. Even though the posterior probability of
\(H\) given \(E\) is still quite high, \(E\) has probative value against
\(H\). Here is a more concrete example. Suppose an expert testifies that
the blood found at the crime scene matches the defendant's and it is
\(.5\) probable that a person unrelated to the crime would match by
coincidence. Disregarding the match, the prior probability that the
defendant is the source of the blood should be set rather low, say
\(.01\). Absent other evidence to the contrary, the defendant, as anyone
else, had little to do with the crime By running Bayes' theorem, the
posterior probability that the defendant is the source is roughly
\(.17\).\footnote{We are assuming that the probability of a match if the
  suspect is the source is approximately 1.} This might seem
unimpressive, but it does not mean that the match evidence has no
...
seventeen times larger than the prior. This example illustrates that
measuring the strength or probative value of the evidence solely by
means of posterior probabilities is inappropriate.

To avoid confusion, the overall, global value of the evidence should be
distinguished from local incremental support (Di Bello \& Verheij
(2018)). The former is more directly connected with the ultimate
decision: if \(E_1, \dots, E_n\) is the total evidence obtained and
\(H\) is the hypothesis of interest, the global value of the total
evidence is simply the posterior probability of the hypothesis,
...
should precede their aggregation into a whole, complex body of evidence.
Hence, a local notion of evidential strength or probative value is
needed.

This chapter offers a formal, probabilistic account of probative value
or evidential strength in the incremental and local sense. We prefer the
expression `evidential strength' because it coveys the fact that
evidential value comes in degrees, in that an item of evidence may
support a hypothesis more or less strongly. Another expression with a
similar meaning that we will sometimes use is `evidential support.' We
will argue that the likelihood ratio is the best measure of evidential
strength that is both (a) incremental---it tracks changes in
probability---and (b) local---it pertains to individual pieces of
evidence bearing on circumscribed hypotheses. In Section \ref{sec:lr} we
explain what this measure of evidential strength consists in, how it
compares to another one, the Bayes factor, and why it is preferable to
it. Our goal is to offer a fair-minded discussion about the pros and
cons of the likelihood ratio as a measure of evidential strength.
Section \ref{sec:fp} shows the likelihood ratio help to achieve a
nuanced assessment of the value of quantitative evidence, especially DNA
matches, when it is necessary to take into account the eventuality of
false positive and false negative matches. We explain the reasons to
take the risk of false positives in DNA identification seriously and use
likelihood ratio to illustrate the impact of such a risk on the value of
DNA evidence.(If this was not enough, Appendix
\ref{sec:coldHitConfusion} takes a detailed look at the use of
likelihood ratios in studying the value ofthe so-called cold-hit
matches.) Section \ref{sec:eyewitness} examines how the likelihood ratio
can help in evaluating eyewitness evidence. This examination should
dispel impression that likelihood ratios are only useful for explicitly
quantitative evidence such as DNA match evidence.
...
two-stain problem. The general lesson from these two sections is that
...
Finally, in Section \ref{sec:relevance} we discuss another problem for
likelihood ratios: the problem of relevance. Likelihood ratios sometimes
fail to identify all the relevant items of evidence. We explain what the
...
The chapter also contains a number of appendices. To illustrate the
utility of thinking in terms of likelihood ratios in theorizing about
the value of evidence, we spend some time in the appendix to go over a
debate about the value of cold-hit DNA matches in which various agencies
studying or performing DNA evaluation have still failed to reach
agreement (Appendix \ref{sec:coldHitConfusion}), and argue that a proper
use of likelihood ratios leads to a fairly clear resolution (Appendix
\ref{sec:cold-hit}). Finally, a more philosophically minded reader, who
might recall that there are quite a few probabilistic confirmation
measures in the vicinity and might wonder why almost none of them were
discussed in the chapter, in Appendix \ref{sec:confirmation} we explain
why we think these other measures are not fit for the particular purpose
at hand: evidence evaluation in legal-fact finding.
...
While the above counterexample makes it extremely clear that the
conjunct is irrelevant, the reader might find it far-fetched, as it
might seem that in no real-life circumstances one would be willing to
seriously consider such a conjunction. Fair enough. But there are more
natural irrelevant hypotheses. For instance, suppose one hypothesis of
interest is whether the victim was running in the part on a certain
night, and the relevant piece of evidence is her footprints in the park.
Perhaps, another important hypothesis is whether she had wine at dinner
later on. Clearly, whether she did is irrelevant to whether she was
running in the park beforehand. However, one should be very hesitant to
say that the evidential strength of the presence of footprints is the
same with respect to \emph{she was running in the park} and to
\emph{she was running in the park and had wine at dinner later on}. But
this is what one would be committed to when using \textsf{BF}.
...
conditions they satisfy, and the reasons why they are not preferable to
likelihood ratio to Appendix \ref{sec:confirmation}. The general lesson
from considering these measures, however, is that there are two
conceptually different questions in the vicinity: (1) to what extend
does a given piece of evidence should change our beliefs about a given
hypothesis? Let's call this the question about confirmation. (2) What's
the strength of this particular piece of evidence with respect of that
hypothesis? Let's call this the question about evidential strength.
While asking the question about confirmation makes sense in certain
contexts, as when we ask a physicists about what we should think about a
given physical theory in light of new experimental results, confirmation
depends on priors, and in legal evidence evaluation we'd prefer to
separate the assessment of strength (that might lie in the expert's
field of expertise) from the reasonable choice of priors (which might be
beyond the competence of the expert). Quite a few confirmation measures
are concerned with (1) rather than (2), and so are not suitable for
legal applications.

So suppose we are after a measure that does not depend on priors, puts
no such cognitive requirements on an agent, and is sensitive to the
...
to distinguish between the strengths of pieces of evidence that we
should distinguish. For instance, what is the probability that the blood
types match if the accused is the source? Well, one, pretty much. What
is the probability that the DNA profiles match if the accused is the
...
requires more information than the calculation of
\textsf{LR}.\footnote{Note however that, as we will argue in another
  chapter, \textsf{LR} for conjunctive hypotheses might depend on the
...
Moreover, the division of labor in legal fact-finding also seems to
