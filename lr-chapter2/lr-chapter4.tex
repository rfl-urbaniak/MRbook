...
sheet.\footnote{In the original formulation, two stains from two
  ...
  characteristics of the first stain. Surprisingly, the likelihood ratio
  does not depend on the frequency associated with the second stain. In
  general, if there are \(n\) bloodstains of different phenotypes, the
  likelihood ratio is \(\nicefrac{1}{nq_1}\), or in other words, the
...
we assign to the DNA match in question? Meester \& Sjerps (2004a) argue
...
across likelihood ratios is not as troubling as it might seem. If a pair
of hypotheses is more generic---that is, it encompasses a larger number
of possible scenarios---we should expect the likelihood ratio to be
lower. It would be surprising that the likelihood for the same piece of
evidence would remain the same when it is applied across hypotheses that
make different commitments about what happened. So, once again, the
likelihood ratio behaves as it should. It finely discriminates between
the granularity of different hypotheses.\footnote{Interestingly,
  however, even though the likelihood ratios are numerically different,
  the posterior the posterior odds for the three different pair of
  hypotheses are exactly the same. To see why, note that the prior odds
  of the three \(H_p\)'s in the table should be written in terms of
  \(\delta\). Following Meester \& Sjerps (2004a), the prior odds of the
  first hypothesis in the table are \(\nicefrac{\delta}{1-\delta}\). The
  prior odds of the second hypothesis are
  \(\nicefrac{(\delta/2)}{(1-\delta)}\). The prior odds of the third
  hypothesis are \(\nicefrac{(\delta/2)}{(1-(\delta/2))}\). In each
  case, the posterior odds --- the result of multiplying the prior odds
  by the likelihood ratio --- are the same:
  \(R\times \nicefrac{\delta}{2(1-\delta)}\). The reason for the
  equivalence in the posterior probability is that, even though the
  likelihood ratios associated with the hypotheses in the table above
  are numerically different, the hypotheses are in fact equivalent
  conditional on the evidence. Smith was one of the crime scene donors
  just in case he was the pillow stain donor, because he is excluded as
  the stain sheet donor. Smith was not one of the crime scene donors
  just in case he was not the pillow stain donor, because he is excluded
  as the sheet stain donor. Dawid (2004) cautions that the equivalence
  of hypotheses, conditional on the evidence, does not imply that they
  can all be presented in court. He argues that the only natural
  hypothesis for the two-stain problem is that Smith is guilty as
  charged. Meester \& Sjerps (2004b) reply that focusing on the guilt
  ...
  hypotheses is prone to arbitrariness. To avoid this problem, Meester
  \& Sjerps (2004a) recommend that the likelihood ratio should be
  ...
  range of priors. In this way, the impact of the likelihood ratio is
  made clear, no matter the hypotheses chosen. This strategy concedes
  that likelihood ratios, in and of themselves, are insufficiently
  informative, and that they should be combined with other information,
  such as a range of priors, to allow for an adequate assessment of the
  evidence.}
...
relative. So we are dealing with a two-step process here: first, select
a pair of hypotheses of interest; and second, assess whether the
...
not, confusions and misinterpretations of the evidence will occur. This
remarks suggest that likelihood are perfectly fine in their place, but
their limits should be kept in mind. As noted at the start of this
chapter, we are after a \textit{local} measure of evidential value
(strength, support), one that is circumscribed to a certain hypothesis
or pair of hypotheses. The likelihood ratio serves this purpose well,
but nothing more than that. We elaborate on this point further as we
discuss the notion of legal relevance in the next and final section.
...
networks can make better sense of this holistic perspective (de Zoete,
...
The distinction between probative value (a local measure) and relevance
(a holistic measure) should be supplemented by another distinction, one
between assessing the strength evidence and making decisions on the
basis of the evidence. The criterion of relevance selects which pieces
of evidence are helpful in determining the defendant's liability. Once
these piece sof evidence are selected, their probative value should be
assessed. It can first be assessed on a local level, relative to
circumscribed set of hypotheses, and then on the global level relative
to the ultimate hypothesis. It is this global assessment of the
evidence---taking into account all the relevant evidence and the
ultimate hypothesis---that should guide trial decisions, for example,
conviction and acquittals in criminal trials. It would be a mistake to
think that an assessment of the value of the evidence using the
likelihood ratio is enough to guide decisions. As we will discuss in
later chapter, we think that several criteria should guide trial
decisions, one of them being the posterior probability of the ultimate
hypothesis based on all the evidence available. In this section, we show
that even though likelihood ratio can be quite variable depending on the
choice of the competing hypothesis, the probability is much less
variable and thus it is offers a more objective basis to make decision.
