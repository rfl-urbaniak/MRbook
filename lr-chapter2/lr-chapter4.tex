...
sometimes use is `evidential support.' This expression highlights the
fact that an item of evidence has value in that it serves to support,
more or less strongly, a hypothesis. We will argue that the likelihood
ratio is the best measure of evidential value (or strength, support)
that is incremental as well as local.
...
literature in formal Bayesian epistemology, but concludes that these
measures are unsuited for evidence evaluation in legal fact-finding. We
then offer several illustrations of how likelihood ratios can be
...
example, DNA matches, when the risk of false positive and false negative
errors should be factored in. To strengthen this point, Appendix
...
testimony and other forms of evidence, likelihood ratios should be
deployed with caution. They can be hard to interpret in practice, as
they depend on the choice of two competing hypotheses. This choice is,
to some extent, subjective, as we discuss in Section \ref{sec:hchoice}.
This interpretative problem arises partly because the competing
hypotheses admit of different levels of specificity: we discuss this
phenomenon in Section \ref{sec:lhTwoSTain}, using the two-stain problem
as an illustration. In Section \ref{sec:relevance} we discuss evidential
...
ratios sometimes may categorize an item of evidence as irrelevant while
intuitively the item is relevant. We explain what the problem is and how
it can be addressed. The moral is that likelihood ratios are
\textit{local} measures of evidential value. Indeed, it might be the
case that a piece of evidence is irrelevant with respect to a certain
choice of competing hypotheses, but relevant to another. The value of an
item of evidence is to be established both locally (relative to specific
hypotheses) and globally (relative to the case as a whole). Instead of a
single likelihood ratio, one should consider various likelihood ratios
for different plausible selections of hypotheses This suggests the need
of formulating a more complex theory. We undertake this task in later
chapters.
...
one. But obviously a DNA profile match is not on par with a blood type
match. The DNA match should be stronger evidence that the blood type
match because a specific DNA type occurs less frequently than a specific
blood type. And yet, a quantity such as \(\pr{E \vert H}\), by itself,
would fail to distinguish the two cases. These remarks suggest that,
besides \(\pr{E \vert H}\), also \(\pr{E \vert \neg H}\) matters. If the
accused is \textit{not} the source, the probability of a blood type
match, while relatively small, should be higher than the probability of
a DNA profile match. Hence, the likelihood ratio
...
This difference would explain why a DNA match is stronger evidence than
a blood type match.
...
given \(H'\) than \(H\), the ratio would be below one.

As with the Bayes factor, support levels correspond to deviations from
one. The greater the likelihood ratio (for values above one), the
stronger the evidence in favor of \(H\) as contrasted with \(H'\). The
smaller the likelihood ratio (for values below one), the stronger the
evidence in favor of the competing hypothesis \(H'\) as contrasted with
\(H\). This apparent simplicity, however, can often give rise to errors
in the assessment of the evidence, especially if the two hypotheses are
not chosen carefully. The choice of the hypotheses that are conditioned
upon is crucial. In the most straightforward case, \(H'\) is simply the
...
later. This is both a strength and weakness of the likelihood ratio.
...
is apparent in the odds version of Bayes' theorem:
...
This feature likelihood ratio fits well with the division of labor
common in legal fact-finding between experts and decision-makers, judges
or lay jurors. A prominent forensic scientist recommends that `in
...
of total probability). Instead, the likelihood ratio describes the value
of evidence and abides by these constraints. It is often used in
practice. Experts sometimes do testify by offering the likelihood ratio
as a measure of the strength of the evidence. An expert, for instance,
may testify that the blood-staining on the jacket of the defendant is
ten times more likely to be seen if the wearer of the jacket hit the
victim (prosecutor's hypothesis) rather than if he did not (defense's
...
\(\mathsf{LR}(E,H) = \nicefrac{1}{\pr{E \vert \n H}}\), while
\(\mathsf{LR}(E, H \et X) = \nicefrac{1}{\pr{E \vert \n H \vee \n X}}\).
Unlike with the Bayes factor, here the two denominators in the
likelihood ratio might differ. For example, suppose a fair coin is
tossed three times. Let \(H=\) ``two first tosses resulted in two
heads,'' let \(E=\) ``at least one of the two first tosses resulted in a
head,'' and let \(X=\) ``the third toss resulted in heads.'' Then
...
strength than fares even better than the likelihood ratio. One set of
measures worth considering come from literature in formal epistemology
on confirmation theory. The expression `confirmation' is more commonly
used in this literature instead of strength of the evidence (or value,
support). An extensive discussion of these measures, however, would
detract us from the main task at hand, namely the study of the
properties and applications of the likelihood ratio, the most promising
candidate championed by legal probabilists. We therefore relegate a
discussion of confirmations measures to Appendix \ref{sec:confirmation}.
The upshot is that two questions should be distinguished. (1) To what
extend should a given piece of evidence change our beliefs about a given
hypothesis? Call this the question about confirmation. (2) What is the
strength of apiece of evidence relative to a hypothesis? Call this the
question about evidential strength. The two questions overlap to some
extent. The key difference is that confirmation depends on prior
probabilities, whereas, in the evaluation of legal evidence in trial
proceedings, the assessment of evidential strength (that might lie
within an expert's field of expertise) should be kept separate from the
choice of prior probabilities (which might be beyond the competence of
the expert). Confirmation measures are concerned with (1) rather than
(2), and thus they are unsuitable for legal applications.
...
generalized to any form of `match evidence': fingerprints, shoe prints,
bite marks, blood, hair, glass, etc. Match evidence usually consists in
a statement by an expert that the defendant matches the physical
material associated with the wrongdoing. The match can be between
genetic profiles, fingerprints, blood type, show prints, etc. For
concreteness, we will focus on genetic, DNA matches.

Suppose an expert testifies that there is a match between the traces at
the crime scene and a sample from the defendant. What is this statement
evidence for? At the very least, the match is evidence that the
defendant was the \textit{source} of the traces---that the materials
found at the scene originate from the defendant. The match can also be
evidence that the defendant was present at the scene or committed the
act, but these claims are more far-fetched, as the chain of inferences
become much weaker. We will discuss these complications later in section
\ref{sec:lhTwoSTain}. For simplicity, let us focus on the source
hypothesis.

That the defendant match the crime traces---or to be more precise, that
an expert witness testifies to that effect---is good evidence that the
defendant is the source of the crime traces. But how strong is this
evidence? Following the likelihood ratio, two conditional probabilities
should be compared here, \(\pr{\textsf{match} \vert \textsf{source}}\)
and \(\pr{\textsf{match} \vert \neg \textsf{source}}\). However, when
experts testify in court about a DNA match, they only use the
...
crime, would coincidentally match the crime stain profile. The random
match probability is often an impressively low number, say 1 in 100
million or lower. The match, accompanied by an impressively low random
match probability, is taken to constitute strong evidence against the
defendant. How this analysis compare to one based on the likelihood
ratio? The random match can be equated to the probability that someone
who is not the source would coincidentally match,
\(\pr{\textsf{match} \vert \neg \textsf{source}}\). This is one of the
conditional probabilities that are part of the likelihood ratio. Should
the other also be reported? In theory, it certainly should, but it
practice, it may not make a difference. The random match probability
alone can be a good measure of evidential strength so long as the other
conditional probability is close to one. That
...
does reflect the fact that the evidence is strongly incriminating
evidence. The lower the random match probability, the more strongly
incriminating the match. The conceptual point still stands, though. If
\(\pr{\textsf{match} \vert \textsf{source}}\) was significantly
different from one, reporting only
\(\pr{\textsf{match} \vert \neg \textsf{source}}\) would be misleading.

But this analysis, while roughly correct, is not quite precise enough in
the denominator or the numerator. Consider the denominator first,
\(\pr{\textsf{match} \vert \neg \textsf{source}}\). Equating this
probability with the probability of a coincidental match (the random
match probability) neglects the possibility of false positive matches,
that is, someone who is not actually a match may be erroneously reported
to be a match. A coincidental match is not the same thing as a false
positive match. Two individuals---say the perpetrator and the
defendant---may happen to share the exact the same DNA profile. If DNA
expert reports that the sample from the crime scene and the sample from
the defendant match, this report would be correct, but it would also be
a coincidental match. So the sources of error that should be included in
the denominator \(\pr{\textsf{match} \vert \textsf{source}}\) are
twofold: a coincidental match as well as a false positive match. Before
we detail how this can be done formally, it pays to comment on the
reality of false positive matches. Even if DNA evidence seems quite
scientific, the risk of false positives is not negligible (Shaer,
2016).\footnote{For instance, Houston Police Department Crime
  Laboratory, a large public forensic center in Texas, handles around
  500 cases a year. In 2016, KHOU 11, a local television station, sent
  dozens of profiles processed by the lab to independent experts. The
  results were not optimistic: police technicians quite systematically
...
found, and in 2003 Sutton was released from prison.\footnote{Christy Kim
  later sued her employer for firing her after the fact and prevailed.
  Her mistakes were attributable to systemic failures and inadequate
  supervision.}

This is only one example of quite a few cases of DNA matching going
awry. Anecdotal evidence suggests there are several sources of error
(see W. C. Thompson, 2013 for a more exhaustive treatment and multiple
examples):
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter4_files/figure-latex/eyewitness4-1} \end{center}
...
\hspace{1cm}\scalebox{.9}{\begin{subfigure}[!ht]{0.6\textwidth}
    ...
    \begin{center}\includegraphics[width=1\linewidth]{lr-chapter4_files/figure-latex/STMdag-1} \end{center}
  \end{subfigure}} \hfill
...
\begin{center}\includegraphics[width=0.8\linewidth]{lr-chapter4_files/figure-latex/fig-puckett-1} \end{center}
\label{fig:puckett}
