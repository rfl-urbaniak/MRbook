...
sometimes use is `evidential suport.' This expression highlights the
...
part of any good measure of evidential strength.\footnote{The need to
  compare both probabilities is made clear by an example by Triggs \&
  Buckleton (2004). In a child abuse case, the prosecutor offers
  \label{text:rock} evidence that a couple's child rocks and that only
  3\% of non-abused children rock,
  ...
  (ENFSI, 2015; Royall, 1997; Triggs \& Buckleton, 2004).} The Bayes
factor includes both probabilities, but---as seen before---it requires
too much information since it requires an assessment of the prior
probability of the hypothesis. A less demanding measure that that still
keeps track of both probabilities is the \textbf{likelihood ratio}:
\begin{align}
\label{eq:LR}
\tag{LR}
...
competing hypothesis \(H'\) as contrasted with \(H\). This apparent
simplicity, however, can often give rise to errors in the assessment of
the evidence, especially if the two hypotheses are not chosen carefully.
The choice of the hypotheses that are conditioned upon is crucial. In
the most straightforward case, \(H'\) is simply the negation of \(H\).
But the competing hypotheses \(H\) and \(H'\) need not be one the
negation of the other, a point to which we will return later. This is
both a strength and weakness of the likelihood ratio.
...
The division of labor in legal fact-finding also recommends the
...
abides by these constraints. It is often used in practice. Experts
...
\(\pr{E\vert H}\) (which here, say, con be assumed to be close to one)
...
\(\pr{\neg H}\). Instead, the likelihood ratio would just require an
...
All in all, the likelihood outperforms the Bayes factor on several
respects. But, of course, there could be other measures of evidential
strength out there than fares better than the likelihood ratio. One set
of measures worth considering come from literature in formal
epistemology on confirmation theory. The expression `confirmation' is
more used in this literature instead of strength of the evidence (or
value, support). An extensive discussion of these measures would detract
us from the main task at hand, namely the study of the properties and
applications of the likelihood ratio, the most promising candidate
widely championed by legal probabilists. We therefore relegate a
discussion of these confirmations measures to Appendix
\ref{sec:confirmation}. The general lesson is that two conceptually
different questions should be distinguished: (1) To what extend should a
given piece of evidence change our beliefs about a given hypothesis?
Call this the question about confirmation. (2) What is the strength of
this particular piece of evidence with respect of that hypothesis? Call
this the question about evidential strength. The two questions are
obviously related and they overlap. The difference is that confirmation
depends on prior probabilities. Instead, in the evaluation of legal
evidence presented in trial proceedings, the assessment of strength
(that might lie within an expert's field of expertise) should be kept
separate from the reasonable choice of prior probabilities (which might
be beyond the competence of the expert). Confirmation measures are
