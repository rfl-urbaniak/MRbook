...
0, we obtain the formula obtained by Colin Aitken, Taroni, \& Thompson
(2003), who did not consider false negatives.
...
likely generic error frequencies won't be reliable enough, and relevant
factors should be identified as used as predictors. To this end, error
rate prediction should be based on data, that are fine-grained enough to
...
a more case-specific assessment of the value of match evidence.
...
probability of error. The underlying intuition is that a laboratory
error is an event in which the identification fails to be proper for
technical or procedural reasons, for instance, the samples have been
switched, or some equipment fails and produces an erroneous
reading---intuitively, whether such a laboratory error occurs does not
depend on whether the source hypothesis hods, and so the argument starts
with the assuming that the errors to be probabilistically independent of
...
What's the connection of this formula to the one developed by Colin
Aitken, Taroni, \& Thompson (2003)? If you think of an error as
something that guarantees a mistaken reported match, \(k\) becomes
\(1\), \(\mathsf{err}\) becomes the false positive rate, FPP. On this
assumption straightforward algebraic manipulation gives: \begin{align*}
 \frac{1 - (1-k)\times FPP}{RMP\times (1-FPP)+k\times FPP} & = \frac{1-FPP+FPP}{RMP(1-FPP)+FPP}\\
 & = \frac{1}{RMP - FPP\times RMP + FPP} 
\end{align*} \noindent   Now, if we disregard false negatives, as Colin
Aitken, Taroni, \& Thompson (2003) do, our general formula
\eqref{eq:LRfp4b} gives: \begin{align*}
\frac{1}{RMP + FPP \times (1-RMP)} 
\end{align*}

\noindent Now it takes a straightforward algebraic manipulation to show
that the denominators in the last two formulae are identical.
