...
shold not increase the confirmation level.

One consequence of this assumption is that confirmation of \(T\) by
\(E\), \(c(T,E)\) should track the posterior order-wise, that is
\(c(T,E)>c(T,E')\) just in case \(\pr{T\vert E} > \pr{T\vert E'}\) even
though \(c(T,E)\) should not be identified with \(\pr{T\vert E}\).
...
Yet another adequacy requirement is local equivalence. Theories that are
...
qualitative-quantitative bridge and local equivalence are ordinally
equivalent with the posterior probability \(\pr{H\vert E}\). Measures
that are so ordinally equivalent are said to explicate confirmation as
\emph{firmness of belief}, especially if in the probabilities used
implicitly rely on total available evidence.
...
measure to be above the neutrality threshold.

For this reason, following the second edition of (Carnap, 1962) it is
customary to distinguish another notion in the vincinity: confirmation
as increase in firmness of belief. Here, the idea is rather than
confirmation is positive (negative) if the posterior of \(H\) is higher
(lower) than the prior, and neutral otherwise.
