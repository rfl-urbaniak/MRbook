...
one. Conversely, again by Bayes' theorem, the probability of \(H\) given
...
the Bayesian Factor is approximately \(3.57\). In fact, a more general
look (Figure \ref{fig:BayesFactorPrior}) shows the prior probability can
have larger impact on the Bayes factor than the likelihood
\(\pr{E \vert \n H}\).
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter3_files/figure-latex/BayesFactorPrior-1} \end{center}
...
For the above reasons, a measure that does not depend on priors, puts no
such cognitive requirements on an agent, and is sensitive to the
addition of irrelevant conjuncts would be preferrable. Clearly, we
should not simply use \(\pr{E\vert H}\). For one thing, in most
interesting cases this conditional probability will be very close to one
and will not allow us to distinguish between the strenghts of pieces of
evidence that we should distinguish. For instance, what is the
...
match if the accussed is the source? Again, one. But obviously a DNA
profile match is not on par with a blood type match insofar as strength
of evidence is involved. Consider an example by Triggs \& Buckleton
(2004). In a child abuse case, the prosecutor offers evidence that a
couple's child rocks and that only 3\% of non-abused children rock,
...
viable. We will discuss these issues in detail in this chapter later
on.\footnote{Note that \textsf{LR} is not susceptible to the alien
  simulation counterexample. Even if
...
not always make a practical difference.

For suppose an expert testifies that the crime traces genetically match
the defendant and that the \textbf{random match probability} is
extremely low, say 1 in 100 million. Is the match strong evidence that
the defendant is the source of the traces? The random match
...
same DNA profile. This is, however, also in agreement with the use of
likelihood ratio in evidence evaluation, because
...
instructive to look at DNA evidence, focusing on the impact of false
positive risk first, and on the so-called cold-hit matches later on.
...
\item
  ...
  of time and effort searching for the so-called Phanotm of Heilbronn,
...
\item
  ...
  mixture aand the DNA profiles of the victim and three suspects
  together those pieces of information that were highly relevant (such
  as the DNA amplification conditions) was sent to 17 lab technicians
  for examination. One of them agreed that the defendant could not be
...
First, let us implement \eqref{eq:indOnS} in four places:
...
probabilistic measure of evidential relevance (Lempert,
1977,lyon1996relevance,aitken2004statistics,
...
\todo{think about independence again once the section is done} One
