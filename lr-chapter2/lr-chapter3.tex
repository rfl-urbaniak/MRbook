...
W. C. Thompson, 2013 for a more exhaustive treatment and multiple
examples):
...
\todo{M: The derivation is fine. It can be relegated to a footnote. We need to explain, intuitively,
what is going on here. Formula (13) at the end is instructive. It shows that a reported match (given not-S) can happen  when there is a true match and when there is no true match, so we need to explain the notion of true match. Can BN help here? There is a path from source to true match to reported math.}
...
probability that a match is reported if it does. In terms of our
...
\todo{M:Using E for error is confusing as we typically use E for evidence.}
...
\todo{M: I could  not follow the derivation. We need to give 
an intuitive explanation and then relegate the 
proof to a footnote or appendix.}
...
FNP. With FPP, the LR is 50 when \(e=0.01\) for RMP\(=10^{-3}\), while
for the same \(e\) and RMP it is \(0.01\) for FNP, an hundredfold
decrease. This illustrates that the exculpatory value of DNA evidence is
higher than its incriminating value, even if the error rates and random
match probabilities are the same.

\todo{M: This is a very interesting point, worth thinking about for a bit more. 
Is it true? Is there an intuitive explanation?}

\vspace{3mm}

\textbf{Marcello: The role of this section is not completely clear yet. Is it meant to illustrate why LR are useful?  Is it because FP and FN cannot ne incorporated into other measures of evidential strength such as RMP? But what about the BF? We should be clearer about the role of this section in defeding the LR approach.}
...
The discussion so far suggests that the likelihood ratio is a fruitful
conceptual framework for assessing the strength of the evidence, even in
complex cases such as cold-hits. Now, however, we turn to a problem with
the use of likelihood ratio in the context of legal fact-finding: it is
sensitive to the hypothesis choice. We discuss this problem in general
in this section, (i) showing how an ad hoc hypothesis can always lead to
a useless likelihood ratio, (ii) discussing a real case in which the
choice of hypothesis was fairly confusing and had impact on evidence
evaluation, (iii) indicating that the problem doesn't arise if the
hypotheses as exclusive and exhaustive, and (iv) arguing that this
theoretical resolution may be unhelpful in practice, as there seem often
to be good reasons why experts do not use such pairs of hypotheses.
Further, in Section \ref{sec:lhTwoSTain} we further illustrate how large
shifts in the likelihood ratio can take place even if the hypotheses
chosen seem reasonable, and argue that another clear source of
variability of likelihood ratio is the choice of hypothesis levels. The
two sections taken together paint a difficulty with using likelihood
ratio alone as a measure of evidential strength.
...
further illustrates both advatanges and limitations of the likelihood
...
\textbf{Marcello: perhaps this section could come right after the discusison of false positive in DNA evidence, just to give a second illustrration of how LR can be used to weigh evidence, and not just quantitive evidence.}
...
\todo{M: These numbers are interesting, but unless we use them later to help us fill out the values of the probabilities, this is besides the point. Get to the point -- i.e. how weuse LR to weigh eyewitness evidence.}
...
\todo{M: Where do these probabilities come from? Here you can cite emprical research to justify those values.}
...
\textbf{M: This discussion delivers  litle insight. As readers, we were sold the idea that LR would help clarify things or  block mistakes, but we are just left with the thought that "it depends". This is hardly progress. Perhaps this section needs an example to make it more exiciting. If not, we need to be clear  there isa lot of work to be done in this area. And how about the claim that DNA evidence is stronger than eyewitness evidence, see the Rush decision I gave you? Is this claim correct?Does LR help?}
