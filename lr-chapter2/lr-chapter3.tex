...
sometimes called the \emph{Bayes factor}:
...
related to the notion of evidence strength, and why none of the
...
requirements ot be explicated probabilistically to be in agreement with
both successful scientific practice and sensible philosophical
principles? The hope is that having answered these questions would
facilitate both rational reconstructions of various developments in the
history of science, and a critical evaluation of various ongoing
...
disagree about the prior probability of \(T\) and this might have impact
on the posterior.

However, there are some clear connections to the probabilities mentioned
in the previous passage. One usual assumption (Sprenger \& Hartmann,
2019)---called the \emph{prior-posterior dependence} is that the level
of confirmation is to be a continuous function of \(\pr{T}\) and
\(\pr{E\vert T}\) which is non-decreasing in the first argument and
non-increasing in the second argument. That is, increasing the prior,
should not lower the confirmation level, and increasing the likelihood
shold not increase the confirmation level.\footnote{Some formulations
  (Crupi, 2015) are a bit more general and include background knowledge
  \(K\). In that setting, the corresponding requirement is called
...
One consequence---called \emph{Final Probability Incrementality}--- of
...
Yet another requirement is local equivalence. Theories that are
...
functions satisfiying these three conditions are ordinally equivalent.
...
confirmation measures, those meant to capture probabilistic relevance.
On this approach, \(E\) confirms (disconfirms) \(T\) just in case
\(\pr{H \vert E} > \pr{H}\) (\(\pr{H \vert E} < \pr{H}\)).

Here is a list of standard confirmation measures available on the market
...
\begin{itemize}
\item
  (Log-likelihood), our good old likelihood ratio, and
  (Kemeny--Oppenheim) are ordinally equivalent (and no other pair on the
  list is).
\item
  Final probability incrementality with prior-posterior dependence
  exclude (Carnap) and (Christensen-Joyce).
\item
  (Carnap) and (Log-ratio) have the unintuitive consequence that
  \(C(T,E)= C(E,T)\).
\item
  (Difference), (Generalized entailment), (Log-ratio), (Carnap) and
  (Rips) depend on the prior of \(T\).
\end{itemize}

Further grouping and assessment of the confirmation measures for a given
puprose is falicitated by the following facts:
...
\item
  ...
  intuitive requirement in our context is debatable. After all, `the
  suspect is the source or the suspect's brother is the source' is
  entailed by `the suspect is the source'
  \todo{finish example? think again?}
\item
  ...
  measure is a strictly increasing function of the Bayes factor.
\item
...
satisfisfies the disjunction of alternative hypotheses is the difference
