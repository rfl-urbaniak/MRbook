...
\todo{M: Would be good to have a clearer introduction to the chapter, what it is about, meant to accomplish, key claims etc.}
...
evidence has on the posterior probability?
\todo{M: Up to this point, you just want a measure of the impact of the evidence onto the priors and leading to new posteriors, you do not necessarily want a measure that is independent of the priors.}
One measure of the strength of evidence is the likelihood of the
evidence compared to the prior of the evidence (this measure is
sometimes called the \emph{Bayes factor}: \begin{align}\label{eq:BF}
...
\(H\) whenever the Bayesian factor is less than one. In general, the
smaller the Bayesian factor (for values below one), the greater the
...
\todo{M: I changed several instances of Bayesian factor into Bayes factor, but I might have miessed some.}
...
\todo{M: Ok, good, but you might want to motivate philosophIcally why we do not want to have dependency on priors. The strenght of the evidence should be the same regardless ofpriors. Why? The strenght of a test result should be the same regardless whether or not a certain condition is prevalent in a population. Plausible, but I think this needs some more careful discussion. Diagnostic properties of medical tests might change depeding on base rate.}
...
(2004). In a child abuse case, the prosecutor offers \label{text:rock}
evidence that a couple's child rocks and that only 3\% of non-abused
children rock, \(\pr{\textsf{child rocks} \vert \textsf{no abuse}}=.3\).
If it is unlikely that a non-abused child would rock, the fact that this
child rocks might seem strong evidence of abuse. But this reading of the
3\% figure is mistaken. It could well be that 3\% of abused children
rock, \(\pr{\textsf{child rocks} \vert \textsf{abuse}}=.3\). Note that
...
\todo{M: Might want to bring the footnote to the main text since it is about the counterexample against BF but not LR. Also motivate this  more or add other examples.}
...
\todo{M: This paragraph about experts is off topic. How does this relate to the BF v. LR comparison?}
...
\todo{M: It is not clear why you switch to the topic of false positives? Is this an illustration of why LR is better than BF? What does this section do?}
...
\item
  ...
  \todo{M: Very nice list. This is good. Very detailed. }
\end{itemize}
...
rather unlikely turn of events. \todo{M: References?}
...
systematically quantify error rates in DNA testing.
\todo{M: Check grammar of 'no serious attempts to systematically quantify error rates in DNA testing' -- missing verb?}
...
\todo{M: How about fale negatives? Should this be mentioned as well?}
...
0.05, for two values of RMP: \(10^{-9}\) (often reported in the case of
...
comparison involves a mixed sample).

\todo{M: Better to refer to Figure \ref{fig:fpplr}
 and elborate on this more. The upshot here feels too short and might go unoticed. Also, woudl be good to have a similar graph with the impact of false negative rates.}

\todo{M: It's not clear what Figure \ref{fig:fpplr}
 says. Does it say that if FP rate is 5\%, then the LR (considering FP) is close to the LR (not considering FPs). Clarify. Try to make the figure concretely relevant. If so, FPs do not matter that much, right?}
...
positive rate. On this assumption we have: \begin{align*}
...
Aitken et al. (2003) if we take \(e\) to be FPP, as we should on the
assumption that \(k=1\).
...
\todo{M: What would strengthen the chapter  is to have another example of how LR can be used (or not used) to assess evidentiary value, namely for eyewitness testimonies. Otherwise it feels as though LRs are just good with quantitive evidence and that's it. What about other forms of evidence? We need to mention other evidence because we want a theory of the value of evidence in general, not just for DNA evidence.}

\hypertarget{eyewitness-identification-and-likelihood-ratio}{%
\section{Eyewitness identification and likelihood
ratio}\label{eyewitness-identification-and-likelihood-ratio}}

To a large extent, in this chapter, we have been focusing on DNA
evidence, which rather uncontroversially is quantitative. This might
give the impression that likelihood ratio is useful only for very
specific type of expert evidence. To balance it out, in this section, we
discuss how likelihood ratio is still useful for evidence which, at leat
seemingly, is not quantitative: eyewitness evidence.

Our first point here is that there is no magical barrier here. A certain
type of evidence can become numerical if sufficient amount of evidence
about its reliability has been collected and statistically analyzed.
Eyewitness testimony is no exception.

Moreover, such analyses tend to lead to more sensible assessment than
merely intuitive judgments. In the case of eyewitness testimony, this is
crucial because eyewitness evidence tends to be overvalued, and it is
the quantitative information that can and should be used to stop this
madness. Field studies indicate filler identification rates of 20-24\%
(Klobuchar, Steblay, \& Caligiuri, 2006) in eyewitness identifications.
That is, around 20\% of the time, an innocent presented to an eyewitness
is going to be `identified' by the eyewitness (well, the situation is a
bit more complicated, read on). 4.1\% is a conservative estimate of
false death sentence convictions in the United States (and those are
based on much stronger and multiple pieces of evidence, not a single
eyewitness' testimony (Gross, O'Brien, Hu, \& Kennedy, 2014)). A study
of 340 exonerations in years 1989-2003 indicates that around 90\% of
false convictions for rape (and pretty much all inter-racial rape
mistaken convictions) are based on eyewitness misidentification, and in
43\% of false convictions for murder the defendant was misidentified by
one or more eyewitnesses.

A study of line-ups in 1561 witnesses and 616 suspect in real cases in
Greater London (Wright \& McDaid, 1996) and of 689 identification
attempts in 271 real identification cases in Sacramento (Behrman \&
Davey, 2001) suggest false positive rate of around 20\%. Also in
experimental setting (where the witnesses are less emotionally affected
by the crime), eyewitnesses identify a filler in approximately twenty
percent of all real criminal line-ups (Thompson, 2007).
...
make such discriminations \citep{wells2003EyewitnessTestimony}. For
instance, in an experiment by \citet{Lindsay1981CanPeopleDetect}, a
representative sample of \(48\) witnesses was cross-examined. Subjects
(\(n = 96\)) viewing the cross-examinations showed no ability to detect
accurate- from false-identification witnesses within conditions as
measured by subjects' belief of witnesses.

However, the situation is a bit more complicated, and here is where the
quantitative analysis shines. Several of the eyewitness quality issues
have been studied by means of experimental methods. Some of them are
systemic variables (simultaneous/sequential lineups,
...
frequency of witnesses per suspect), some of them --- estimator
variables -- are not (the effect of delay, cross versus own-race
effects, weapon focus effects, presence of violence) --- see (Behrman \&
Davey, 2001) and (Wells \& Olson, 2003) for an interesting discussion of
such factors.
...
Information about all the factors we disussed and the data used in the
experiment can, with appropriate offer, be used to develop an
appropriate multivariate model which would lead to quantitative
eyewitness reliability estimates, including some estimation of the
uncertainty involved (such as confidence intervals). Claiming that an
untrained jury member can do better by just intuitively evaluating what
the eyewitness said instead is at least hasty.

So let us think about eyewitness testimony in terms of likelihood
ratios. We need some estimate of \(\pr{E \vert H}\) and
\(\pr{E \vert \n H}\). As for the former, real cases surveys, such as
those in Sacramento, indicate that the ratio with which witnesses
correctly point out the suspects in lineups, is actually below 0.5. But
say we know the conditions were pristine (in the sense of Wixted \&
Wells (2017)), and use 0.9 with 99\% confidence interval \(\pm 2\%\).

\hypertarget{confirmation-measures}{%
\section{Confirmation measures}\label{confirmation-measures}}
