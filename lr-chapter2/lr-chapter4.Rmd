...
So, a careful analysis should take into account the risks of false positive and false negative matches in the assessment of DNA evidence This can be done using the likelihood ratio,  following @aitken2003probability.
...
\begin{figure}[h]
```{r ex-inc-dna-b,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%", warning = FALSE, message = FALSE}
ggarrange(s1+theme_tufte(base_size=8),s2+theme_tufte(base_size=8), ncol =2 )
```
...
# Evidential strength, posterior probability and decisions

The distinction between probative value (a local measure) and relevance (a holistic measure) should be supplemented by another distinction, one between assessing the strength evidence and making decisions on the basis of the evidence.
The criterion of relevance selects which pieces of evidence are helpful in determining the defendant's liability. Once these piece sof evidence are selected, their probative value should be assessed. It can first be assessed on a local level, relative to circumscribed set of hypotheses, and then on the global level relative to the ultimate hypothesis. 
It is this global assessment of the evidence---taking into account all the relevant evidence and the ultimate hypothesis---that should guide trial decisions, for example, conviction and acquittals in criminal trials. It would be a mistake to think that an assessment of the value of the evidence using the likelihood ratio is enough to guide decisions. As we will discuss in later chapter, we think that several criteria should guide trial decisions, one of them being the posterior probability of the ultimate hypothesis based on all the evidence available. In this section, we show that even though likelihood ratio can be quite variable depending on the choice of the competing hypothesis, the probability is much less variable and thus it is offers a more objective basis to make decision.
...
\noindent  Let the first hypothesis be that the suspect was one of the two men who committed the crime and the second hypothesis the negation of the first. 
 ...
 \end{center}
\vspace{2mm}
@Evett1987 shows that the likelihood ratio of the match relative to these two hypotheses is $\nicefrac{1}{2q_1}$ where $q_1$ is the estimated frequency  of the characteristics of the first stain. Surprisingly, the likelihood ratio does not depend on the frequency associated with the second stain. In general, if there are $n$ bloodstains of different phenotypes, the likelihood ratio is $\nicefrac{1}{nq_1}$, or in other words, the likelihood ratio depends on the number of stains but not on the frequency of the other characteristics. 
