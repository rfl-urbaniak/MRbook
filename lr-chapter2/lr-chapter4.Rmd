...
A true match will exist not only if the suspect is the source, but also if, even though the susoect is not the source, the profiles are in fact the same due to a random, coincidental match. Similarly, a reported match might arise not only if there is a true match, but also if a false positive error has been made. These possibilities are represented in Figure \ref{fig:fpp}. 

In this more precise set up, the evidence whose strength is to be assessed is the \textit{reported} match relative to the pair of hypotheses $S$ and $\neg S$. So the likelihood 
...
\noindent Keep in mind that the negative reported match $\neg R$ is evidence \textit{against} the source hypothesis $S$ so long as the likelihood ratio is below one. At the extreme, if the false negative probability FNP is zero,  the numerator is zero. So the the likelihood ratio will be zero, as it should. In such a case, the negative match is completely exculpatory, and the posterior probability that the suspect is the source will also be zero.  If the false negative probability is not zero, the likelihood ratio will be greater than zero. The greater the likelihood ratio (for values between 0 and 1), the weaker the value of the exculpatory match. As Figure \ref{fig:fpfnplr-exc} shows, the likelihood ratio progressively moved way from zero as the fasle negative error probability increase. However, the impact of the false \textit{positive} probability on the likelihood ratio is nearly null, especially if the random match probability is low (as is usually the case). 
...
Let us take stock of the discussion. Likelihood ratios are useful in the evaluation and comparison of the impact of positive and negative error rates. Even a seemingly small error rate trumps the random match probability. If there are good reasons to worry about random matches, there are even stronger reasons to worry about error rates. While they have not been studied systematically, our analysis shows which error rates we should be concerned about and in which circumstances. <!---the above application of likelihood ratio to investigate the potential impact of hypothetical error rates is still useful in guiding further research, as it clearly suggests that error rates may have serious impact on the value of various types of evidence, and so require attention. --->
We have seen that the impact of error rates is not the same, contrary to what one might intuitively think. 
The false positive probability has a marked impact on the value of incriminating DNA evidence (positive matches), while the false negative probability has a marked impact on the value of exculpatory DNA evidence (negative matches).^[As noted in the prceeding paragraph, the behavior of false positive and false negative error is not perfectly symmetric when we measure evidential value in terms of the likelihood ratio. But it is ultimately symmetric since  a positive match followed by a negative match makes no impact on the probability of the source hypothesis.] A false negative probability has a negligible impact on the value of incriminating DNA evidence, and conversely, false positive probability have a negligible impact on the value of exculpatory DNA evidence. So, besides the random match probability, the false positive probability  matters for assessing the value of a positive match, and the false positive probability matters for assessing the value of a negative match. Both error probabilities should be studied, even though they are most relevant in different circumstances. 
...
The ineffectiveness of cross-examination at detecting a false identification might stem from its reliance on intuitive, folk assessment of the risks of error, not on well-researched quantitative data. The research on eyewitness testimony can improve this state of affairs. Much of this research examines the conditions under which an identification goes awry. These conditions are typically divided into system variables and estimator variables [@behrman2001EyewitnessIdentificationActual; @wells2003EyewitnessTestimony] . The former refer to how the identification took place in a regimented setting, say whether it was a line-up or a showup;\footnote{A showup refers to the observation of a single suspect by a witness in the field, typically at the crime scene. A lineup refers to the presentation of the suspect and several foils, either live or via photographs.} whether the line-up was simultaneous or sequential; whether the witness identified a suspect prior to the line-up; etc. Estimator variables, instead, refer to the environmental conditions. These variables include distance, lighting conditions, time, the presence of a weapon, etc. 
