...
The two conditional probabilities that make up the likelihood ratio---$\pr{E \vert H}$ and $\pr{E \vert \neg H}$---should be used in the evaluation of any form of evidence, both quantitative and non-quantitative evidence. This sections explore how DNA match evidence, the most widely used form of quantitative evidence, should be evaluated using the likelihood ratio. The argument formulated here can be generalized to any form of 'match evidence': fingerprints, shoe prints, bite marks, blood, hair, glass, etc. Match evidence usually consists in a statement by an expert that the defendant matches the physical material associated with the wrongdoing. The match can be between genetic profiles, fingerprints, blood type, show prints, etc. For concreteness, we will focus on genetic, DNA matches. 

Suppose an expert testifies that there is a match between the traces at the crime scene and a sample from the defendant. What is this statement evidence for? At the very least, the match is evidence that the defendant was the \textit{source} of the traces---that the materials found at the scene originate from the defendant. 
The match can also be evidence that the defendant was present at the scene or committed the act, but these claims are more far-fetched, as the chain of inferences become much weaker. We will discuss these complications later in section \ref{sec:lhTwoSTain}. For simplicity, let us focus on the source hypothesis. 

That the defendant match the crime traces---or to be more precise, that an expert witness testifies to that effect---is good evidence that the defendant is the source of the crime traces. But how strong is this evidence? Following the likelihood ratio,  two conditional probabilities should be compared here, $\pr{\textsf{match} \vert \textsf{source}}$ and $\pr{\textsf{match} \vert \neg \textsf{source}}$. However, when experts testify in court about a DNA match, they only use the \textbf{random match probability} as an indicator of evidential strength. This is the probability that a random person, unrelated to the crime, would coincidentally match the crime stain profile. The random match probability is often an impressively low number, say 1 in 100 million or lower. The match, accompanied by an impressively low random match probability, is taken to constitute strong evidence against the defendant. How this analysis compare to one based on the likelihood ratio? The random match can be equated to the probability that someone who is not the source would coincidentally match, $\pr{\textsf{match} \vert \neg \textsf{source}}$. <!---A low random match probability indicates it is  unlikely two people could share the same DNA profile.---> 
This is one of the conditional probabilities that are part of the likelihood ratio. 
Should the other also be reported? In theory, it certainly should, but it practice, it may not make a difference. The random match probability alone can be a good measure of evidential strength so long as the other conditional probability is close to one. That $\pr{\textsf{match} \vert \neg \textsf{source}}$ is low would be enough to ensure that the likelihood ratio is significantly above one. For practical purposes, then, a suitably low random match probability  does reflect the fact that the evidence is strongly incriminating evidence. The lower the random match probability, the more strongly incriminating the match. The conceptual point still stands, though. If $\pr{\textsf{match} \vert \textsf{source}}$ was significantly different from one, reporting only   $\pr{\textsf{match} \vert \neg \textsf{source}}$ would be misleading.

But this analysis, while roughly correct, is not quite precise enough in the denominator or the numerator. Consider the denominator first, $\pr{\textsf{match} \vert \neg \textsf{source}}$. Equating this probability with the probability of a coincidental match (the random match probability) neglects the possibility of false positive matches, that is, someone who is not actually a match may be erroneously reported to be a match. A coincidental match is not the same thing as a false positive match. Two individuals---say the perpetrator and the defendant---may happen to share the exact the same DNA profile. If DNA expert reports that the sample from the crime scene and the sample from the defendant match, this report would be correct, but it would also be a coincidental match. So the sources of error that should be included in the denominator $\pr{\textsf{match} \vert \textsf{source}}$ are twofold: a coincidental match as well as a false positive match. Before we detail how this can be done formally, it pays to comment on the reality of false positive matches. Even if DNA evidence seems quite scientific, the risk of false positives is not negligible [@Shaer2016False].^[For instance, Houston Police Department Crime Laboratory, a large public forensic center in Texas, handles around 500 cases a year. In 2016, KHOU 11, a local television station, sent dozens of profiles processed by the lab to independent experts. The results were not optimistic: police technicians quite systematically misinterpreted samples.] 

One notorious case involving a false positive is that of  Josiah Sutton (then 16) and Gregory Adams (then 19), who were arrested for a rape of a 41-year-old woman. The victim was abducted in a parking lot and assaulted in a driving car (Ford Expedition). A few days after the incident, the victim spotted Sutton and Adams walking down a street, flagged down a patrol car, and accused them of the assault. Both Sutton and Adams had alibis, neither of them matched the victim's original description of the perpetrators. Sutton and Adams agreed to a DNA test to clear their names.  A Houston lab analyst Christy Kim compared their results with DNA obtained from a  vaginal swab, which  contained a mixture of genetic material from at least three contributors, including the victim herself. The lab report did not report a match for Adams, but concluded that Sutton's DNA was consistent with the mixture DNA. In result, in 1999, Sutton was sentenced to 25 years in prison. Later on, a re-examination by prof. William Thompson, indicated that the three DNA profiles typed by Kim (two from blood, one from saliva) varied, despite reportedly coming from a single source. Moreover, Kim failed to report that the DNA from the semen found on the car seat did not match that of Sutton. In effect, the DNA evidence was reprocessed, no DNA match was found, and in 2003 Sutton was released from prison.^[Christy Kim later sued her employer for firing her after the fact and prevailed. Her mistakes were attributable to systemic failures and inadequate supervision.]

This is only one example of quite a few  cases of DNA matching going awry. Anecdotal evidence suggests there are several sources of error [see @thompson2012forensic for a more exhaustive treatment and multiple examples]:
