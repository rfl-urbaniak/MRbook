...
3\% of abused children rock, $\pr{\textsf{child rocks} \vert \textsf{abuse}}=.3$. Note that  the two probabilities need not add up to 1. Similarly,  learning only  that    $\pr{\textsf{child rocks} \vert \textsf{abuse}}=.3$ does not provide full information needed for evidence evaluation, and one also needs information about  $\pr{\textsf{child rocks} \vert \textsf{no abuse}}=.3$. In our particular case, given that  rocking is equally unlikely under either hypothesis, rocking cannot count as evidence of abuse, and any  of the low conditional probabilities involved alone does not allow us to notice this. Thus, in order to avoid exaggerations, both conditional probabilities need to be involved in the evaluation of the evidence [@Royall1997; @triggsCommentWhyEffect; @enfs2015].]  The Bayes factor includes both probabilities, but---as seen before---it requires too much information since it requires an assessment of the prior probability of the hypothesis. 
A less demanding measure that that still keeps track of both probabilities is the \textbf{likelihood ratio}:
\begin{align}
\label{eq:LR}
\tag{LR}
...
All in all, the likelihood outperforms the Bayes factor on several respects. But, of course, there could be other 
measures of evidential strength out there than fares better than the likelihood ratio. One set of measures 
worth considering come from literature in formal epistemology on confirmation theory. The expression 'confirmation' is more used in this literature instead of strength of the evidence (or value, support). An extensive discussion of these measures would detract us from the main task at hand, namely the study of the properties and applications of the likelihood ratio, the most promising candidate widely championed by legal probabilists. We therefore relegate a discussion of these confirmations measures to Appendix \ref{sec:confirmation}. The general lesson is that two conceptually different questions should be distinguished: (1) To what extend should a given piece of evidence change our beliefs about a given hypothesis? Call this the question about confirmation. (2) What is the strength of this particular piece of evidence with respect of that hypothesis? Call this the question about evidential strength. The two questions are obviously related and they overlap. 
The difference is that confirmation depends on prior probabilities. Instead, in the evaluation of legal evidence presented in trial proceedings, the assessment of strength (that might lie within an expert's field of expertise) should be kept separate from the reasonable choice of prior probabilities (which might be beyond the competence of the expert). Confirmation measures are concerned with (1) rather than (2), and thus they are unsuitable for legal applications.
