...
However, there are some clear connections to the probabilities mentioned in the previous passage. One usual assumption [@sprenger2019bayesian]---called the \emph{prior-posterior dependence} is that the level of confirmation is to be a continuous function of $\pr{T}$ and $\pr{E\vert T}$ which is  non-decreasing in the first argument and non-increasing in the second argument. That is, increasing the prior,   should not lower the confirmation level, and increasing the likelihood shold not increase the confirmation level.

One consequence of this assumption is that confirmation of $T$ by $E$, $c(T,E)$ should track the posterior order-wise, that is $c(T,E)>c(T,E')$ just in case $\pr{T\vert E} > \pr{T\vert E'}$ even though $c(T,E)$ should not be identified with $\pr{T\vert E}$. 
...
Yet another adequacy requirement is local equivalence. Theories that are logically equivalent given the evidence should receive equal confirmation from this evidence. Interestingly, all confirmation measures which satisfy prior-posterior dependence, qualitative-quantitative bridge and local equivalence are ordinally equivalent with the posterior probability $\pr{H\vert E}$. Measures that are so ordinally equivalent are said to explicate confirmation as \emph{firmness of belief}, especially if in the probabilities used implicitly rely on total available evidence.

However, another notion of confirmation seems often at play. For instance, even if the posterior $T$ is low, one might still think that a given experiment still speaks strongly in favor of $T$. And relatedly, $E$ can lower the posterior of $T$ while still leading the posterior to be sufficiently high for the firmness confirmation measure to be above the neutrality threshold.  

For this reason, following the second edition of [@carnap1962logical] it is customary to distinguish another notion in the vincinity: confirmation  as increase in firmness of belief. Here, the idea is rather than confirmation is positive (negative) if the posterior of $H$ is higher (lower) than the prior, and neutral otherwise. 
