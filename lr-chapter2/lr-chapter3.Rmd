...
In contrast, if the eyewitness is a friend of the suspect, so that you estimate $\pr{E \vert H}$ to be .6, while the conditions were subpar, so that $\pr{E\vert \n H} = .8\pm .05$, the likelihood ratio range is $.7-.8$ and the impact of such eyewitness evidence is quite different.
...
sNeg2 <- ggplot(scenarioNeg2)+
  ...
  ggtitle("Exculpating eyewitness testimony", subtitle = ("subpar conditions"))+ylab("posterior")
...
sJoint2 <- ggplot(scenarioJoint2)+
  geom_ribbon(aes(x = prior, ymin=posteriorMinJoint2,ymax=posteriorMaxJoint2), fill="darkblue", alpha=1)+theme_tufte()+
