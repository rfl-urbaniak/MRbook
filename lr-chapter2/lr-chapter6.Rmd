...
The posterior probability of $H$ given $E$ could still be low even when the Bayes factor is significantly above one, indicating that the evidence is strongly probative of $H$ despite the low posterior probability. So, as desired,  this measure captures a dimension of the value of evidence that is not reflected in the posterior probability. Unfortunately, the Bayes factor suffers from three shortcomings that make it suitable for applications in trial proceedings.


The first shortcoming is a dependency on the prior probability of the hypothesis. To see why, consider the denominator. It can be unpacked by the law of total probability:
