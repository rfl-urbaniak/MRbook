...
with its probative value or strength. A hypothesis may have a low
posterior probability given the evidence, even though the evidence
...
the final decision. But, as a preliminary step, it is useful to locally
evaluate the impact of an individual piece of evidence \(E_i\) on the
probability of a specific hypothesis \(H_i\). When lay witnesses and
experts testify at trial, the assessment of the evidential value of
their individual testimonies should precede their aggregation into a
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/fig-BayesFactorPrior-1} \end{center}
...
So suppose we are after a measure of evidential strength that (i) does
not depend on priors, (ii) places no unreasonably heavy cognitive
requirements, and (iii) does not fall prey to the problem of irrelevant
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/effect-evidence-b-1} \end{center}
...
should be kept separate from prior probabilities. Some confirmation
measures may be seen as concerned with (1) rather than (2), and thus
they are not always suitable for the evaluation of evidence in trial
proceedings.
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/fig-fpplr-1} \end{center}
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/fig-fpfnplr-1} \end{center}
\caption{Positive match: negligible impact of the  false negative probability FNP on the likelihood ratio, assuming RMP=$10{^-9}$. For FPP=0 the likelihood ratio remains very close to $10^8$, and for FPP = $.01$ it drops drastically ar remains very close to 50.}
\label{fig:fpfnplr}
\end{figure}
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/fig-fpfnplr-exc-1} \end{center}
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/ex-inc-dna-b-1} \end{center}
...
So, we have seen that even a seemingly small error probability trumps
...
matches).\footnote{The impact of false positive and false negative
  probabilities is not symmetric across positive and negative matches
  when evidential value is measured by the likelihood ratio. For
  positive matches, even a small false positive probability can cause
  the likelihood ratio to drop significantly, as Figure \ref{fig:fpplr}
  and \ref{fig:fpfnplr} show. Instead, for negative matches, Figure
  ...
  relatively close to zero. But this asymmetry is just an artifact of
  ...
  \(S\) from .5 to just about .04. See, more generally,
  \ref{fig:ex-inc-dna}. Furthermore, a positive match followed by a
  negative match, each subject to the same error probabilities, would
  have no impact on the posterior probability of the source hypothesis.
  All in all, the likelihood ratio should be interpreted carefully, a
  point to which we will return later in this chapter.} Instead, a false
negative probability has a negligible impact on the value of
incriminating DNA evidence, while a false positive probability has a
negligible impact on the value of exculpatory DNA evidence. So this
analysis shows which error probabilities we should be concerned about
...
general \todo{add references}. To this end, numerical data should be
...
abbreviations, and algebraic manipulation.\footnote{The probability of a
  ...
  \(\pr{R \vert \n ERR, \n S}=RMP\). The probability that a reported
  match occurs when the source hypothesis is true and no error has made
  is assumed to be one, so \(\pr{R \vert S, \n ERR} =1\).}
...
subject to little empirical testing. \todo{reference missing} In fact,
...
\begin{center}\includegraphics[width=1\linewidth]{lr-chapter5_files/figure-latex/eyewitness2b-1} \end{center}
