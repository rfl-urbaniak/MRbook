...
%---prosecutor and defendant in a criminal trial, plaintiff and defendant in a civil trial---
...
%, since the litigants have an interest in defending their own version of what happened. 
...
%After all, hardly any evidence can establish the facts with certainty. 
...
%Proponents of this view claim that evidence presented at trial can in principle be weighed by means of probability theory. 
...
Probability theory also helps to clarify certain pivotal ideas such as the relevance of the evidence (Section \ref{sec:relevance}). Further, probability theory and cognate theories %such as expected utility theory 
can be used to theorize about the fallibility of trial decisions and standards of proof.
%---decision rules about when the evidence is strong enough to warrant a conviction in criminal cases or a finding of liability against the defendant in civil cases 
(Section \ref{sec:burden}).
%The literature on legal probabilism has devoted considerable energies to formulate probabilistic theories of the legal standards of proof. 
...
`It would be useful'---he wrote---`if definite limits for moral certainty were established by the authority of the magistracy. For instance, it might be determined whether \nicefrac{99}{100} suffices or whether \nicefrac{999}{1000} is required. Then a judge \dots would have a reference point to keep constantly in mind in pronouncing a judgment' (English translation in \cite{bernoulli-trans2006}, Ch.\ 2). 
%
%\cite{Cullison1969Probability} and 
...
%\cite[see, for example, ][]{kaye1986admissibility,NRCI1992, Robertson1995DNA-Evidence:-W, Koehler1996On-Conveying-th}.
...
%
%other scholars gave credibility to the second tenet of legal probabilism, namely that legal decision rules can be explicated in probabilistic terms. 
...
In response to these seminal contributions, %on legal probabilism that appeared in the  sixties and  seventies, 
...
offering hybrid theories \citep{verheij2014catch, Urbaniak2017Narration-in-ju}. %REDUNDANT  We will examine the different sides as fairly as possible. 
...
%\todo{Note to self: Legal opinions by lawyers as probability assessments of success, ABA guidelines. Makes money out of betting whether you can won case. Injury cases contingency.}
...
% BELOW ARE SOME LEFT OVER REFERENCES THAT I AM NOT SURE WHERE TO PUT
...
% When an issue is litigated at trial, the parties 
%make their case by presenting evidence. The fact-finders, experienced judges or lay jurors, assess and weigh 
%the evidence, and on that basis reach a decision in accordance with the governing standard of proof. 
%The standard can be `proof beyond a reasonable doubt' in a criminal case and `preponderance of the evidence' in a civil case. 
 %Since the evidence relied upon at trial and the methods used for its assessment cannot be entirely free from error, trial decisions are almost always taken under conditions of uncertainty.
 %Probability theory is a calculus of uncertainty, so it is only natural to  seriously  consider the idea that it  offers  a formal framework for weighing evidence at trial.
...
when they assess evidence at trial, such as the prosecutor's fallacy, the base rate fallacy and the defense attorney's fallacy. %(\ref{sec:fallacies}).
%Cold-hit DNA matches are discussed as a case study(\ref{subsec:cold-hit}). 
The section also 
examines how probabilities are assigned to hypotheses  %(\ref{subsec:sensi-ana}) 
...
This interpretation dispels the worry that since past events  did or did not happen, their probabilities should be 1 or 0. Even if their objective physical chances are 1 or 0, %(and even this is not uncontroversial), 
...
%\todo{revised last bit (will abbreviate this as RLB), check}
...
Although base rate information should not be overlooked, paying excessive attention to it and ignoring other evidence leads to another fallacy, called \emph{defense attorney's fallacy}. %Consider a simple case involving DNA evidence. 
...
Given the upward shift in the probability, the  match should not be considered worthless evidence (more on this later in \ref{subse:LRlogical}).  %since $\pr(\textrm{source}\vert \textrm{DNA match})=\nicefrac{1}{100}$ and
%$\pr(\textrm{source})=\nicefrac{1}{10^6}$. 
%
...
probability theory helps to assess the strength of evidence. It begins with a discussion of probabilistic measures such as the Bayesian factor and the likelihood ratio, %(\ref{subse:LRlogical}) 
...
how evidentiary strength may depend on the choice of hypotheses, using  %(\ref{subsec:levels})
 the two-stain problem as an illustration. %(\ref{subsec:two-stain})
...
%Moreover, if such considerations are to
%be presented to a fact-finder, it is rather unlikely that they would be
...
From a legal standpoint, the requirement that guilt be established with high probability, still short of 100\%, accords with the  principle that proof beyond a reasonable doubt is the most stringent standard of all but %at the same time 
...
%\todo{This seems too hasty, please explain in more detail.}
...
\cite{ho2008philosophy} and \cite{Haack2014-HAAEMS} %\todo{you used an old key which didn't work here, did you mean the book or the legal probabilism paper?} 
...
%One of the most discussed  difficulties with probabilistic thresholds has to do with naked statistical evidence. 
...
%This is so despite the fact that trace or eyewitness evidence may give rise to erroneous decisions with the same or even greater probability than naked statistical evidence.  

%Are we really justified in exhibiting this intuitive preference for eyewitness testimony and trace evidence?
...
Legal probabilists can also argue that the puzzles of naked statistical evidence are confined to hypothetical cases, and our judgments about statistical evidence  may well differ in more realistic cases \citep{hedden2019, ross2020}.
 ...
 Few have defended such revisionist views, however. Even when it advances one variant or another of legal probabilism, the literature has predominately tried to vindicate the intuitive asymmetry between naked statistical evidence and other forms of evidence. %The paradoxes of naked statistical evidence have been approached from many perspectives, epistemological, moral, legal or a combination of them. 
...
advances a similar argument.  
  This solution has been criticized by \citet{Urbaniak2019standards2} for a number of reasons. % , because for the cases in which it works the assumptions needed to make it work are fairly strong, and because some unresolved cases remain.    For one thing, Cheng assumes that the costs of wrongful decision is the same, be it conviction or acquittal. Moreover, the 
...
In legal contexts, a claim $A$ is sometimes used to establish another claim $B$ which in turn (perhaps together with $A$) is used to establish a third claim $C$. For example, the fact that the defendant is the source of the crime traces ($A$) is used to establish that he visited the crime scene when the crime occurred ($B$) which is then used to establish that he committed the crime ($C$). This inference pattern can be modeled  probabilistically. The conditional probability $\pr(B \vert A)$ would correspond to the reasoning  from $A$ to $B$ and $\pr(C \vert B\wedge A)$ would correspond to the reasoning  from $B$ (together with $A$) to $C$. The reasoning from the initial claim $A$ to the final claim $C$ would correspond to the conditional probability $\pr(C \vert A)$. %At this point, the natural question to ask is how high the intermediate conditional probabilities should be in order to establish the final claim $C$ based on $A$ by, say, the preponderance standard and thus $\pr(C \vert A)$ above the $50\%$ threshold.   
...
\noindent 
\eqref{eq:inf1} rewrites the probability of a conjunction in terms of a conditional probability. %, with an additional proposition $A$ kept in the condition.
\eqref{eq:inf2} results from \eqref{eq:inf1} by dividing both sides by $\pr(B\vert C\wedge A)$. Then, \eqref{eq:inf3} follows because $\pr(B\vert C\wedge A)$ belongs to the interval $(0,1]$ and so dividing by it (in the right-hand side of \eqref{eq:inf2}) cannot lead to a larger number. The move in \eqref{eq:inf4} is analogous to the one in \eqref{eq:inf1}. % the difference lying only in the order in which we look at the conjuncts. 
Finally,   \eqref{eq:inf5} follows from \eqref{eq:inf3} and \eqref{eq:inf4} \cite[p.\ 69]{Cohen1977The-probable-an}.}
%
%This results shows that the probability of the ultimate claim  $C$  conditional on $A$ may be greater than the product of conditional probabilities related to the intermediate steps. This, he insists, means that 
%The resemblance with the conjunction paradox is evident. 
...
%to estimate the impact of corroborating evidence will yield a combined  likelihood ratio that 
...
An examination of this topic, along with a detailed discussion of Cohen's objections and candidates for a solution, can be found in \citep{urbaniak2019ProbabilisticModelsLegal}. % where one important factor at play is the disproportion between there being only one true version of events, and many different false scenarios that the witness could have said---assuming the witnesses are independent, the chances on them agreeing on falsehoods are very low.
...
class problem. 
The reference class problem, originally formulated by  \cite{venn1866logic}, arises because the same event may belong to multiple reference  classes in which the frequencies of the event in question are different. %Which frequency should you use as your estimate and why? 
...
Shonubi fit the reference class `people found carrying heroin while travelling between Nigeria and New York' but also the class `toll collectors at the George Washington bridge'. Why rely on the former and not the latter to make inferences about how much heroin Shonubi smuggled into the United States? %Depending on the choice of the reference class, Shonubi may be more or less likely to have smuggled a total of, say 2,000 grams of heroin. %After all, few if any toll collectors have ever smuggled heroin into the United States. %\citet{colyvan2001crime}. 
...
The court wrote: `It does not matter how many Hispanics, Caucasians, Blacks, or Native Americans resemble the perpetrator if the perpetrator is actually Asian.' %We already know that it does matter, because 
