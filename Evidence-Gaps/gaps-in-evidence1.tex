...
relevant for adjudicating a dispute about a matter of fact. Contrary to
existing arguments, we will see that the answer is in a wide range of
cases surely positive.
...
\(\pr{L \vert E_a \wedge E_m}\). Of course we agree with Kaye that once
such a probability can be calculated, it can be used further down the
path of evidential reasoning. The challenge, though, is that Kaye's
approach assumes it is available, while we want to elaborate on how it
falls out from other factors and probabilities that are often easier to
ascertain or elicit.

One key question is whether this evidence-missing evidence \(E_m\) is
...
cancel each other out. Moreover, the plausibility hinges on the
missigness being at random. Once this assumption is false and we have
reasons to think that given the context, missigness itself is more
likely on some scenarios than on others, even the intuition that missing
evidence should balance out in expectation and therefore that missigness
should be ignored does not work.
...
and arrows between the nodes. A graphical representational of the trip
and fall case is given in Figure \ref{fig:dag-missing-video}. The
\textit{nodes} stand for event variables, such as the slippery floor
...
This line of questioning is well taken. But we think the distinction
drawn here is more of a quantitative than a qualitative type. It by no
means is clear what ``scrutinize'' means and whether any non-missing
piece of evidence can be ``scrutinized'' in this sense---it is even
unclear whether this sort of scrutiny would be systematically
epistemically or decision-theoretically valuable. For instance, there is
a stereotype that the cross-examination of an eyewitness contributes to
the more adequate evaluation of the value of her testimony, but most of
the existing psychological research suggests otherwise,
\todo{refer to my discussion of this in another paper} as (somewhat
unsurprisingly), a range of adversarial questions asked long time after
an event in stressful circumstances may be subjected to and result in
various biases. Moreover, in principle there is no reason why one
couldn't scrutinize the evidence about the missing evidence itself, say
by questioning the shop personnel about the data-management practices,
to evaluate how likely it is that the evidence went missing due to
negligence rather than to ill will. Having said this, missing evidence
does often involve more uncertainty---and ultimately, we think that such
sort of uncertainty could be better modeled in terms of higher-order
probabilities, but that's a discussion for another day.

The fact that often there is more uncertainty about missing evidence,
nevertheless, does not undermine the claim that the missing evidence can
be relevant, as we have shown. There may be overriding reasons to ignore
it, but its epistemic value is not necessarily null. To better
