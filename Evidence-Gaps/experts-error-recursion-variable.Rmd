...
## Three experts, different reliability 


Suppose the first expert is highly reliable while the other two are much less. What happens when we combine the three experts opinion together?



```{r}
## expert report function
...
expert_report_high <- function(w, s){
  
  r1_high <- 0.95
  r2_high <- 0.95
  expert_report(r1_high, r2_high, w, s) 
}

expert_report_mid <- function(w, s){
  
  r1_mid <- 0.6
  r2_mid <- 0.6
  expert_report(r1_mid, r2_mid, w, s) 
}


expert_report_low <- function(w, s){
  
  r1_low <- 0.51
  r2_low <- 0.51
  expert_report(r1_low, r2_low, w, s) 
}
...
# Probability of hypotheses calculations 

posterior_odds <- function(n, w) {
  
         if (n == 1)                              # one expert, high rel
            return(
               ifelse(
                  expert_report_high(w, s) == 1, 
                  ((-prior)/(prior-1))*(r1_high/(1-r2_high)), 
                  ((-prior)/(prior-1))*((1-r1_high)/(r2_high))
                    )
                  )
        
        else if (n == 2)                              #  two experts, low rel 
            return(
               ifelse(
                  expert_report_low(w, s) == 1, 
                  posterior_odds(1, w)*(r1_low/(1-r2_low)), 
                  posterior_odds(1, w)*((1-r1_low)/(r2_low))
                    )
                  )
  
        else if (n == 3)                              # three experts, low rel
            return(
               ifelse(
                  expert_report_low(w, s) == 1, 
                  posterior_odds(2, w)*(r1_low/(1-r2_low)), 
                  posterior_odds(2, w)*((1-r1_low)/(r2_low))
                    )
                  )
...
## sensitivity, specificity and accuracy of probability-based decision threshold
...
grid.arrange(p1, p2, p3, p1n, p2n, p3n, ncol=3) 
```
...
## Three experts, same reliability 


Suppose the three expert are midly equally reliable. What happens when we combine the three experts opinion together?
...
posterior_odds <- function(n, w) {
         ...
         if (n == 1)                              # one expert, high rel
            return(
               ifelse(
                  expert_report_mid(w, s) == 1, 
                  ((-prior)/(prior-1))*(r1_mid/(1-r2_mid)), 
                  ((-prior)/(prior-1))*((1-r1_mid)/(r2_mid))
                    )
                  )
        
        else                            #  two experts, low rel 
            return(
               ifelse(
                  expert_report_low(w, s) == 1, 
                  posterior_odds(n-1, w)*(r1_mid/(1-r2_mid)), 
                  posterior_odds(n-1, w)*((1-r1_mid)/(r2_mid))
                    )
                  )
