...
<!---## The compatibility problem [to be revsied still and moved somewhere else]-->


<!---it is hard to make sense of the idea that an agent would learn that a probabilistic  measure is  incompatible with the evidence.   How, exactly, does the evidence exclude probability measures, other than if the evidence is that a proposition is true (false) while the measure assings to it probability zero (one)? 
--->
...
In quantitative terms, degrees of evidential value (or support) 
...
<!--
For two items of evidence, $E_a$ and $E_b$, 
the posterior odds are obtained by multiplying the likelihood ratio by the prior odds while filling in the required probabilities:
...
Likelihood ratios can also be used for assessing the value of multiple pieces of evidence in the aggregate, again relative to a pair of hypotheses of interest. To evaluate items of evidence in the aggregate, it is enough to multiply their individual likelihood ratios. 
$$
...
<!--To be sure, even in more complex scenarios, precise probabilism handles evidence aggregation elegangly, with the help of Bayesian networks. Suppose two pieces of information bear on different hypotheses and probabilistically depend one another in important ways. Think, for example, of two witnesses testifying in a trial about two different yet related issues, say the defendant's whereabouts and the defendant's motive. Bayesian networks have been been developed to model these more complex evidential scenarios. Bayesian networks have a graphical and a numerical component. The grahical part visually represents relationships of probabilistic depedency between different hypotheses and items of evidence. The numerical part consists of probability tables in which precise probabilities are entered so that the Bayesian network can run the calculations. The calculations can quickly get out of hand, so softwares exist to perform the calculations automatically.-->


\noindent
The equality holds provided $E_1, E_2, \dots, E_k$ are probabilistically independent conditional on hypothesis $H$ and its negation. Think, for example, at several diagnostic tests performed by independent laboratories or independent witnesses in a trial testifying about the same issue. The precise probabilist holds that, in cases of evidence aggregation, the precise probabilities must be put together following the formula above. We will show, however, that this approach risks over- or under-estimating the value of aggregate evidence. (We will focus on the likelihood ratio, but the claim we defend can be generalized 
to other measures.\footnote{On different measures of degrees of evidential value (or support, confirmation), see \textbf{CITE FITELSON}.})
...
\noindent
So far so good. But how do we fill in the numbers? The precise probabilist will answer along these lines. The numerators can be equated to one: if the defendant is a contributor, the laboratory will declare a match for sure. This is a simplication, but it will do for our purposes. To fill in the denominators, a trial expert will provide the match probabilities. They express the likelihood that, by coincidence, a random person (or a random dog) who is not a contributor would still match.  The match probabilities are approximated by counting of many matches are found in a representative sample of the human population (or the canine population). Suppose the matching hair type occurs  `r round(hairMean,4)` times in a reference database, and the matching dog fur type occurs  `r round(dogMean,4)` times in a reference database.^[Probabilities have been slightly but not unrealistically modified to be closer to each other in order 
to make a conceptual point: the same first-order probabilities, even when they sound precise, may come with different degrees of second-order uncertainty (more on this soon). The original probabilities were  1/100 for the dog fur, and 29/1148 for Wayne Williams' hair.] 
These frequencies can fill in the match probabilities. 
Putting everything together:
\begin{align*}
\frac{{\pr{\s{dog} \vert \s{source}}}}{{\pr{\s{dog}  \vert \neg \s{source}}}} \times
 \frac{{\pr{\s{hair} \vert \s{source}}}}{{\pr{\s{hair} \vert \neg \s{source}}}}
& =  \frac{1}{`r hairMean`} \times  \frac{1}{`r dogMean`} = \frac{1}{`r hairMean * dogMean`}
\end{align*}
...
<!---First, in order to find out the posterior odds, the likelihood ratios must be multiplied by the prior odds $\frac{\pr{\s{source}}}{\pr{\neg \s{source}}}$. What precise value should they take? This question is hard to answer definitively. To circumvent this difficulty, @fig-impactOfPoint shows how the items of match evidence, combined, change the probability of the source hypothesis given a range of possible priors.  This strategy---known 
as *sensitivity analysys*---shows that the posterior of .99 is reached as soon as the prior is higher than  `r threshold`.^[These calculations 
assume that the probability of a match if the suspect and the suspect's dog are the sources is one.]
-->
<!--While perhaps not sufficient for outright belief in the source hypothesis, the match evidence is extremely strong:
 a minor additional piece of evidence could make the case against the defendant overwhelming. 
-->

<!--
Sensitivity analysis moves away from precise probabilism. It gives up on the prospect of assigning precise numbers to the prior probabilities. 
--->

This is the story about evidence aggregation (in the simple case) told from the perspective of precise probabilism.  But, as we know now, this story misses something crucial. The match probabiltiies $\pr{\s{dog}  \vert \neg \s{source}}$ and $\pr{\s{fur}  \vert \neg \s{source}}$ are themselves subejct to uncertainty. They are assessed by looking at sample data. Suppose that the match probability for the hair evidence is based on 29 matches found in a sample database of size 1148, while the random match probability for the dog evidence is based on finding two matches in a smaller database of size 78. The relative frequencies are about 2.5% in both cases, but the two samples differ in size. The smaller the sample, the greater the uncertainty about the match probabilities. Since the match probability for hair evidence is based on a larger sample than the match probability for dog fur evidence, there is less uncertainty about the former than the latter. So simply reporting the exact the numbers `r round(hairMean,4)` and  `r round(dogMean,4)` makes it seem as though the value of the two items of match evidence is the same, but actually it is not. 

Here is another variant of the same problem. Imagine that the match probabilities associated with two items of match evidence derived from very small population samples.  Suppose two matches were found in the dog fur database and two matches in the human hair database, both of size 10. 
In this scenario, the evidence against the defendant consists of two matches, one based on dog fur and the other based on human hair. This set-up is the same as our running scenario, but the samples are smaller. The combined evidential value of the two matches should be worthless. And yet, by multiplying 0.02 and 0.02, it would seem overwhelming. If, on the other hand, the samples were much larger, the evidential value of the two matches would be the same, but intuitively it should be greater. The evidence should be assessed as having different values in these different scenarios---smaller versus larger samples, and anything in between---but precise probabilism makes no difference between them. So it will inevitably overestimate and underestimate the value of the evidence in some scenarios. 
...
<!-- Now, the representor covers
  ...
  -->
The range is wider for dog fur match evidence than hair match evidence. This is as it should be: the uncertainty about the the dog fur match probability is greater since the sample database was assumed to be smaller. To assess the joint uncertainty in the context of imprecise probabilism, it is enough to focus on what happens at the edges of the two intervals. Reasoning with representor members at the edges of the intervals will yield the most extreme probability measure the impreciser is going to be committed to, the worst-case and best-case scenarios. On the interval approach, we end up with a new range for the joint evidence: (.00003, .003811). The two matches, combined, could be much stronger evidence against the defendant (lower bound) or much weaker evidence (upper bound) than previously thought. This is not particularly clarifying.\footnote{Redoing the calculations using the upper bounds of the two intervals, $.037$ and $.103$,  yields the following:
\begin{align*}
...
Using plausible ranges for the match probabilties leaves the impression that any value in the interval is just as good as any other. Perhpas we should just pick the middle value as representative of the interval? But this choice will misrepresent the data. To see why, consider @fig-densities (upper part). It depicts the probability distribution of different match probabilities given the sample data---the actual number of matches found in the sample database. By hypothesis, 29 matches were found in the sample database of human hair of size 1148, and 2 matches were found in the sample database of dog fur of size 78.  As expected, some random match probabilities are more likely than others.  Further, since the sizes of the two databases are different, the distribitions have different spreads: the smaller the database the greater the spread, the greater the uncertainty about the match probability. In light of this, now consider @fig-densities (lower part). It depicts the probability distribtion for the joint match probability associated with both items of match evidence, hair and fur evidence. Interestingly, the distribution for the joint evidence 
...
it is not clear where such precise values come from.  The standard response invokes *sensitivity analysis*: a range of plausible values is tested. As already discussed, this approach ignores the shape of the underlying distributions. Sensitivity analysis  does not make any difference between probability measures (or point estimates) in terms of their plausibility, but some will be more plausible than others. Moreover, if the sensitivity analysis is guided by extreme values, these might play an undeservedly strong role. 
