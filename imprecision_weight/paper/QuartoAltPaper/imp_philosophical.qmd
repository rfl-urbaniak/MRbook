...
With this notation, we can now formulate our notion of the inaccuracy of a probability distribution $p$ if the true hypothesis is $\theta_k$. It's the KL divergence between $p$ and $Ind^k$.

\begin{align*}
I_{\text{DK}}(p, \theta_k)  = D_{KL}(Ind_k\vert \vert p)
\end{align*}
...
We will now give a higher-level description of the proof of strict propriety of the scoring rule $I_{DK}$. The first step is to define the score's expected inaccuracy \todo{Score's EI or EI of p from the persp. of q?}, as follows: 
