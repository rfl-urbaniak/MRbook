...
the evidence woud have no value for $H$.
...
For the precise probabilist, filling in the precise probabilities in the formula above gives an assessment of the value of aggregate evidence. But, as we will soon see, this approach risks over- or under-estimating the value of the evidence. (We will focus on likelihood ratios, but the claim we defend can be generalized to other measures of evidential value and to the posterior probabilites.\footnote{On different measures of degrees of evidential value (or support, confirmation), see \textbf{CITE FITELSON}. Posterior probabilities can be obtained from the likelihood ratios via the prior probabilities. For multiple items of evidence, $E_1, E_2, \dots, E_K$, the posterior odds are obtained by multiplying the likelihood ratio by the prior odds, as follows:
...
This is the story about evidence aggregation (in the simple case) told by the precise probabilist.  But this story misses something crucial. As it happens, the match probability for the hair evidence is based on 29 matches found in a sample database of size 1148, while the match probability for the dog evidence is based on finding two matches in a smaller database of size 78. The relative frequencies are about $.025$ in both cases, but the two samples differ in size. The smaller the sample, the greater the uncertainty about the match probabilities. So simply reporting the exact the numbers `r round(hairMean,4)` and  `r round(dogMean,4)` makes it seem as though the value of the matches is the same, but actually it is not. 

Here is another way to bring the problem into sharper focus. Suppose $.02$ the match probabilities associated with two matches rely on small population samples: two matches occur in a dog fur database and two matches occur in a human hair database, where both databases are of size 10. 
Suppose, as before, both the defedant's hair and defendant's dog match the traces at the crime scene. By mutiply the individual likelihood ratios associated with the two matches, their evidential value against the defendant would seem overwhelming: $1/.02\times 1/.02=`r 1/(.02*.02)`$. But, since the match probabilities are based on frequencies resulting from small databses, the evidential value of the two matches should be close to worthless. Whether the samples are small or extremely large, or anything in between, a precise likelihood ratio would count the two matches as having the same evidential value. But, intuitively, their value should be quite different in these scenarios. So, in one case or another, precise probabilism will inevitably overestimate or underestimate the value of the evidence. 
...
the worst-case probability  for their conjunction is  $xy$. But this conclusion does not follow if the margin of error (credible interval) is fixed.
...
So, what are we to conclude? Using plausible ranges for the match probabilities leaves the impression that any value in the interval is just as good as any other. Perhpas we should just pick the middle value as representative of the interval? But relying on the entire interval or the middle value will misrepresent the evidence. To see why, consider @fig-densities (upper part). It depicts the probability distribution of different match probabilities given the sample data---the actual number of matches found in the sample database. By hypothesis, 29 matches were found in the sample database of human hair of size 1148, and 2 matches were found in the sample database of dog fur of size 78.  As expected, some random match probabilities are more likely than others.  Further, since the sizes of the two databases are different, the distributions have different spreads: the smaller the database the greater the spread, the greater the uncertainty about the match probability. In light of this, now consider @fig-densities (lower part). It depicts the probability distribtion for the joint match probability associated with both items of match evidence, hair and fur evidence. Interestingly, the distribution for the joint evidence 
  ...
  relying on the edges---or taking central values as representative of the interval---can lead to overestimating or underestimating the probabilities at play. <!-- This also means that, following our advice on how to illustrate the impact of evidence on prior probabilities a better representation of the dependence of the posterior on the prior should comprise multiple possible sampled lines whose density mirrors the density around the probability of the evidence (@fig-lines).-->\todo{What is the mean of the joint distribution? Does it coincide with the value that precise probabilism would give? Does the mean of the joint distribution change depending on the sizes of hair and dog samples?} 
...
We have looked at a simple case of evidence aggregation: the different pieces of evidence were about the same hypothesis and assumed to be probabilistically independent. In more complex cases, precise probabilism handles evidence aggregation with the help of Bayesian networks. Suppose two pieces of information bear on different hypotheses and probabilistically depend one another in important ways. Think, for example, of two witnesses testifying in a trial about two different yet related issues, say the defendant's whereabouts and the defendant's motive. Bayesian networks have been been developed to model these more complex evidential scenarios. They have a graphical and a numerical component. The grahical part visually represents relationships of probabilistic depedency between different hypotheses and items of evidence. The numerical part consists of probability tables in which precise conditional probabilities are entered so that the Bayesian network can run the calculations. The calculations can quickly get out of hand, so softwares exist to perform the calculations automatically. As we will see, Bayesian network can also be used to handle imprecise and higher-order probabilities. When they are so extended, they inherent the 
difficulties of imprecise probabilism as well as the advatanges 
of high-order probabilism. This is the topic of this section. 
...
The choice of the probabilities in the Bayesian network is quite specific, and 
it often not clear where the precise values come from or whether they are justified.  
The standard response to these concerns is to invoke *sensitivity analysis*: a range of plausible values is tested. 
Sensitivity analysys is another variation on the interval approach we considered earlier. As already discussed, the interval approach and sensitivity analysis ignore the shape of the underlying distributions. They do not make any difference between probability measures (or point estimates) in terms of their plausibility, but some will be more plausible than others. Moreover, if the sensitivity analysis is guided by extreme values, these might play an undeservedly strong role. 
