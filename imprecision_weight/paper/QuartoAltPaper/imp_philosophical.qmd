...
As already noted, one challenge for the imprecisers is providing a proper scoring rule that would be a counterpart of, say, the Brier score for the precise case. Imprecise probabilism cannot meet this challenge, but we show that higher-order probabilism can.  The task is to define a scoring rule for comparing the accuracy of probability distributions and show that it is proper.
The scoring rule we propose is based on well-known measures of divergence between probability distributions, building on existing work on this topic (@HersbachDecomp2000, @Pettigrew2012Epistemic-Utili, @GneitRafter2007). We first show that this scoring rule is improper given the assumption that there are only two possible outcomes. \todo{Is this summary correct? Check and modify as nedded!} But, as we will see, this assumption is unwarranted. When it is dropped, the scoring rule turns out to be proper. 

  Our starting point are two well-known measures of divergence between probability distributions. For computational ease, we will use a grid approximation instead of continous distribitions, as in practice we are unable to work with infinite precision.^[Note  also for instance that there are no readily computable solutions to the integral used in the definition of CRPS, although it can sometimes be evaluated in closed form [ @GneitRafter2007, p. 366].]  To this end, $x$ will denote the finite vector of discrete states under consideration (by default, we will use equally spaced values between 0 and 1 here). 
\begin{enumerate}
...
\item Kullback-Leibler Divergence measure: 
...
\todo{Are these the scoring rules whose propriety will be demonstrated? Or are these improper scoring rules? Section is ambiguous on this. Is the problem that we take 0 and 1 as the only outcomes? If so, I'd say it explicitly.}

\noindent Analogously, KL-inaccuracy of $p$ is defined, as follows: 
\begin{align*}
...
Another notion is needed, that of *expected inaccuracy*, which we will define using the discretized vector $x$ of outcomes. \todo{Why need expected inaccuracy? Is it nedded to show (im)propriety?}

$$\mathbb{E_{discretized}}I(p,q) = \sum  I(p,w) q(w),$$

\noindent where $I(p,w)$ is an inaccuracy score of distribution $p$ relative to the possible world $w$, whethere in the form of KL-divergence or Cramer-Von-Mises measure. In practice, instead of working with $w$ and saying things like $V(w)=x$, we will be working with scalars and finite vectors, and simply say that the outocome is $x$.

If you think there are only two possible outcomes, $\mathsf{tails}$ and $\mathsf{heads}$, *and you take the distributions* expected values as the probabilities they assign to those outcomes, you might think a seemingly simpler notion of expected inaccuracy is preferrable:

$$\mathbb{E_{binary}}(p,q) = I(p,\mathsf{heads}) \mathbb{E}q(\mathsf{heads}) + I(p,\mathsf{tails}) \mathbb{E}q(\mathsf{tails}).$$

\todo{Are you going to show impropriety---that is, Ebinary(p,p)< Ebinary(p, q), for some q? If so, better to be explicit about this.}
\noindent  We will start by reflecting on this binary approach, but we will soon see that it runs into difficulties. To fix ideas, consider a variation of a scenario by @Schoenfield2017accuracy. A rational agent is invited to engage in a bet by an opponent who has a representative bag of coins coming from a factory where the distribution of bias among the coins produced, the true generative process, is known. It is a mixture of two normal distributions  centered at $.3$ and at $.5$, both  with standard deviation of $.05$. The opponent randomly selects one of these coins and flips it. The rational agent knows all the details of this set-up. 
...
The discussion so far, while based on an example, may raise questions about the strict propriety of the KLD as an inaccuracy measure. To address this concern, a proof is provided in the appendix. \todo{From the footnote the scorign rule seems to -log p(theta). how does this connect with teh divergence measures fro before?}In a nutshell, the argument demonstrates that for a second-order discretized probability mass $p$ over a parameter space $[0,1]$, with the actual probability denoted as $\theta$,  the Kullback-Leibler divergence of $p$ from 
the indicator distribution  of $\theta$  (which assigns 1 to $\theta$ and 0 to all other parameter values in the parameter space) is expressed as $\mathcal{I}_{\dkl}^2$.^[The argument generalizes 
