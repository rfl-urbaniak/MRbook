...
`r round(mean(jointEvidenceThree2),6)`, `r round(mean(jointEvidenceFive2),7)` and `r round(mean(jointEvidenceSeven2),10)` (for the higher-order probabilist, based on larger databases of size 20,000).] Who should be trusted? Since the higher-order probabilist takes into accout more information---that is, information about the strength of evidence as reflected in sample size---there is good reason to think that the higher-order probabilist should be trusted more than the precise probabilist.
