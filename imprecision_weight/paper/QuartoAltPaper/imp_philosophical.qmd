...
This challenge becomes particularly pronounced in the context of imprecise probabilism, where meeting the requirement of strict propriety proves elusive. However, this section aims to illustrate that higher-order probabilism can indeed offer a scoring rule that has this crucial characteristic.
...
The scoring rule we propose is based on well-known measures of divergence between probability distributions, building on existing work on this topic (@HersbachDecomp2000, @Pettigrew2012Epistemic-Utili, @GneitRafter2007). 

A key modification to this framework involves abandoning the assumption that only two possible outcomes (truth and false) exist. Once we present the positive proposal, we will illustrate how such an assumption is problematic.


Our method employs Kullback-Leibler Divergence between probability distributions. For computational ease, we will use a grid approximation instead of continous distribitions, as in practice we are unable to work with infinite precision.  To this end, $x$ will denote the finite vector of discrete states under consideration (by default, we will use equally spaced values between 0 and 1 here).   Kullback-Leibler Divergence is defined as follows: 
$$D_{\text{KL}}(p \,||\, q) = \sum_{x} p(x) \log\left(\frac{p(x)}{q(x)}\right)$$

\noindent It is a standard information-theoretic measure of divergence of $q$ from $p$ from the perspective of $p$.\footnote{In the continuous case, we would need to use the so-called differential KL divergence, but our main points wouldn't change otherwise.}
\noindent


To convert this measure into the measure of *(in)accuracy*, we need to define relations between probability distributions and possible worlds. Let the outcome under consideration in $w$  be $V(w)$ (in a very simple case you might take there to be two possible oucomes, 0 and 1, the range of $V$ is simply the vector of possible states). The  KL-inaccuracy of a distribution $p$ with respect to $w$ is defined, as follows:
...
Let's now compare, and show on an example the difference between our approach, which uses KL-divergence and a fairly standard  approach represented by CRPS^[Note that there are no readily computable solutions to the integral used in the definition of CRPS, although it can sometimes be evaluated in closed form [ @GneitRafter2007, p. 366].].
todo{Say that it is used for example by Konek}. We will start by providing a definition of another inaccuracy measure, Cramer-Von-Mises. In the discretized version, it is defined as:\footnote{In the continuous case, this measure is defined as the area under the squared Euclidean distances between the corresponding cumulative density functions That is, $D_{CM}(p,q)  = \int_{0}^{1} \vert P(x) - Q(x)\vert^2 \, dx$. Looking at cumulative densities ensures that all densities are considered on the same scale.}
\begin{align*}
D_{\text{CM}}(p,q) & = \sum_{x} \vert P(x) - Q(x)\vert^2 
\end{align*}

\noindent where $\mathsf{P}$ is the cumulative probability corresponding to the probability distribution $p$.

\noindent This inaccuracy measure can be used to define the inaccuracy of $p$ with respect to $w$ as follows:
...
\end{align*}
\noindent 
Cramer-Von-Mises can also be used to define the expected inaccuracy, analogously to the case of KL-divergence. 

At this point, a friend of point estimates might insist, \emph{but who do we have to pay attention to all these possible values, we can just run with point estimates!} To illustrate why the move to consider values in-between without prior integration, we will  go over an example in which there are only two possible lowe-level outcomes, $\mathsf{tails}$ and $\mathsf{heads}$, *and we take the distribution's expected values as the probabilities they assign to those outcomes. On this approach, instead of calculating inaccuracy w.r.t. to all possible hypotheses about the coin's bias, we first average out, and proceed in terms of probabilities of the two outcomes only instead. That is, we use:
...
Denote these distributions as $b, c,$ and $w$ correspondingly. Now consider what happens if we think of expected inaccuracy of these distributions assuming there are only two possible true outcomes, conceptualized as either heads ($H$) or tails ($T$). Whatever our inaccuracy measure, we will have six inaccuracy scores of the form $I(\mathsf{distribution}, \mathsf{outcome})$, where $\mathsf{outcome}$ is one of two omniscient distributions that give all weight to either heads or tails. In the calculation of expected values,these will need to be multiplied by the probability of $H$ and the probability of $T$. However, none of these distributions assigns such a probability. They all assign probabilities to different values of coin bias, but not to different *outcomes* directly. Suppose we do what seems to be the natural way to go for the friends of point estimation and  obtain such probabilities by  taking expected values of the form $\mathbb{E}_{\mathsf{distribution}}(H) = \sum (x * \mathsf{distribution}(x))$, where $x$ are the values on the discretized grid  (as in our example these expectations are pretty much the same, we can simply take $\mathsf{distribution}(H)$ to be .4): 
...
Rational agents are often tasked with aggregating pieces of evidence and assessing their value relative to a hypothesis. A popular measure of the value of the evidece is the likelihood ratio.  This ratio is relative to a pair of competing hypotheses, say $H$ and its negation $\neg H$ (though the two hypotheses 
...
\noindent
The resulting ratio is a large number. Taken at face value, the two matches, combined, strongly favor the source hypothesis. This is bad news for the defendant.
...
This is the story about evidence aggregation (in the simple case) told by the precise probabilist.  But this story misses something crucial. As it happens, the match probability for hair evidence is based on 29 matches found in a sample database of size 1148, while the match probability for the dog evidence is based on finding two matches in a smaller database of size 78. The relative frequencies are about $.025$ in both cases, but the two samples differ in size. The smaller the sample, the greater the uncertainty about the match probabilities. So simply reporting the exact numbers `r round(hairMean,4)` and  `r round(dogMean,4)` makes it seem as though the evidential value of the matches is the same, but actually it is not.^[The match probabilities in the Wayne Williams case  on which our running example is based were 1/100 for the dog fur, and 29/1148 for Wayne Williams' hair. Match probabilities have been slightly but not unrealistically modified to be closer to each other in order to make a conceptual point: the same first-order probabilities, even when they sound precise, may come with different degrees of second-order uncertainty.] 

Here is another way to bring out the problem. Suppose the match probabilities associated with two matches are both set to $.02$, since they are based on the following relative frequencies: two matches occur in a dog fur database and two matches occur in a human hair database, where both databases are small, say of size 10. Suppose, as before, both the defedant's hair and defendant's dog match the traces at the crime scene. By multiplying the individual $1/.02$ likelihood ratios associated with the two matches, their evidential value against the defendant would seem overwhelming: $1/.02\times 1/.02=`r 1/(.02*.02)`$. But the match probabilities are based on frequencies resulting from very small databases, 
so the evidential value of the two matches should be close to worthless. Precise probability here exaggerates the value of the two matches. On the other hand, if the same $.02$ match probabilities were based on larger databases, the evidential value of the two matches should be correspondingly greater, but precise probabilism woudl still assign them the same value. All in all, the evidential value of the two matches, intuitively, should be quite different, depending on whether the samples are small, large, or anything in between. And yet, a precise likelihood ratio would count the two matches, combined, as having the same evidential value no matter the sample size, thereby overestimating or underestimating the value of the evidence. 

A better alternative is easily available: the evaluation of multiple items of evidence should take into account higher-order uncertainty. To see how, consider @fig-densities (upper part). It depicts the probability distribution of different match probabilities given the sample data---the actual number of matches found in the sample database. As expected, some random match probabilities are more likely than others. Recall that, by hypothesis, 29 matches were found in the sample database of human hair of size 1148, and 2 matches were found in the sample database of dog fur of size 78.  Since the sizes of the two databases are different, the distributions have different spreads: the smaller the database the greater the spread, the greater the uncertainty about the match probability. In light of this, now consider @fig-densities (lower part). It depicts the probability distribtion for the joint match probability associated with both items of match evidence, hair and fur evidence. So, whenever probability distributions for the probabilities of interest are available (and they should be available for match evidence and many forms of scientific evidence whose reliability has been studied), those distributions should be reported for assessing the value of the evidence. This approach avoids hiding actual aleatory uncertainties under the carpet. It also allows for a more balanced assessment of the evidence, whereas using precise values may exaggerate or underestimate the value of the evidence.


The precise probabilist might insist that the value of the evidence---whether in the form of one item or multiple items of evidence---is naturally captured by a precise likelihood ratio. In particular, our best assessment of the match probability should still rely on the relative frequencies of matches found in a sample database, whether large or small. If this is right, our best assessment of the match probabilities for both fur and hair evidence is about $.025$, based on the relative frequencies of 2/78 and 29/1148. After all, if we were to bet whether a dog or a human picked at random would have the matching fur or hair type, our betting odds should be $.25$ no matter the size of the database. This argument has some bite for a single item of evidence because the expected values of the match probabilities for hair and match evidence---based on the higher-order distributions in  @fig-densities (upper part)---ends up being 2/78 and 29/1148. So, the higher-order approach does not seem to reccomend a different assignment of first-order probabilities. But this line of reasoning breaks down for two or more items of evidence. 

What should our betting odds be for the proposition that a human and a dog, both picked at random, would have the matching fur and hair type in question? For the precise probabilist, the answer is straightforward: on the assumption of independence, it is enough to multiply the individual match probabilities. 
The higher-order reccomend a different route: first, aggregate distributiion for the two match pribabilities in the probability distribtion for the joint match probability associated with both items of match evidence; next, take the expected value of this probability distribution. As it turns out, this first-order probability value is not the same as the one reccomended by the precise probabilist. 
...
\textbf{Marcello (first point): Can we have a formula of how the two matches are combined in the higher-order approach? In precise probabilism, you multiply the individual LRs, in higher-order probabilism, what do we do formally? The next section then generalizes this simple cases to all complex cases with dependencies between items of evidence and hypotheses.}  
 ...
 All in all, precise and imprecise probabilims---as well as the interval approach, a variation of imprecise probabilism---do not fare well in modelling  the value of evidence in the aggregate. 
...
<!---How to assess higher order uncertainty is difficult. We provide no method for doing that. Our point, rather, is that higher-order uncertainty should not be overlooked when evaluating the value of the evidence, especially when multiple items  are pieced together. -->
