...
A scoring rules or inaccuracy score quantifies the distance (inaccuracy) between a probability distribution and the true state of the world.  A desirable property that any such score should have is strict propriety. In the precise case, let $I_w(p)$ be an inaccuracy score of a probability distribution $p$ relative to the true state of the world $w \in W$. The score $I_w(p)$ is strictly proper if, for any other probability distribution $q$ different from $p$, the following holds: 

$$\sum_{w\in W} p(w)I_w(p)< \sum_{w\in W} q(w)I_w(p).$$

\noindent 
That is, the expected inaccuracy of $p$ from the perspeective of $p$ itself shoud always be smaller than the expected inaccuracy of $p$ from the perspective of another distribution $q$. Other common requirements for an inaccuracy score are extensionality (the score should be a function of the probability distribution and the true state) as well as continuity (the score $I_w(p)$ should be a continous function around $p$). Well-known results show that the Brier score is an extensional score for precise probabilities that is both proper and continous.^[The Brier score is defined as the squared distance between the true state of the world and the probability forecast, or formally,  $(p(w)-V(w))^2$, where $p(w)$ is the probability forecast and $V(w)$ determines if a state $w\in W$ obtains ($V(w)=1$) or not ($V(w)=0$). If, for example, the true state of the world is 'rain', the forecast 'rain with .9 probability' would be more accurate than the forecast 'rain with .8 probability'. If, on the other hand, the true state were 'not rain', the latter forecast would be more accurate.]\todo{Is the formal statement of Brier score in the footnote correct? Are probabilities  defined over possible worlds or propositions that (partially) describe possible worlds?}
...
Can a similar result be established for imprecise probabilities? The answer is likely to be negative, since several hurdles exist. 
First, it is is not straightforward to formulate an intuitively plausible scoring rule for imprecise probabilities. Suppose an imprecise forecast assigns the [.8, .9] interval to the probability of rain, where the true state is 'rain'. Would the [.6, .99] interval be more accurate since its .99 upper bound is closer to the true state? Intuitively, the [.6, .99] interval should not be more accurate than [.8, .9], otherwise the trivial interval [0, 1] would always be more accurate than any other interval. To avoid this outcome, a plausible inaccuracy score for imprecise probabilities could perhaps depend on two things. First, it decreases with the Brier score computed using the side of the interval closer to the true state. Second, the inaccuracy score increaes with the size of the interval itself. This blocks the result that the [0, 1] interval is the most accurate. For more details, see @seidenfeld2012forecasting. 

Still, however the score for imprecise probabilities is defined, a second hurdle remains. The notion of expected inaccuracy is needed to establish the score's strict propriety, but this notion cannot be easily defined for imprecise probabilities. To see why, let $I_w([p_-, p_+])$ be an inaccuracy score for the interval $[p_-, p_+]$. Its expected inaccuracy from the perspective of the interval $[p_-, p_+]$ itself cannot be easily defined. After all, $I_w([p_-, p_+])$ cannot be multiplied by $[p_-, p_+](w)$, in the way in which $I_w(p)$ can be multipled by $p(w)$. One option is to evaluate the expected inaccuracy of $[p_-, p_+]$ from the perspective of $p_-$ or $p_+$.^[So the expected inaccuracy of $I_w([p_-, p_+])$ would equal $\sum_{w\in W} p_-(w)I_w([p_-, p_+])$ or $\sum_{w\in W} p_+(w)I_w([p_-, p_+])$.] But the impreciser would still face a problem. If the expected inaccuracy of the interval $[p_-, p_+]$ is evaluated from the perspective of the precise probabilities $p_-$ or $p_+$, finding a proper inaccuracy score that is also continous turns out to be impossible  [@seidenfeld2012forecasting].  If, instead, the notion of expected inaccuracy cannot be defined, the very question whether an inaccuracy score for imprecise probabilities can be proper is ill-defined to begin with. 


Despite the difficulties that plague imprecise probabilism in defining scoring rules that are proper, here we show that there is a intuitively plausible scoring rule for higher-order probabilities that is both proper and continous. Building on existing work on this topic (@HersbachDecomp2000, @Pettigrew2012Epistemic-Utili, @GneitRafter2007), the higher-order scoring rule we propose is based on well-known measures of divergence between probability distributions. In particular, we rely 
on the Kullback-Leibler (KL) divergence, which is defined as follows: <!--A key modification to this framework involves abandoning the assumption that only two possible outcomes (truth and false) exist. Once we present the positive proposal, we will illustrate how such an assumption is problematic.--> 
$$ D_{\text{KL}}(q \,||\, p) = \sum_{x} q(x) \log\left(\frac{q(x)}{p(x)}\right) $$

\noindent This is a standard information-theoretic measure of divergence of $p$ from $q$ from the perspective of $q$. For computational ease, we are using a grid approximation instead of continous distribitions, as in practice we are unable to work with infinite precision.\footnote{In the continuous case, we would need to use the so-called differential KL divergence, but our main points wouldn't change otherwise.}   To this end, $x$ will denote the finite vector of discrete states under consideration (by default, we will use equally spaced values between 0 and 1 here).  \todo{Marcello: I am not clear what x is here. What are states? Is the same x used below (outcomes)?}

 The goal now is to deploy KL divergence as a measure of inaccuracy of $p$ relative to the true state of the world $W$, denoted by $I^{KL}_w(p)$. To this end, it is enough for the probability distribution $q(x)$ to play the role of the 'true' distribution tracking the true state $w$; call it $t(x)$, 

$$ D_{\text{KL}}(t \,||\, p) = \sum_{x} t(x) \log\left(\frac{t(x)}{p(x)}\right), $$

where to the possible outcomes $x$, the ominscient distribution $t$ will either assigns 1  (full probability one, if the outcome obtain in $w$) or 0 (zero probability, if the state does not obtain in $w$). 
...
The standard response to these concerns is to invoke *sensitivity analysis*: a range of plausible values is tested. Say we are interested in the output probability that Sally is guilty. The network is updated by the known facts---the items of evidence---following standard Bayesian conditionalization.  The input probabilities in the network are then assigned a range of possible values to see how they impact the output probability of Sally's guilt. Sensitivity analysys is another variant, perhaps more rudimentary, of the interval approach we considered earlier.  \todo{add references to imprecise BNs; see footnote.} In fact, Bayesian networks for reasoning with intervals and imprecise probabilisties already exist.^[One can use uniform sampling with Bayesian networks to approximate the impreciser's commitments \textbf{Cite} \url{https://arxiv.org/abs/2302.09656}. Another approach is to rely on probabilistic programs with the restriction that the
...
We should unnderscore that, mathematically, we do not
 propose anything radically new. Concepts from the Bayesian toolkit that can model higher-order uncertainty already exist. Our suggestion is that they have been under-appreciated in formal epistemology and should be more widely used. This is not to say that there is no need for any novel technical work. For example, we still need a proper accuracy argument in defense of higher-order probabilism. Will an agent who relies on higher-order probabilities would accuracy-dominate one who relies on just first-order probabilisties? We leave this as an open question. Another concern is the lack of a clear semantics for higher-order probabilities. While a more elaborate account is beyond the scope of this paper, the answer 
