...
-->
 ...
 The central idea of higher-order probabilism is this: instead of a number between 0 and 1, rational agents assign a probability density to each proposition of interest. For example, consider the scenario in which the agent knows that the bias of the coin is either .4 or .6 but the former is three times more likely.  This scenario---which imprecise probabilism cannot model---can be easily modeled within high-order probabilism by assigning different probabilities to the two biases, as illustrated in @fig-evidenceResponse. So, an agent's state of uncertainty (or credal stance) towards a proposition $x$ will not be represented by a single probability value $p(x)$ between 0 and 1, but by a probability density $f_x(p(x))$, where the first-order probability of $x$ is the parameter in question and is treated as a random variable.^[For computational ease, we will use a higher-order density that is discretized and ranges over 1000 first-order probabilities.] More generally, let $\mathbf{X}$ be the set of first-order propositions an agent is interested in (more on this restriction soon).  So, for each proposition $x\in\mathbf{X}$, the rational agent will assign a probability density $f_x(p(x))$, and not simply a probability value $p(x)$. This mapping from a set of propositions to densities models the agent's overall state of uncertainty.  

 Note that the set $\mathbf{X}$ is restricted to first-order propositions. There is an intuitive distinction between
propositions more directly about events in the world, such as \emph{the coin will land heads on the next toss} and more abstract
constructs such as the probabilities of these propositions. So we call propositions about the former events first-order, and propositions about the latter events second-order. Now, instead of going higher-order, the set of $\mathbf{X}$ can be extended to include second-order propositions, denoted $\mathbf{X}^+$. Then, an agent's state of uncertainty will be modeled by a single, precise probability measure defined over all members of the extended set $\mathbf{X}^+$.
This proposal can easily model the scenario of uneven coins by assigning different probabilities to the (second-order) propositions 
about the two possible biases of the coin, again as illustrated in @fig-evidenceResponse.

Higher-order probabilist and this variation of precise probabilism in which the event space is suitably extended are quite similar. But a few differences are worth pointing out. Consider the (first-order) proposition \emph{the coin will land heads on the next toss} (in short, \emph{heads}) and the (second-order) \emph{the coin has a bias of .5}. We expected them to be related in precise ways. For example, it is inconsistent to assign a probability of one (or an extremely high probability) to both \emph{heads} and the coin having a fair bias, or to assign an extremely high probability to \emph{heads}. Clearly, the two propositions are not probabilistically independent and thei dependence is conceptual or formal, not an empirical question. The simplest way to model this formal probabilistic dependence is to posit that the expectation of the density $f_{heads}(p(heads))$ equals $p(heads)$.
...
An agent's uncertainty about a proposition could---perhaps, should---sometimes be represented by a single probability value. Higher-order probabilism does not prohibit that. For example, there may well be cases in which an agent's uncertainty is aptly represented by the expectation.^[The expectation is usually defined as $\int_{0}^{1} x f(x) \, dx$. In the context of our approach here, $x$ is the first-order probability of a given proposition and $f$ is the density representing the agent's uncertainty about $x$.]
...
Representing the agent's credal state with the expectation $.75 \times .4 + .25 \times .6 = .45$ would fail to capture the agent's different epistemic attitudes towards the two biases. The agent believes the two biases have different probabilities, and is also certain the bias is *not* .45. 
...
The precise probabilist will note that, by suitable extending the set of propositions of interest to include also second-order propositions---such as propositions about the probability of first-order event---

This higher-order approach at the technical level is by no means novel. Bayesian probabilistic programming languages embrace the well-known idea that parameters can be stacked and depend on each other [@Bingham2021PPwithoutTears]. But, while the technical machinery has been around for a while, it has not been deployed by philosophers to model a rational agent's uncertainty or credal state. 
  
We should distinguish our version of higher-order (precise) probabilism from a couple of other variants. First, consider what we might call higher-order (imprecise) probabilism. On this account, an agent's state of uncertainty is modeled by (a) a set of probability measures defined over a fixed algebra of first-order propositions (just like imprecise probabilism does), and in addition, (b) a probability distribution over those first-order probability measures. Our higher-order (precise) probabilism instead models an agent state of uncertainty 


(a) a single, precise probability measure defined over a fixed algebra of first-order propositions (just like precise probabilism does), and addition, (b) by a probability density 
