...
53 Issue: 5, 1984, pp. 10-19.} What are the fact-finders to make of this
evidence?

The standard story among legal probabilists goes something like this. To
evaluate the strength of the two items of match evidence, an expert
provides their random match probabilities. Suppose the expert testifies
that the probability of a random person's hair matching the reference
sample is about 0.0253, and the probability of a random dog's hair
matching the reference sample happens to be about the same,
...
Then, to evaluate the overall impact of the evidence on the source
...
evidence leaves out something crucial. You reflect on what you have been
...
uncertainty that may affect these numbers? The expert agrees, and tells
you that in fact the random match probability for the hair evidence is
based on 29 matches found in a database of size 1148, while the random
match probability for the dog evidence is based on finding two matches
in a reference database of size 78.
...
You wonder whether the new calculation you carried out is the correct
one. Are the two items of match evidence strong incriminating evidence
(as you initially thought) or somewhat weaker (as the new calculation
suggests)? For one thing, simply using precise random match
...
bounds might be too favorable toward the defendant. There must be a
middle way that avoids both overestimating and underestimating the value
of the evidence.
...
probability of a single event, it is not straightforward to correctly
calculate a new interval once a second event is included. We need
additional information, specifically, the distributions that were used
to calculate the intervals for the probabilities of the individual
events. So, in the case of the two items of match evidence, if you knew
what distributions the expert used (it should have been beta
distributions in this context), you could work your way back and
calculate the 99\% credible interval for both items of evidence. As it
turns out, the credible interval for the probability
\(\mathsf{P}(\s{dog}\wedge \s{hair} \vert \neg \s{source})\) is
...
Unfortunately, the interval approach---even the corrected version just
...
exaggerate or underestimated the value of the evidence.
...
take for assessing the value of evidence presented at trial. The first
approach uses precise probabilities; the second uses intervals; the
third uses distributions over probabilities. By relying on an example
featuring two items of match evidence, we suggested that the third
approach is preferable. This section strengthened this claim by
providing principled, philosophical reasons for this preference.
...
credal stance towards a hypothesis \(H\) is to be represented by means
of a \emph{set of probability measures}, typically called a representor
...
\noindent The hope is at least that if we start with a range of
probabilities that is not extremely wide, point-wise learning will
behave appropriately. For instance, if we start with a prior probability
of \emph{heads} equal to .4 or .6, then those measure should be updated
to something closer to \(.5\) once we learn that a given coin has
already been tossed ten times with the observed number of heads equal 5
(call this evidence \(E\)). This would mean that if the initial range of
...
problems plague imprecise probabilism. Three more problems are
...
credence function, such as the Brier score, are available. Scoring rules
measure the distance between one's probability assignment and the actual
value. A key feature of scoring rules is that they should be
\emph{proper}: any agent will score her own credence function to be more
accurate than every other credence function. After all, if an agent
...
accuracy-oriented arguments for precise probabilism. These arguments
show (roughly) that, if your precise credence follows the axioms of
probability theory, no other credence is going to be more accurate than
yours whatever the facts are. Turning to imprecise probabilism, however,
there are impossibility results to the effect that no proper scoring
rules are available for representor sets. So, as many have noted, the
...
stance, as for any imprecise stance there will be a precise one with the
same accuracy.

The third problem with imprecise probabilism has to do how the evidence
may be incompatible a probability measures in the representor set.
Recall that that the probability measures allowed in a representor set
are only thoise compatible with the agent's evidence. How, exactly, does
the evidence exclude probability measures? This is not a mathematical
question: mathematically (S. Bradley, 2012), evidential constraints are
easy to model. They can take the form, for example, of
...
result in such constraints. Most of the examples in the literature start
with the assumption that the agent is told by a believable source that
the chances are such-and-such, or that the experimental set-up is such
that the agent knows that such and such structural constraint is
satisfied. But, besides ideal circumstances, it is unclear how an agent
could come to accept such structural constraints upon
observation.\footnote{Even if you were lucky enough to run into an
  expert that you completely trust that provides you with a constraint
  like this, how exactly did the expert come to learn the constraint?
  The chain of testimonial evidence has to end somewhere. Mere
  consistency constraint wouldn't get the agent very far in the game of
  excluding probability measures, as way too many probability measures
  are strictly speaking still consistent with the observations for
  evidence to result in epistemic progress.}
...
Within the higher-order approach, the scenarios we discussed---some of
which imprecise probabilism had hard time distinguishing---can be easily
modeled in the manner illustrated in Figure \ref{fig:evidenceResponse}.
...
\(\mathsf{P}(X)\) as a random variable. For consider again the scenario
in which the agent knows that the bias of the coin is either .4 or .6
but the former is three times more likely. The agent might refuse to
summarize their credal state by saying that
\(\mathsf{P}(X) = .75 \times .4 + .25 \times .6 = .45\). More generally,
on this perspective, the agent might deny that
...
complexity of the agent's credal state.\footnote{Whether this
  expectation should be used in betting behavior is a separate problem,
  here we focus on epistemic issues.} This approach lines up with common
...
representing the uncertainty involved in a given study.
...
A notable advantage of higher-order probabilism over imprecise
probabilism is that the problem of belief inertia does not arise.
Consider a situation in which you have no idea about the bias of a coin.
So you start with a uniform density over \([0,1]\) as your prior. By
using binomial probabilities as likelihoods, observing any non-zero
number of heads will exclude 0 and observing any non-zero number of
tails will exclude 1 from the basis of the posterior. The posterior
distribution will become more centered around the parameter estimate as
the observations come in. Figure \ref{fig:intertia2} shows---starting
with a uniform prior distribution--- how the posterior distribution
changes after successive observations of heads, heads again, and then
tails.\footnote{More generally, learning about frequencies, assuming
  independence and constant probability for all the observations, is
  modeled the Bayes way. You start with some prior density \(p\) over
  the parameter values. If you start with complete lack of information,
