...
\begin{align}
\tag{Jpyce} w(X,E) & = sum_x \vert c(ch(X) = x  \vert E) \times (x - c(X\vert E))^2 - c(ch(X) = x) \times (x - c(X))^2\vert
\end{align}

\noindent This looks a bit complicated, so let us take this slow. Suppose you only consider three chance hypotheses, that the coin bias is one of $.4, .5,$ and $.6$, that is, the hypotheses are $ch(X) = .4, ch(X)=.5,$ and $ch(X)=.6$. To each of this hypotheses (that is, to each $x\in \{.4, .5, .6\}$) you attach a prior credence $c(ch(X) = x)$. Say you start with equal priors, that is for all $x\in \{.4, .5, .6\}$ you have $c(ch(X) =x) = \nicefrac{1}{3}$. Then, your expected value of $X$, which Joyce to be your credence in $X$ directly is $\sum_x c(Ch(X)=x)x$, which is .5. 
...
\end{align*}
\noindent so we need to calculate the likelihoods, $c(E \vert ch(X) =x)$. We assume you are probabilistically coherent and know the experimental setup, so you 



First you need to calculate the likelihoods for the chance, that is the values of $ch(E\vert x)$. These follow the binomial distribution and  are approximately $0.042, 0.117$, and  $0.214$, respectively. Next, we need the denominator to plug into an application of the Bayes theorem, that is, we need $ch(E)  = \sum_x ch(E \vert x) ch(x)$. 





where $B=x$ is a coin bias hypothesis, $D$ is the new data, $H$ is the hypothesis that the next coin toss will come up heads, $f$ is the density that corresponds to $p$, $f_D$ is the density obtained from $f$ by conditionalizing on $D$. Now imagine $p$ is concentrated on a narrow range of hypotheses. Then $f(B=x)$ is small/large for large/small $(x - p(X))^2$, so the minuend will  tend to be small, and similarly for the subtrahend, so the whole number will be close to zero for a wide range of data $D$.
...
Yet, he still insists that in such a case RA's stance towards a proposition should be represented as the expected value of this density---this is an assumption that I will drop.
