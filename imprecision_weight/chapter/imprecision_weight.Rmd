...
straightforward,^[One might expect that entropy in the continous case  could be made by binning and
...
abusePlot_3_2 <-  ggplot()+theme_tufte(base_size=7)+xlab("parameter values")+
  ...
  labs(title = "Prior beta(2,4) and posteriors in the abuse case",
...
abusePlot_norm_3_05 <- ggplot()+theme_tufte(base_size=7)+xlab("parameter values")+
  ylab("density")+theme(plot.title.position = "plot")+
  labs(title = "",
  ...
  annotate(geom = "label", 
           label = paste("prior median: ", round(median(priorSample),2), ", 89% HPDI = (", round(priorHPDI[1],2), 
                         ", ", round(priorHPDI[2],2), ")", sep = ""),
           x = median(priorSample), y = 1.4, size = 2.5)+
  geom_density(aes(x = pae3), color = "skyblue", lty = 1 )+
  annotate(geom = "label", 
           label = paste("posterior median: ", round(median(pae3),2), ", 89% HPDI = (", round(pae3HPDI[1],2), 
                         ", ", round(pae3HPDI[2],2), ")", sep = ""),
           x = median(pae3), y = .8, size = 2.5)
```
...
\begin{figure}[H]
```{r fig:abuse4,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
grid.arrange(abusePlot_3_2,abusePlot_3_05,abusePlot_norm_3_05 , ncol = 1)
```
