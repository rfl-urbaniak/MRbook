...
trial decision-making. Given at least one illustration each. 
...
Despite this limitation, this conception allows us to ask questions about *accuracy*. Are decisions based on B+ as opposed to B, in the long run, always more accurate (fewer false positive and fewer false negatives)? Answering this question is by no means obvious and already requires setting a rather complex set of formal definitions. A computer simulation might be a good way to address this question.
...
How do we draw that list of *reasonably complete* evidence? This list is drawn in a number of different ways: 
      ...
      (b) using a case-specific *shared narrative* (=the following facts are well-established, and given those facts, we would expect to see these items of evidence); 
      (c) using a *partisan narrative* (=the prosecutor offers this narrative from which we would expect to see these items of evidence); or 
...
Need to restrict the resilience test to a *reasonable set* of possible further items of evidence, otherwise no body of evidence would could ever count as resilient.  
