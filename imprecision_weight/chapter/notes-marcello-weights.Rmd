...
trial decision-making broadly construed (not just standards of proof, but also questions of admissibility, retrial, appellate decision, etc.) Give at least one illustration each. 
...
## Possible plan for chapter -- more detailed 
...
\todo{R:Hm, this is an interesting argument, will need to work it out; at this point, I'm not convinced as it is not formulated in terms of a probability measure; will think more about this.}
...
\todo{R: Well, the proportional change does not, wDelta might, will have to think about it.}
...
\todo{R: in my calculations now I look at expected and at the range as well.}
...
It is not clear whether Nance has any account of expected weight. Perhpas this account is not even needed. Suppose we are considering taking a DNA test. The test could be positive or negative. But, presumably, the weight of the rest (for either result) should be the same since we are taking the absolute of the log of the LR. The same applies to a potential eyewitness testimony. And so on. The key here seems to be that weight is unidirectional and thus it does not mater what the actual value of the evidence is. Whether the test is positive or negative, it will have the same weight. This allows to bypass the problem of expected weight of evidence.\todo{R: why the same, you can have different conditional probabilities involved?}
...
\todo{See my discussion of the absolute value}
...
\todo{R: Either way, we need to discuss this approach in the chapter, not sure at which point.}
...
\todo{Do we discuss this notion in this chapter, or do we postpone this to another one, in which we also discuss the stability approach?}
...
\todo{R: A somewhat longer discussion of Joyce is now in the chapter draft.}
...
\todo{ok, good, I side with Sjers, but a discussion of this is a good seagwey into moving from point estimates to HOP in the context of propositions; need to read the papers; let's talk about how much of this to include.}
...
\todo{R: Generally, RMP might  be misleading as it only reports aleatory uncertainty about one particular issue, ignoring aleatory uncertainty about other possibly relevant issues, and epistemic uncertainties. We can refer back to the stuff we said about false positives etc. in the likelihood ratio chapter.}
...
\todo{R: Ok, right, but there should be a quantitative approach to establish this, right? Like, fora regular person the probability that there is a family member who'd match and could be the culprit is veeeery low, so the expected impact of this option can be ignored (can it?), but if there is a sensible story increasing the probability of this scenario...}
...
\todo{R: right, this is a real-life argument for the Bayesian approach instead of LR: a serious case in which to report the overall strength of evidence, priors on options are needed; another strategy: report two LRs and leave the rest to the court, analogous to multiple RMPs for difference races of the assailant?}
...
\todo{R: idea: model this with a BN where the HOP uncertainty about races is very high, while HOP uncertainty about particular LRs is very low, see what happens!}
...
\todo{Talk about "Uncertain Judgments"}
...
\todo{R: Seems like buck passing in evidence evaluation, no? Hey, just tell the jury and it's up to them what to do about this.}
...
\todo{If you use uderscore in regular text, it might not knit (it did not for me), as it is used in LaTeX for lower subscript. Use url}
\url{judicature.duke.edu/wp-content/uploads/2020/10/may2017_rule37.pdf}


\todo{Interesting, isn't that instead of two rules we should have somewhat different uncertainties about unintended deletion, which is much easier for electronic evidence?? This would be more unifying, no? Also, realistically this uncertainty should be HOP}
...
\todo{R: read this paper, we need to discuss it, but the problems start with how he defines weight (if I remember well, certainty is just distance from the extreme values, right?)}
...
\todo{But he isn't even right about cancelling out, this really depends on what the priors are, should I look at his analytic proof and discuss it in the chapter?}
