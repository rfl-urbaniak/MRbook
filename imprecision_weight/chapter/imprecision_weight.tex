...
comceptualizes the weight of evidence as an increase of concentration of
...
Suppose you have three that the coin bias is \(x\). \(H\) is the
hypothesis that the next coin toss will come up heads. \(D\) is the
observations that you have made.

where \(B=x\) is a coin bias hypothesis, \(D\) is the new data, \(H\) is
the hypothesis that the next coin toss will come up heads, \(f\) is the
density that corresponds to \(p\), \(f_D\) is the density obtained from
\(f\) by conditionalizing on \(D\). Now imagine \(p\) is concentrated on
a narrow range of hypotheses. Then \(f(B=x)\) is small/large for
large/small \((x - p(X))^2\), so the minuend will tend to be small, and
similarly for the subtrahend, so the whole number will be close to zero
