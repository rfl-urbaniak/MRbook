...
The bias of the coin is either 4. or .6 v.  bias  .4 is three times more likely than bias .6

- Imprecise probabilism cannot distinguish between these scenarios
...
Predictive distributions based on point estimates are systematically less accurate than posterior predictive distributions that incorporate parameter uncertainty 

Accuracy is quantified by Kullback-Leibler divergence which measures the closeness of a distribution   (say, the true distribution) to another (say, the distribution whose accuracy we want to quantify)


## What difference does this make for prediction? (example)
...
##  Distances from the actual distribution
...
## (4) Higher-order Bayesian networks (cont'd)
...
- **Weak Increase**: In Bernoulli trials, weight does not decrease as sample size increases, keeping frequency fixed
...
## Weak Increase holds and Monotonicity fails
...
1. Resilience increases with weight, completeness improves weight.

2. Completeness and resilience are harder to track, as they are relative to lists of potential items of evidence or multiple  probabilities conditional on a range of potential items of evidence we do not have
...
```{r goodWEights,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "90%", warning = FALSE, message = FALSE}
six <- seq(0,10, by = 1)
