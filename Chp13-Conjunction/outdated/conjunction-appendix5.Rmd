...
plotLRindBelow2 <- conjunctionTable %>% filter(LRAs < 1 & LRBs < 1) %>% ggplot( aes(x = LRdifsMax))+geom_histogram(aes( y = ..density..), bins = 40)+
  ...
  labs(subtitle = expression(paste("Assuming ",  LR[A], ", ",   LR[B] < 1)))+xlim(c(-1.5,0))
...
Once the individual likelihood ratios are above 1, the joint likelihood ratio can be lower than the maximum, but is not lower than the minimum of the individual likelihood ratios (Figure \ref{fig:LRabovePlot}).
