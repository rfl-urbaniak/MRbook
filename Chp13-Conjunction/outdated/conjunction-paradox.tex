...
enjoyed a great deal of scholarly attention every since (Ronald J.
Allen, 1986; Ronald J. Allen \& Stein, 2013; R. Allen \& Pardo, 2019;
Haack, 2014; Schwartz \& Sober, 2017; Stein, 2005). This difficulty
...
defendant's action was premeditated. Cohen (1977) argues that common law
systems subscribe to what he calls a \textit{conjunction principle},
that is, if two claims---call them \(A\) and \(B\)---are established
according to the governing standard of proof, so is their conjunction
\(A\wedge B\) (and vice versa). If the conjunction principle holds, the
...
\begin{center}\includegraphics[width=0.6\linewidth]{conjunction-paradox_files/figure-latex/fig:hEDAG-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{conjunction-paradox_files/figure-latex/fig:aADAG-1} \end{center}
...
\begin{center}\includegraphics[width=0.6\linewidth]{conjunction-paradox_files/figure-latex/fig:bBDAG-1} \end{center}
...
probability of \(B\) is above a given threshold, the probability of the
conjunction \(A\wedge B\) usually is not. This can happen whether or not
the probability of \(A\) and \(B\) are conditional on the supporting
evidence and whether or not \(A\) and \(B\) are probabilistically
independent. These failures of aggregation occur assuming the standard
of proof is understood as a posterior probability threshold. Thus,
...
justify a finding of liability. Instead of a posterior probability
threshold, the standard of proof can be modeled using a probabilistic
measure of evidential strength. Does this alternative way of modeling
proof standards succeed in solving the conjunction paradox work? To some
extent, but not completely. To understand why is the task of this
section.
...
factor and the likelihood ratio. We discussed this topic in earlier
chapters (\textbf{REFER TO EARLIER CHAPTERS}). As will become clear
later, under plausible assumptions, these measures of evidential
strength validate one direction of the conjunction principle:
...
\(\nicefrac{\pr{b \vert B}}{\pr{b}=s_B}\) (abbreviated \(BF_B\)) are
greater than one. Does the combined evidence \(a \wedge b\) provide at
least as much support in favor of the joint claim \(A \wedge B\) as the
individual support by \(a\) and \(b\) in favor of \(A\) and \(B\)
considered separately? The combined support here is measured by the
combined Bayes factor
\(\nicefrac{\pr{a \wedge b\vert A\wedge B}}{\pr{a \wedge b}}\)
(abbreviated \(BF_{AB}\)). The latter, under suitable independence
assumptions, equals the product of the individual Bayes factors
\(BF_{A}\) and \(BF_{B}\). That is:\footnote{Here is the argument:
  \begin{align*}
...
\noindent Essentially, we are assuming that hypotheses \(A\) and \(B\)
are independent and that items of evidence \(a\) and \(b\) are
independent. As noted earlier, these assumptions are plausible insofar
as the Bayesian network in Figure \ref{fig:conjunctionDAG} is a
plausible representation of the situation at hand. Thus, the combined
...
\ref{fig:conjunctionDAG2}? The combined Bayesian factor \(BF_{AB}\) is
still greater than both the individual Bayesian factor \(BF_{A}\) and
\(BF_{B}\) in a number of circumstances. First, the following
...
\hypertarget{move-to-appendix-materials}{%
\paragraph{MOVE TO Appendix
materials}\label{move-to-appendix-materials}}
...
threshold is that its value can be determined in a decision-theoretic
...
Cohen, J. L. (1988). The difficulty about conjunction in forensic proof.
