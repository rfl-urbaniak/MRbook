...
To further complicate things, there are differences between countries
and legal traditions. The tripartite distinction of proof
standards---beyond a reasonable doubt; preponderance; clear and
convincing evidence---is common in Anglo-american jurisprudence. It is
not universal, however. Different countries may use different standards.
France, for example, uses the standard of `intimate conviction' for both
civil and criminal proceedings. Judges deciding cases `must search their
conscience in good faith and silently and thoughtfully ask themselves
what impression the evidence given against the accused and the defence's
arguments have made upon them' (French Code of Criminal Procedure,
art.~353). German law is similar. Germany's Code of Civil Procedure,
Sec.~286, states that `it is for the court to decide, based on its
personal conviction, whether a factual claim is indeed true or not.'
...
But it would be misleading to say that all legal practitioners resist a
probabilistic inteoretation of standard of proof. Some actually find
such an intepretation inevitable. For example, here is Justice Harlan of
...
The resistance of legal practioners toward quantifying standards of
proof probabiistically has many causes. One key reason for this
resistance is certainly the conviction that a probabilistic
intepretation of proof standards is unrealistic. Its implementation
might face unsurmountable practical challenges. How are
probabilities---say the probability of someone's guilt---going to be
quantified probabilistically? How will the triers of facts apply
probabilistic thresholds? Should the application of the threshold be
automatic---that is, if the evidence is above the requisite threshold,
find against the defedant (say, convict in a criminal trial) and
otherwise find for the defedant (say, acquit)? The challenge, in
general, is how probabilistic thresholds can be operationalized as part
of trial decisions. This is by no means clear. Judges and jurors do not
seem to weigh evidence in a probabilistic manner. They do not use
probability thresholds as the guiding standards for their decisions.

To make the challange more precise, the probabilistic interpretion of
proof standards can be broken down into two separate claims, what we
...
\noindent Those worried about practicalities might reason thusly. If
...
One way to answer these practical challanges is to bite the bullet.
Legal probabilists can admit that probabilistic thresholds constitute a
...
this way? Can we give a principled justification for the difference? A
...
in civil trials are mostly about imposing monetary compensation on one
of the two litigants. This argument can be made precise by pairing
probability thresholds with expected utility theory, a well-establish
paradigm of rational decision-making used in psychology and economic
theory.

To see how this works, note that trial decisions can be factually
erroneous in two ways. The decision could be a false positive---i.e.~a
decision to hold the defendant liable (to convict, in a criminal case)
even though the defedant committed no wrong (or committed no crime). The
decision could also be a false negative---i.e.~a decision not to hold
the defendant liable (or to acquit, in a criminal case) even though the
defendant did commit the wrong (or committed the crime). Let
...
evidence that establishes the guilt of the defendant with at least 90\%
...
lower the requiste threshold. So, if the cost ratio is 99, the threshold
would be as high as 99\%, but if the cost ratio is 1, the threshold
would only be 50\%.
...
non-liability probability on the basis of the evidence presented at
trial. The rule of decision would be as follows:
...
threshold for liability judgments equals 50\%, which is the common
probabilistic intepretation of the preponderance standard in civil
cases. More specifically, \(\pr(L | E)\) should be at least 50\% for a
defedant to be found liable.
...
in the different costs ratio, for \(\frac{cost(FP)}{cost(PN)}\) is
typically lower than \(\frac{cost(CI)}{cost(AG)}\), or in other words, a
false positive in a criminal trial (a mistaken conviction) is considered
a graver error than a false positive in a civil trial (mistaken
attribution of civil liability). This difference in the costs ratio can
also be given a retributivist justification. For suppose the moral wrong
that results from the mistaken conviction of an innocent person is
regarded as more egregious than the moral wrong that results from the
mistaken acquittal of a guilty person. This would require the cost ratio
This moral asymmetry of the two wrongs can be captured by an
appropriately high costs ratio \(\frac{cost(FP)}{cost(PN)}\), where the
costs are the moral wrongs being compared (Walen, 2015). The same moral
asymmtry would not seem to exist in the case of civil trials.
...
theretucal level---align with the probabilistic analysis of standards of
prof outlined here. For example, the United State Supreme Court in
Speiser v.~Randall (1958), writes:
...
is even moe explicit about differences in the costs ratio between civil
and criminal cases:
...
Claims about the moral asymmetry or costs ratios can of of course be
contested. Some have argued, for example, that the standard of proof in
criminal case shoudl in fact be much lower than 90\%, and they have done
so by offerring a different assessment of the cost ratio (CITE LAUDAN).
We will not examine this debate here. Rather, the point for now is that
probabilistic thresholds, when paired with expected utility theory,
provide an analytical framework to justify and meanglfully debate the
different degrees of stringency necessary for decision criteria---proof
standards---in civil or criminal trials. In later chapters, we will
examine more in detail these ana;lytical framework and how it help
theorize about the values that should informa trial decisions, such as
minimization of expected costs, truth and accuracy, the risk of of error
and its fair allocation. (REFER HERE TO THESE LATER CHAPTERS)

\hypertarget{theoretical-challenges}{%
\section{Theoretical challenges}\label{theoretical-challenges}}

Even if they set aside practical challenges about and focus on
probabilistic thresholds as an analytical dn theorethical tool, legal
probabilist are not out of the woods yet. Another set of problems for a
probabilistic intepretation of proof standards is analytical and
theoretical. Even if the practical problems can be overcome, theoretical
difficulties remain. We will focus on two in particular: naked
statistical evidence and the difficulty with conjunction.
