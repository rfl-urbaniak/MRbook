...
different probabilistic approaches, formulate 
the most common challanges to these approaches, 
and offer a numbere of responses from the perspective of legal probabilism.
The legal and philosophical 
literature have focused on the theoretical and analytical difficilties. We will 
do the same in this chapter. We will focus on two key theoretical difficulties that have galvanized the philosophical and legal literature in evidence law in the last thirty years: the problem of naked statistical evidence and the conjunction paradox. These problems have benn formulated  
 quite some time ago by @Cohen1977The-probable-an.
 One reason to choose these two in particular 
 is that it would be desirable if we could handle basic 
 conceptual difficulties  before we move to more complex issues or even questions 
...
For vividness, suppose the charge is that 
the defedant caused the victim's death with premeditation.
As a trier of fact, the question you are confronted with is, 
does the evidence presented 
at trial, all things considered, prove guilt beyond a reasonable doubt?
...
probability of guilt does not reach the threshold,  the defendant should be 
acquitted.  This intepretation can be spelled out more formally using 
conditional probabilities. That is, a body of evidence $E$ establishes 
...
This interpretation is, in some respects, plausible.
...
people use a 75\% or 95\% threshold, but does not question 
...
[@buchak2014belief] notes that an attribution of criminal culpability is an ascription of blame and such an ascription should require a full belief in someone's guilt, not just a probabilistic belief, however strong.
...
Besides resistance from legal practitioners, 
the probabilistic interpretation of proof standards
faces practical challanges.\footnote{With a few exceptions, most of the arguments about legal probabilism in the early stage of the debate were concerned mostly with practicality. See   [@ball1960moment; @kaplan1968decision; @cullison1969probability; @simon1970quantifying;  @tribe1971trial; @tribe1970further; @lempert1977modeling @kaye1979paradox; @tillers1988probability].} How are probabilities going to be estimated? How will triers of facts apply probabilistic standards? Should the application of the thresholds be mechanic -- that is, if the evidence is above the requisite threshold, find against the defedant (say, convict in a criminal trial) and if not,  find for the defedant (say, acquit)? More generally, the challenge is how 
...
take decision instead of human beings?
 ...
 have no easy answer. To avoid setting the bar too high, let's set these worries aside. 
 We will not be concerned with the lack of real data to support certain probability assessment. 
 We will not be concerned with people being bad at reasoning about probabilities. Etc. We will not be concerned with practical issues that would arise if one would like to deploy a probabilistic model directly. We will grant that, at least for now, successful deployments of  this type  are not viable. For the time being, 
 probabilistic thresholds 
 are perhaps best understood as offerring an 
 analytical model of how trial decisions are made. The fact that this model cannot be easily operationalized does not mean it is pointelss. There are multiple ways in which such a model, even if unfit for direct deployment in trial proceedings, could be useful and offer insight into trial decision-making. 
...
of proof standards. Intuitvely, standards of proof are arranged from the least demanding (preponderance of the evidece) to the most demanding (proof beyond a reasonable doubt). But why think this way?
Exected utility theory explains why the choice of a threshold -- say 95\% in criminal cases and 50\% in civil cases -- is well-justified [@kaye1986we]. Say the probability of guilt in a criminal case (or the probability of civil liability in a civil case) is $p$. Trial decisions could be factually 
mistaken in two ways. The decision could be a false positive, a decision to find agaiinst the defedant even though the defedan committed no wrong (or no crime in a criminal case). 
The decision could be a false negative, a decision to finds in favor of the defedant (xcuit in a crimina case) even thioiugh the defedant did committ the wrong or crime. Say teh cost of a false positive is
$c_{fp}$ and the costs of a falsev negative is $c_{fn}$. Expected utiloty theory says that one should take the action that maxmizes expected utility or minimizes expected costs. So the action `find against the defedant' (`convict' in a criminal case) is justified if and only if the expected costs of a false positive is less than 
the expected cost of a false negative. 


The disutility of finding for the defendant $d_g$, and the disutility of convicting an innocent person is $d_i$. From the perspective of minimalization of expected disutility, we would like to convict, or find for the plaintiff, just in case the expected disutility of mistaken acquittal is greater than the expected disutility of incorrect conviction:
\begin{align*}
p d_g &> (1-p)  d_i
\end{align*}
