...
To further complicate things, there are differences 
between countries and legal traditions. The tripartite distinction of proof standards---beyond a reasonable doubt; preponderance; clear and convincing evidence---is common in Anglo-american jurisprudence. It is not universal, however. Different countries may use different standards. 
...
The resistance of legal practioners  toward quantifying standards of proof 
probabiistically has many causes. One key reason for this resistance is certainly 
the conviction that a probabilistic intepretation of 
proof standards is unrealistic. Its implementation might face unsurmountable practical challenges. How are probabilities---say the probability of someone's guilt---going 
...
This is by no means clear. Judges and jurors do not  seem to
weigh evidence in a probabilistic manner. They do not use 
probability thresholds as the guiding standards for their decisions. 

To make the challange more precise, the probabilistic interpretion of proof standards can be broken down into
...
\noindent
Those worried about practicalities might reason thusly.
...
One way to answer these practical challanges 
...
way to respond to the practical challenges. 
...
justification for the difference? A common argument is that more is at stake in a criminal trial than in a civil trial. A mistaken conviction will injustly deprive the defedant of basic liberties or even life. Instaed, a mistaken decision in a civil trial would not encroach upon someone's basic liberties since decisions in civil trials are mostly about imposing monetary compensation on one of the two litigants. This argument can be made  precise by pairing probability thresholds with  expected utility theory, a well-establish paradigm of rational decision-making used in psychology and economic theory. 

To see how this works, note that trial decisions can be factually 
erroneous in two ways. The decision could be a false positive---i.e.\ a decision to hold the defendant liable (to convict, in a criminal case) even though the defedant committed no wrong (or committed no crime). The decision could also be a false negative---i.e.\ a decision not to hold the defendant liable (or to acquit, in a criminal case) even though the defendant did commit the wrong (or committed the crime). Let $cost(CI)$ and $cost(AG)$ 
...
\[ \text{convict \it only if }  		 \frac{cost(CI)}{cost(AG)} < \frac{\pr(G | E)}{\pr(I | E )}.\footnote{This follows from $\pr(I | E ) \cdot cost(CI) <  \pr(G | E) \cdot cost(AG)$; see [@Kaplan1968Decision]. This model assumes, simplistically, that correct decisions do not bring any positive utility. More complex models 
are also possible, but the basic idea is the same; see [@Dekay1996].} \]

For the inequality to hold, the posterior ratio 
$\frac{\pr(G | E)}{\pr(I | E )}$ should exceed the cost ratio $\frac{cost(CI)}{cost(AG)}$. 
So long as the costs can be quantified,  
the probability threshold can be determined.
...
The corresponding probability threshold will be 90\%. On this reading, in order to meet the standard of proof beyond a reasonable doubt, the prosecution should provide evidence that establishes the guilt of the defendant 
with at least 90\% probability, or in formulas, $\pr(G | E) > 90\%$. The higher the cost ratio, the higher the requisite threshold. The lower the cost ratio, the lower the requiste threshold. So, if the cost ratio is 99, the threshold would be as high as 99\%, but if the cost ratio is 1, the threshold would only be 50\%.
...
a factually guilty defendant in a criminal case). 
Let $\Pr(L | E)$ and $\Pr( NL | E)$ be the liability probability and the 
non-liability probability on the basis of the evidence presented at trial. 
The rule of decision would be as follows:


\[ \text{find the defendant civilly liable provided}  		 \frac{cost(FP)}{cost(PN)} < \frac{\pr(L | E)}{\pr(NL | E )}.\footnote{This follows from $\pr( NL | E ) \cdot cost(FP) <  \pr(L | E) \cdot cost(FN)$}. \]

If the cost ratio  $\frac{cost(FP)}{cost(PN)}$ is set to 1, 
the threshold for liability judgments equals 50\%, which is the common probabilistic intepretation of the preponderance standard in civil cases. More specifically, $\pr(L | E)$ should 
be at least 50\% for a defedant to be found liable. 

DISCUSS HARLAN DECISION IN RE WHINSHIP ABOUT RELATIVE COSTS.

The difference between proof standards in civil and criminal cases lies 
in the different costs ratio, for $\frac{cost(FP)}{cost(PN)}$ is typically 
lower than $\frac{cost(CI)}{cost(AG)}$, or in other words, a false positive in a criminal trial (a mistaken conviction) is considered a  graver error than a false positive in a civil trial (mistaken attribution of civil liability). This difference in the costs ratio can also be given a 
retributivist justification. For suppose the moral wrong that results from 
 the mistaken conviction of an innocent person is regarded as more 
egregious than the moral wrong that results from the mistaken 
 acquittal of a guilty person. This would require the cost ratio
 This moral asymmetry of the two wrongs can be captured 
by an appropriately high costs ratio $\frac{cost(FP)}{cost(PN)}$, where 
the costs are the moral wrongs being compared [@walen2015]. The same moral asymmtry 
would not seem to exist in the case of civil trials. 

While courts are often resistant in allowing a numerical 
intepretation of proof standards, they sometimes make remarks that---at least, at the theretucal level---align   with the probabilistic analysis of standards of prof outlined here. 
For example, the United State Supreme Court 
in Speiser v.\ Randall (1958), writes:
...
Justice Harlan's concurring opinion In re Winship, 397 U.S. 358 (1970) 
is even moe explicit about differences in the costs ratio between civil and criminal cases:
...
In a criminal case, on the other hand, we do not view the social disutility of convicting an innocent man as equivalent to the disutility of acquitting someone who is guilty (397 U. S. 358, 371).

\end{quote}


Claims about the moral asymmetry or costs ratios can of of course be contested. 
Some have argued, for example, that the standard of proof in criminal case shoudl 
in fact be much lower than 90\%, and they have done so by offerring a different 
...
paired with expected utility theory, provide an analytical framework to justify and meanglfully debate  the different degrees of stringency necessary for decision criteria---proof standards---in civil or criminal trials.
In later chapters, we will examine more in detail these ana;lytical framework and how it help 
theorize about the values that should informa trial decisions, such as minimization of expected costs, 
truth and accuracy, the risk of of error and its fair allocation. (REFER HERE TO THESE LATER CHAPTERS)
...
Even if they set aside practical challenges about 
and  focus on probabilistic thresholds as an analytical 
dn theorethical tool, legal probabilist 
are not out of the woods yet. Another set of problems for a probabilistic 
intepretation of proof standards is analytical and theoretical. 
Even if the practical problems can 
be overcome, theoretical difficulties remain. 
We will focus on two in particular: naked statistical 
evidence and the difficulty with conjunction. 
...
That is,
%
\[ \text{convict provided }  		 \frac{cost(CI)}{cost(AG)} < \frac{Pr(G | E)}{\pr(I | E )}.\footnote{This follows from $\pr(I | E ) \cdot cost(CI) <  Pr(G | E) \cdot cost(AG)$; see [@Kaplan1968Decision]. This model assumes, simplistically, 
that correct decisions do not bring any positive utility. More complex models 
are also possible, but the basic idea is the same; see \cite{Dekay1996, kaplow2012}.} \]
%
For the inequalityto be satisfied, the posterior ratio 
$\frac{\pr(G | E)}{\pr(I | E )}$ should exceed the cost ratio $\frac{cost(CI)}{cost(AG)}$. 
