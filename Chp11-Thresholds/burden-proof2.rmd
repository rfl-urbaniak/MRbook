...
s_{AB}& =  s_{A}\times s_{B} 
 ...
 they are codified in the Bayesian network in Figure \ref{network-conjunction}. \todo{more work on explaining independence in BNs is needed at some point, not sure if here. } \todo{How about without indepedence? Can we find a counterexample?}
...
a <- ggplot(data.frame(x=c(0,1)), aes(x)) +   ylim(0, 5) +
  ...
  geom_line(aes(x = x, y =  combined(x,0.1, 0.2)))+
  #geom_line(aes(x = x, y =  combinedMarcello(x,0.1, 0.2),color = "joint LR for k = 0.1, t=0.2"))+
  xlab("rel(a)=rel(b)")+ ylab("LR")+theme_tufte()
  geom_line(aes(x = x, y =  combined(x,0.1, 0.4)))

a

m <- ggplot(data.frame(x=c(0,1)), aes(x)) +   ylim(0, 5) +
  stat_function(fun=function(x)(x/(1-x)), geom="line", aes(colour="LR(a)=LR(b)"))  +
