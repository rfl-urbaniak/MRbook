...
Now, does a similar claim if we drop the independence assumption specific to \textsf{DAG1}? A claim somewhat weaker than Fact \ref{fac:BFindep} can be proven without employing independencies entailed by \textsf{DAG1}, relying only on some independencies entailed by  \textsf{DAG2}. Let's abbreviate:
\begin{align*}
...
First, let's look at simulations based on \textsf{DAG1} (Figure \ref{fig:DAG1BF}). We  can illustrate the distribution of Bayes factors in the two separate scenarios used as assumptions of the above corollaries. 
...
The picture doesn't change when we move to \textsf{DAG2} (Figure \ref{fig:BFind2}). One reason this is interesting is that this suggests that the additional assumption we used in the proof of Corollary \ref{cor:BFweaker} was not needed. We will take a closer look at this issue soon.  
...
\hspace{2mm}\scalebox{1}{\begin{subfigure}[!ht]{0.45\textwidth}
```{r,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%"}
CPkable0("conjunctionBNL","A") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 

CPkable1("conjunctionBNL","B") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 

CPkable1("conjunctionBNL","a") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 
...
```{r,echo=TRUE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%", dpi = 300,fig.width=2.5, fig.height=2}
#graphviz.plot(conjunctionDAG, layout = "dot")
...
# LRAs > LRABs
newdata <- subset(doubleDepTable, LRAs > LRABs & BFAs > 1 & BFBs > 1 & BFABs > BFAs)
one_row <- head(newdata, 1)
...
\hspace{2mm}\scalebox{1}{\begin{subfigure}[!ht]{0.45\textwidth}
```{r,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%"}
CPkable0("conjunctionDoubleBNL","A") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 

CPkable1("conjunctionDoubleBNL","B") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 

CPkable1("conjunctionDoubleBNL","a") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 
...
\subcaption{Marginal probabilities. }
\end{subfigure}} 
\caption{A double dependency. $LR_A  \approx 2.11$, $LR_B \approx 1.183$,  $LR_{AB} \approx 1.319$. $BF_A \approx  1.06, BF_B \approx	1.076, BF_{AB}\approx	1.14$}.
\label{tab:CPTDoubleL}
\end{figure}
