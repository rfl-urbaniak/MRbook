...
probability threshold. Some have regarded these failures as strong
enough ground to advocate a different conception of probability, along
the lines of Baconian probability (L. J. Cohen, 2008), fuzzy logic or
belief functions (Clermont, 2013), or reject legal probabilism
altogether (Allen \& Stein, 2013). But posterior probability thresholds
are not the only way of understanding standards of proof from a
probabilistic perspective. Standards of proof can also be modeled using
probabilistic measures of \textit{evidential strength}. This is the
approach we explore in this section.
...
conjunction paradox to `evaporate.' But this is not quite right. On the
evidential strength interpretation of the standard of proof, the other
direction of the conjunction principle, distribution, fails. If
...
strong evidence in favor of \(B\). This is odd. If we interpret the
standard of proof using evidential strength, it follows that one can
...
The prospects for legal probabilism look grim. If the standard of proof
...
\end{align*} \noindent (See the appendix for a proof.) Thus, the
combined support \(BF_{AB}\) will always be higher than the individual
support so long as the individual pieces of evidence positively support
their respective hypothesis. This claim holds assuming (roughly) that
hypotheses \(A\) and \(B\) are independent and that items of evidence
\(a\) and \(b\) are independent. These assumptions are plausible insofar
as the \textsf{DAG 1} in Figure \ref{fig:conjunctionDAGchapter} is a
plausible representation of the situation at hand. The result
generalizes beyond two pieces of evidence; see Figure
\ref{fig:bfconjunction5}. So Dawid's claim that `the support supplied by
...
Bayes factor) is vindicated.\footnote{The order is reversed if the items
  of evidence oppose the individual hypotheses. Neutral evidence results
  in a combined Bayes factor of 1, no matter the prior or the number of
  items of evidence}
...
\(BF_B\) is greater than 1, so is \(BF^{'}_B\), and similarly, whenever
\(BF_A\) is greater than 1, so is \(BF^{'}_A\). Thus, the joint Bayes
factor \(BF_{AB}\) will be greater than any of the individual Bayes
factors. In contrast, if the underlying \textsf{DAG} were to contain a
direct dependence between the items of evidence, the claim would no
longer hold, and the joint Bayes factor could be lower than either of
the individual Bayes factors (see the appendix for a proof).
...
aggregation. But aggregation is a principle about the standard of proof,
repeated below for convenience:

\[S[a, A] \wedge S[b, B] \Rightarrow  S[a \wedge b, A\wedge B]. \]

\noindent  The Bayes factor and the likelihood ratio are instead
measures of evidential strength. So, to complete the argument, the
standard of proof should be formulated in terms of evidential strength.
How can this be done? The rule of decision could be: liability is proven
according to the governing standard of proof if and only if the
evidential support in favor of \(H\)---measured by the Bayes factor or
the likelihood ratio---meets a suitably high threshold. The threshold
would no longer be a posterior probability between 0 and 1, but a number
somewhere above one. The greater this number, the more stringent the
standard of proof, for any value above one.

The only question left at this point is, how to identify the appropriate
evidential strength threshold? The answer to this question is not
obvious. Below we examine two possible approaches.

First, the threshold for the Bayes factor and the threshold for the
likelihood ratio, call them \(t_{BF}\) and \(t_{LR}\), can be derived
from the threshold \(t\) for the posterior probability by manipulating
two simple equations. Consider first the Bayes factor threshold. Since
\begin{align*}
...
compared to the composite claim \(A \wedge B\). For simplicity, suppose
the individual claims \(A\) and \(B\) are probabilistically independent.
Consider a posterior threshold of \(.95\) as might be appropriate in a
criminal case. If \(A\) and \(B\) both have a prior probability of, say
\(.1\), the threshold \(t^A_{BF}=t_{BF}^B=\nicefrac{.95}{.1}=9.5\) for
\(A\) or \(B\) individually.\footnote{Note the superscripts \(A\) and
  \(B\). The Bayes factor threshold \(t_{BF}\) is now indexed by the
  superscript to the claim of interest because the threshold \(T_{BF}\)
  is prior-dependent and thus claim-dependent (since different claims
  have different prior probabilities).} The composite claim
...
fixing the evidential strength threshold regardless of the prior
probability of the hypothesis of interest. This approach raises the
question of how the threshold should be fixed irrespective of the
priors. Standard decision theory can no longer help But assuming the
question can be answered, the good news is that the fixed threshold
approach vindicates aggregation.

It is easy to see why the principle of aggregation holds for any fixed
...
\(BF_B\) meet the threshold \(t_{BF}\), then usually also the combined
Bayes factor \(BF_{AB}\) meets \(t_{BF}\). The same applies for the
likelihood ratio threshold. Whenever \(LR_A\) and \(LR_B\) meet the
threshold \(t_{LR}\), then usually also \(LR_{AB}\) meets \(t_{LR}\).
Success. Aggregation is finally vindicated. Since aggregation could not
be justified using posterior probabilities \(\pr{A \vert a}\) and
\(\pr{B \vert b}\) or using a variable evidential strength threshold,
this result militates in favor of the fixed evidential strength
threshold.
...
The trouble is that, under the fixed evidential strength threshold
interpretation, the seemingly uncontroversial principle of distribution
...
individual conjuncts. How can this principle possibly fail?
