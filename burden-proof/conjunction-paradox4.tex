...
First formulated \ali{R: fix references to Cohen} by Cohen (1977), the
difficulty with conjunction has enjoyed a great deal of scholarly
...
\ali{R: fix ref} Cohen (1977) argues that common law systems subscribe
to what he calls a \textbf{conjunction principle}, according to which if
two claims, \(A\) and \(B\), are established according to the governing
standard of proof, so is their conjunction \(A\wedge B\) (and
\emph{vice versa}). If the conjunction principle holds, the following
must be equivalent, where \(S\) is a placeholder for the standard of
proof:
...
even in combination with \(a\).\footnote{Because of the relations of
  independence in Figure \ref{fig:conjunctionDAG2chapter},
...
random Bayesian networks based on \textsf{DAG 1} and \textsf{DAG 2}. The
joint posterior is lower than both individual posteriors 68\% of the
time for \textsf{DAG 1}, and around 60\% for \textsf{DAG 2} (see the
...
altogether. We do not rehearse these lines of argument here.
\mar{R:Add footnote about what these strategies are like} Our focus is
on \textit{probabilistic} strategies for capturing the principle of
...
decision criteria formulated in terms of evidential strength. This is
the strategy we examine in this section. Instead of a posterior
...
aggregation (at least, given some additional assumptions), but is still
