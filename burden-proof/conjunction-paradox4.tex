...
The standard of proof is a decision criterion. It answers the question,
what characteristics should the evidence presented at trial possess to
warrant a verdict of civil or criminal liability? According to many
legal probabilists, it is enough for the evidence to establish liability
with a probability above a suitable threshold (see Bernoulli, 1713;
Dekay, 1996; Kaplan, 1968; Kaye, 1979; Laplace, 1814; Laudan, 2006). In
criminal cases, the bar is quite high, and so the threshold will be, say
``at least .95'' probability``. In civil cases, the bar is lower, and so
the threshold will be, say''at least .5 probability".
...
and .5 thresholds can be formally justified. Unfortunately, this
intepretation is not without problems either. Chapter Chapters ZZ
\todo{REFER TO EARLIER CHAPTER} examines a first theoretical problem:
the paradox of naked statistical evidence. This chapter examines a
second theoretical problem: what we will
\textbf{the difficulty with conjunction}, often called
...
supporting evidence. With this in mind, for two claims, the conjunction
...
interacting with a public official. A witness testimony (call it
\textsf{witness}) is offered in support of the proposition that the
defendant injured the victim (call this proposition \textsf{injury}). In
addition, the fact that the defendant made a call to an emergency number
(call this fact \textsf{call}) is offered as evidence for the
proposition that the defendant knew he was dealing with a firefighter
(call this proposition \textsf{firefighther}). If
...
generally will not.\footnote{An additional assumption to make here is
  that both \(\pr{\textsf{injury} \vert \textsf{witness}}\) and
...
interpreted as a probability threshold. Can we say something more
general? Do the independence assumptions hold in other cases? To address
these questions, we will represent formally the relationship between
generic claims \(A\), \(B\) and the conjunction \(A\wedge B\), as well
as the supporting evidence \(a\), \(b\) and the conjunction
\(a\wedge b\). Bayesian networks are well-suited for this task. We have
already introduced Bayesian networks in Chapter ZZ.
\todo{REF TO OTHER CHAPETR} Below we only sketch the essential ideas.
...
graphs visualized in Figure \ref{fig:abBDAG} represent the fact that the
two items of evidence each separately support their own hypothesis:
\(a\) supports \(A\) and \(b\) supports \(B\). The two graphs can be
joined to represent the conjunction \(A\wedge B\). To this end, a
...
equal 1. Once again, the conjunction principle fails because aggregation
fails. The difficulty with conjunction persists.
...
is displayed in Figure \ref{fig:conjunctionDAG2chapter}, call it
\textsf{DAG 2}. Since there is now an open path between nodes \(A\) and
\(B\), the new graph no longer guarantees the probabilistic independence
of \(A\) and \(B\), nor the indepedence of evidence nodes \(a\) and
\(b\). Note however, we still assume there is no \emph{direct}
dependence between the items of evidence; this assumption will turn out
crucial for the status of aggregation). The items of evidence are still
probabilistically independent of one another \textit{conditional} on
their respective hypotheses. That is,
