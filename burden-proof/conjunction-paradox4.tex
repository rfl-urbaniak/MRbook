...
should be judged criminally or civilly liable, and otherwise they should
be acquitted. But when does the evidence meet the standard of proof and
thus warrant a judgement of liability? According to some legal
probabilists, a verdict against the defendant is warranted in case the
evidence establishes, with a probability above a suitable threshold,
that the defendant is criminally or civilly liable (see Bernoulli, 1713;
Dekay, 1996; Kaplan, 1968; Kaye, 1979; Laplace, 1814; Laudan, 2006). In
...
also known as \textbf{the conjunction paradox}. The chapter is
structured as follows. First, we describe the difficulty with
conjunction. Next, we detail several strategies that legal probabilists
have pursued as a response to this difficulty. These strategies are
promising and worth examining, but we show that they are ultimately
unsatisfactory. Finally, we put forward our own proposal.
...
conjunction principle, specifically, aggregation. Even though
...
probability of each conjunct, in isolation, can meet the required
probability threshold without the conjunction as a whole meeting the
threshold.
...
probability threshold. Some think these failures are strong enough
...
XX. \todo{REFER TO EARLIER CHAPTERS} As we show in detail later, under
...
conjunction paradox to `evaporate.' But, as we show in detail later,
this is not quite right. On the evidential strength interpretation of
the standard of proof, the other direction of the conjunction principle,
...
interpret the standard of proof using evidential strength, one can
...
The result generalizes beyond two pieces of evidence (see Figure
\ref{fig:bfconjunction5}). So, under these circumstances, Dawid's claim
that `the support supplied by the conjunction of several independent
testimonies exceeds that supplied by any of its constituents' (if
support is to be measured in terms of Bayes factor) is
vindicated.\footnote{The order is reversed if the items of evidence
...
probabilistic measure of evidential strength, extensively discussed in
Chapter XX. \todo{REFER TO EARLIER CHAPTER} The likelihood ratio
compares the probability of the evidence on the assumption that a
...
contrasted to its negation). The question is whether the combined
support measured by the combined likelihood ratio
...
First, let the sensitivity and specificity of the items of evidence be
the same and equal \(x\). Second, let \(A\) and \(B\) be
...
let \noindent where \(STR[E, H]\) stands for the strength of the
evidential support of a piece of evidence \(E\) toward a hypothesis
\(H\), measured by the Bayes factor or the likelihood ratio. We have
established the following claim:

\[STR[a \wedge b, A\wedge A] \geq min(STR[a, A], STR[b, B]),\] This fact
can be used to justify aggregation. But there is a step missing here.

Aggregation is not directly a principle about evidential strength. It is
a principle about the standard of proof. So, to complete the argument,
the standard of proof should be tied to evidential strength. How can
this be done? The rule of decision could be: liability is proven
according to the governing standard of proof in case the strength of the
evidential support in favor of \(H\)---measured by the Bayes factor or
the likelihood ratio---meets a suitably high threshold. On this reading,
the threshold is no longer a posterior probability between 0 and 1, but
a number somewhere above one. The greater this number, the more
stringent the standard of proof, for any value above one.

The only question left at this point is, how to identify the appropriate
evidential strength threshold? The answer is not obvious. Below we
examine two possible approaches. First, the threshold for the Bayes
factor and the threshold for the likelihood ratio, call them \(t_{BF}\)
and \(t_{LR}\), can be derived from the threshold \(t\) for the
posterior probability by manipulating two simple equations. Consider
first the Bayes factor threshold. Since \begin{align*}
...
\noindent The same strategy works for the likelihood ratio threshold. By
...
\noindent The stringency of the posterior probability threshold \(t\)
can be determined in a decision-theoretic manner via the minimization of
expected costs (see Chapter XX \todo{REFER TO ERALIER CHAPPER}). Once
the value of \(t\) is fixed in this way, the value \(t_{BF}\) and
\(t_{LR}\) can be easily derived by the equations above. The threshold
\(t_{BF}\) and \(t_{LR}\) will depend on the prior probability of the
hypothesis. The higher the prior probability, the lower the threshold.
Whether this is a desirable property for a decision threshold can be
questioned, but a similar point applies to the posterior threshold
...
For simplicity, suppose the individual claims \(A\) and \(B\) are
...
individually.\footnote{Note the superscripts \(A\) and \(B\). The Bayes
  ...
  probabilities).} The composite claim \(A \wedge B\) will be associated
with the threshold
\(t^{A\wedge B}_{BF}=\nicefrac{.95}{(.1\times .1)}=95\), a much higher
value. Suppose each claim barely meet the 9.5 Bayes factor threshold. If
they are independent, the joint Bayes factor results from multiplying
the individual Bayes factors, that is, \(9.5 \times 9.5=90.25\). This is
not quite enough to meet \(t^{A\wedge B}_{BF}=95\). So aggregation
fails. An analogous point holds for the likelihood ratio
threshold.\footnote{Say \(A\) and \(B\) have prior probabilities of
  \(.2\) and \(.3\) respectively. On this approach, the likelihood ratio
  threshold for \(A\) and \(B\) will be \(t_{LR}^{A}\approx 76\) and
  ...
  namely a likelihood ratio of 297. Once again, aggregation fails.} The
culprit is the fact that \(t_{BF}\) and \(t_{LR}\) have different values
when applied to individual claims \(A\) and \(B\) as opposed to the
composite claim \(A \wedge B\).

The second approach we consider is more straightforward. It consists in
fixing the evidential strength threshold regardless of the hypothesis of
interest. Criminal cases would still have a higher decision threshold
than civil cases, but the threshold would be constant across individual
and composite claims.\footnote{ This approach raises the question of how
  the threshold should be fixed in the first place. Standard decision
  theory can no longer help.}\\
The good news is that the fixed threshold approach vindicates
aggregation. As seen previously, the combined evidential support is
usually greater than at least one, if not all, individual evidential
supports, whether measured by the Bayes factor or the likelihood ratio.
So whenever \(BF_A\) and \(BF_B\) meet the threshold \(t_{BF}\), then
usually also the combined Bayes factor \(BF_{AB}\) meets \(t_{BF}\). The
same applies for the likelihood ratio threshold. Whenever \(LR_A\) and
\(LR_B\) meet the threshold \(t_{LR}\), then usually also \(LR_{AB}\)
meets \(t_{LR}\). Success. Aggregation is finally vindicated.

\hypertarget{the-failure-of-distribution}{%
\subsection{The failure of
distribution}\label{the-failure-of-distribution}}

The trouble is that, if the evidential strength threshold is held fixed
across individual and composite claims, the principle of distribution
becomes problematic. This is the other direction of the conjunction
principle. Distribution posits that establishing the conjunction, by a
governing standard of proof, is sufficient for establishing the
individual conjuncts. This is an uncontroversial principle. How can it
possibly fail? Suppose the combined Bayes factor,
...
not happen if \(b\) positively supports \(B\)). Looking at the graph in
Figure \ref{fig:bfconjunction5} should be enough to convince oneself
that this is the case. The problem for likelihood ratio is analogous.
For suppose evidence \(a \et b\) supports \(A \et B\) to the required
threshold \(t\). The threshold in this case should be some order of
magnitude greater than one. If the combined likelihood ratio meets the
threshold \(t_{LR}\), one of the individual likelihood ratios may well
be below \(t_{LR}\); see graph in Figure \ref{fig:jointLRMarcello}.
So---if the standard of proof is interpreted using a fixed evidential
strength threshold---even though the conjunction \(A \et B\) was proven
...
by \(a\) and \(b\) individually. So even when the conjunction
\(A \wedge B\) is established by the requisite standard given evidence
\(a\wedge b\), it might still be that \(A\) does not meet the requisite
standard (given \(a\)) nor does \(B\) (given \(b\)). Or at least one
might argue this way.

To accommodate this line of reasoning, consider this weaker distribution
...
for establishing one of the conjuncts. It seems that any formalization
of the standard of proof should obey (DIS2). Yet, if \(A\) and \(B\) are
...
evidential strength
threshold.\raf{M: Need proof of this. Is it in the appendix?} So the
counterexamples to the first distribution principle also work against
the weaker version (see appendix for details.) But if distribution
fails, there would be cases in which, even though the conjunction
\(A\et B\) is established by the desired standard of proof, one of the
individual claims fails to meet the standard. Surely this is odd.
...
aggregation. The evidential threshold is subject to another kind of
difficulty, the failure of distribution. As a third approach, the
...
\(H_d\), and decide for the one that garners the better evidential
support.
