...
In offerring a definition of proof beyond a reasonable doubt, Justice Newman of United States Court of Appeals for the Second Circuit worries that the use of the expressions such as 'probability' or 'likelihood' may confuse jurors, as it these expressions are often applied to predictions about future events that may or may not occur. 
Instead, Newman prefers the expressions 'degree of certainity'. This is how he chracterized the 
...
Here is an illustration of the probabilistic interpretation of proof standards 
as an analytical tool. Standards of proof are usually ranked from the least demanding, such as preponderance of the evidence, to the most demanding, such as proof beyond a reasonable doubt, even though 
this distinction  has not always been around; see e.g. United States v. Feinberg, 140 F.2d 592 (2d Cir. 1944) (`` to distinguish between the evidence which should satisfy reasonable men, and the evidence which should satisfy reasonable men beyond a reasonable doubt. While at times it may be practicable to deal with these as separate without unreal refinements, in the long run the line between them is too thin for day to day use'' p. 594). 
...
The analysis offerred 
so far is limited in that it only considers 
the costs of mistaken decisions, but leaves out the benefits 
associated with correct decisions. More comprehensive analyses would 
consider both, even though the basic insight would remain 
the same [@laudan2016law]. Trial decision-making is viewed as one instrument among others for 
maximizing overall social welfare [@Posner1973; @Dekay1996].
But considering the costs of incorrect decisions as well as the 
...
where benefits will correspond to positive numbers and 
costs to negative numbers  In a criminal trial, the following costs, 
should be assessed: $\ut(CI)$ and$\ut(AG)$, as well as the following benefits:
$\ut(CG)$ and $\ut(AI)$. Given this set-up, a conviction 
is justified provided its expected utility---that is,
$\pr{I | E } \cdot \ut(CI) + \pr{G | E} \cdot \ut(CG)$---exceeds the expected utility 
of an acquittal---that is, $\pr{G | E} \cdot \ut(AG) + \pr{I | E} \cdot \ut(AI)$. The 
threshold is identified by the equation:

\[ \pr{G | E} > \frac{1}{1+\frac{\ut(CG)-\ut(AG)}{\ut(AI)-\ut(CI)}}\]

If the benefits of convictions and acquittals are zero, this inequality reurns the same threshold 
the earlier inequality that only considered costs. Say, if the costs of a mistaken conviction is nine times the cost of a mistaken acquittal, that is, $\frac{\ut{CI}}{\ut{AG}}=9$, while the benefits are zero, the decision threhshold should be a guilt probability of $\frac{1}{1+(0+1)/(0+9)}=0.9$, as before. Or suppose the magnitude of the benefits resulting from acquitting an innocent defendant is the same as the magnitude of the costs resulting from convicting an innocent. Say the benefit is +9 units of utility (9 units of utility are gained), while the cost is -9 units of utility (9 units of utility are lost).  Similarly, suppose the magnitude of the benefits resulting from convicting a guilty defendant is the same as the magnitude of the cost resulting from acquitting a guilty defendant. Say the benefit is the acquisition 
of 1 unit of utility, while the costs is the loss of 1 unit of utility. Then, once again, the threshold would be 
$\frac{1}{(1+(1+1)/(9+9)}=0.9$, as before. 

But consider now these assignments of utilities: $\ut(CI)=-9, \ut(AG)=-1, \ut(CG)= 5, \ut(AI)=5$.
The threshhold then would be $\frac{1}{1+(5+1)/(5+9)}=0.7$, singificanly below 
the 90\% threshold. If the benefits of correct decisions are further increaesed, say at 7 units of utility each, the threhsold would be further lowered to 66% since $\frac{1}{1+(7+1)/(7+9)}=0.66$. So the the benefits of correct decisions are not inconsequential in setting up the standard. In order to still keep the stadard for criminal 
conviction releatively high the benefit of correct conviction would have to be close to zero or alternatively, the benefit of a correc acquittals would have to be significantly higher than the benefit of correct convcition. Say
$\ut(CG)= 2, \ut(AI)=18$, while still $\ut(CI)=-9, \ut(AG)=-1$, the threhhold would be $\frac{1}{1+(2+1)/(18+9)}=0.9$.

What the new inequality shows is also that what matters is the ratio of the difference 
between utilities, not so much the costs ratio or the benefit ratio. 
The same cost ratio and the same benefit ratio could give 
rise to different threshold, as follows:

$\ut(CI)=-9, \ut(AG)=-1, \ut(CG)= 1, \ut(AI)=1$, and thus $\frac{1}{1+(1+1)/(1+9)}=0.83$

$\ut(CI)=-9, \ut(AG)=-1, \ut(CG)= 10, \ut(AI)=10$, and thus $\frac{1}{1+(10+1)/(10+9)}=0.63$.

Let's try to explore this more closely. How should utilities be assigned? 
This may be an empirical matter, but it is also a political matter. 
It is elected officials who should set the appropriiate 
assignments of utilities, and elected officials should of coruse represent 
the will of the people. We wil now look at three plausible assignments 
of utility, which might call right-wing conservative, 
centrist liberal and left-wing radical. The appropriate threshold will vary 
dramtucally depeding on which political views one favors. 


DESCRIBE ASSIGNMENTS OF UTILITIES RELATED TO 
THESE DIFFERENT POLITICAL VIEWS. 

1) CONSERVATIVE WILL EMPHASIZE SECURITY AND THE WAR ON CRIME

2) CENTRIST WILL BE MORE BALANCED

3) MORE LEFTIST WILL LEAN TOWARD ABOLITION 
OF CRIMINAL TRIAL 



Consider the loss resulting from convicting 
an innocent defendant. This loss may include: the inappropriate assignment of culpability (-1); damaged reputation (-1); loss of income (-1); severance from family members (-1); putting other citizens at higher risk of victimization by the actual perpetrator who is not convicted (-1). This a total loss of utility of -5. What about the correct acquittal of an innocent? There woud be no inappropriate assignment of culpability (0); no loss of income (0); no severance from familiy members (0). The actual perpetrator, however, would still potentially victimize others (-1). 
The innocent defendant who was acquitted could still experience damaged reputation, but we shall leave this aside. 
What this shows is that there is a significant cost of trying an innocent defendant, no mater the final decision. It is the cost of failing to identify the true perpetrator and thus putting other citizens at potential risk of victimization, as well as the costs of damagign someone's reputation. This baseline cost of trying an innocent defedant become even graver when an innocent defendant unjustly convicted. The costs of trying an innocent are significant, but they become even higher whne an innocent person is convicted. So, all things considered, $\ut(AI)-\ut(CI)=-2+5=3$.

Consider the loss resulting from failing to convict a guilty defendant. This loss will mainly include putting other citizens at risk of victimization by the perpetrator who is not convicted (-1) and possibly weakened 
the deterrent effects of the trial system (-1). There would be no damaged reputation (0); no loss of income (0); no severance from family members (0). What about the conviction of an innocent defendant? The defendant would still experience damaged reputation (-1); loss of income (-1); severance from family members (-1). Howvere, citizens 
would not be subject to a higher risk of victimization (0) and there would be no weakend deterrence effect of the trial system (0). So, all things considered, $\ut(CG)-\ut(AG)=-3+2=-1$.

So the threshold would be: $\frac{1}{1+\frac{-1}{3}}=1.5$. IS THIS RIGHT? 
DOES THIS MEAN NO THRESHOLD WILL EVER SUFFICE? THIS IS 
A DERIVATION OF THE ABOLITIONIST VIEW. INTERESTING!



Claims about cost ratios and their magnitude, and claims 
about differences between criminal and civil cases, 
can of course be contested.  Some have argued, for example, that the standard of proof in criminal cases should 
be  lower than commonly assumed, and they have done so by offerring a different 
assessment of the cost ratio (CITE LAUDAN -- PERHASP LAUDAN'S DERIVATION OF A LOWERE THRESHOLD COULD BE DISCUSSED HERE IN DETAIL AS ANOTHER EXAMPLE OF THE ANALYTICAL POWER OF THE PROBABILISTIC FRAMEWORK). 

Details aside, probabilistic thresholds, when 
paired with expected utility theory, provide an analytical framework 
to justify as well as meaningfully debate the different degrees 
of stringency necessary for decision criteria---i.e. legal proof 
standards---in civil or criminal trials. This analytical framework allows for even 
more finer distinctions, not explictily codified in the law.

The law typically  makes coarse  distinctions between standards of proof, such as 'proof beyond a reasonable doubt' for criminal cases, 'preponderance of the evidence' for civil cases and `clear and convincing evidence' for a narrow subset of civil cases in which the accusation against the defendant is particularly serious. But for rather different crimes, associated with rather different punishments, say murder and grand theft, the same standard of proof is applied for both. It is not obvioys why this should be so, except that a finer distinction may cause more confusion than there need to be.  If the probability required for a conviction or a finding of civil liability against the defendant is a function of weighing the costs and benefits that would result from true and false positives (as well as true and false negatives), the stringency of the threshold should depend on costs and benefits, and thus different cases may require different thresholds. Cases in which the charge is more serious than others---say, murder compared to grand theft---may require higher thresholds so long as the cost of a mistaken decision against the defendant is more significant.
In countries that allow for the death penalty or life imprisonment for certain 
crimes but not others, the cost of a mistaken conviction would be more serious for crimes with harsher punishments, other things being equal. Thus, the threhold should be placed appropriately higher. We could even think that the threhsold should vary across individual cases even for defendants charged with the exact same crime, provided the costs are different for different individuals. Needless to say, whether or not standards of proof should vary in this way is debated [@kaplow2012; @picinali2013]. Ultimately, the question is what considerations should be admissible in the calculus of costs and benefits. 
