...
address is the difficulty about conjunction. This difficulty arises 
...
\noindent
More generally, the conjuction principles requires that
...
not become certain as the number of claims under consideration increases. Thus, the conjunction principle is not clearly implausible. For the sake of argument, however, suppose the legal probabilist rejects it. 
...
What would an account of legal that obeys the conjunction principles look like?
Say a piece of evidence $E_1$ defeasibly suports claim $C_1$; $E_2$ defeasibly supports claim $C_2$; and so. Each of these relationships of evidential support could be undercut by defeaters $D_1$, $D_2$, and so on. Suppose the pieces of evidence are combined into the conjunction $E_1 \wedge E_2 \wedge \dots \wedge E_k$. Do the pieces of evidence combined support the conjunction $C_k \wedge C_2 \wedge \dots \wedge C_k$? Presumably yes. So long as no defeater $D_i$ undercuts any of the relatioships of evidential support, the combined evidence $E_1 \wedge E_2 \wedge \dots \wedge E_k$ supports the conjunction $C_1 \wedge C_2 \wedge \dots \wedge C_k$. On this view, the conjunction principle holds. Note, however, the difference between each individual claim $C_i$ and the conjunction
$C_1 \wedge C_2 \wedge \dots wedge C_k$. While each claim $C_i$ could be attacked by a defeater $D_i$, the conjunction $C_1 \wedge C_2 \wedge \dots \wedge C_k$ could be attacked by a larger set of defeaters, $D_1$, $D_2$, \dots, $D_k$. So $C_1 \wedge C_2 \wedge \dots \wedge C_k$ is more susceptible to be proven wrong than any individual claim $C_i$.  On other hand, $C_1 \wedge C_2 \wedge \dots \wedge C_k$ is supported 
by a  larger body of evidence $E_1 \wedge E_2 \wedge \dots \wedge E_k$ than each idnividual claim $C_i$. 

The probabilistic account is unable to capture any of 
these evidential relationships. Evidentila support could be captured in terms of posterior probability or in term,s of likelihood ratio. 

First note that $\pr{C_1 | E_1}>t$ and
$\pr{C_2 | E_2}>t$, it does not follow that 
$\pr{C_1 \wedge C_2 | E_1 \wedge E_1}>t$.  EXAMPLE?
