...
legal standards pf proof is unrealistic because its implementation would 
...
probability thresholds to guide their decisions. CITE EMPIRICAL EVIDENCE HERE FOR NARRATIVE AND PLAUSIBILITY THEORY AGAINST PROBABILISTIC APPROACH.
...
Let's start with an illustration of how 
...
of correct decisions. More comprehensive analyses should 
consider both, even though the basic insight would remain 
the same. That is, trial decision-making is viewed as an instrument 
for maximizing overall social welfare [@Posner1973; @Dekay1996; @laudan2016law].
Still, weighing both the costs of incorrect decisions and the 
benefits of correct decisions adds nuance to the analysis. 
So it is instructive to explore the implications 
of this more comprehensive analysis. 
...
of a mistaken conviction should be weighed against the (negative) 
utility of an incorrect acquittal: $\ut(CI)$ v. $\ut(AG)$. In addition, 
the (positive) utility of a correct conviction should be weighed against the (positive) 
...
returns the same threshold the earlier inequality that only considered costs. 
For example, if the costs of a mistaken conviction is nine times the cost of a mistaken acquittal, that is, $\frac{\ut(CI)}{\ut(AG)}=9$, while the benefits are zero, the decision threhshold should be a guilt probability of $\frac{1}{1+(1/9)}=0.9$, as before. Or suppose the magnitude of the benefits resulting from acquitting an innocent defendant is the same as the magnitude of the costs resulting from convicting an innocent. Say the benefit is +9 units of utility, while the cost is -9 units of utility.  Similarly, suppose the magnitude of the benefits resulting from convicting a guilty defendant is the same as the magnitude of the cost resulting from acquitting a guilty defendant. Say the benefit is +1 unit of utility, while the cost is -1 unit of utility. Again, the threshold would be $\frac{1}{(1+(1+1)/(9+9)}=0.9$. 

What the new inequality underscores is that what fixes the threshold is the ratio of the difference 
between utilities, not so much the cost ratio or the benefit ratio. The same cost ratio and the same benefit ratio could give rise to rather different thresholds. Consider these assignments of utilities: $\ut(CI)=-9, \ut(AG)=-1, \ut(CG)= 5, \ut(AI)=5$. The threshhold then would be $\frac{1}{1+(5+1)/(5+9)}=0.7$, significanly below 
the 90\% threshold. If the benefits of correct decisions are further increaesed, say at 7 units of utility each, the threhsold would be further lowered to 66% since $\frac{1}{1+(7+1)/(7+9)}=0.66$. 
In both cases, the cost ratio is fixed at 9:1 and the benefit ratio at 1:1. 
So the the benefits of correct decisions are not at all inconsequential. In order to keep the threshold for criminal 
convictions releatively high the benefit of correct conviction would have to be close to zero (as seen before) 
or alternatively, the benefit of a correct acquittals would have to be significantly higher than the benefit of correct conviction. Say $\ut(CG)= 2$ but $\ut(AI)=18$, while still $\ut(CI)=-9, \ut(AG)=-1$, the threhhold would be $\frac{1}{1+(2+1)/(18+9)}=0.9$.
...
(what the majority of people think the utilities should be), or 
a political question (how political ideaologies affect utility 
...
on the political party who is is charge at the moment. At least, even if 
standards of proof should be part of a country's constitution and 
not vary depeding on the political party in power, we can still agree 
that the stringency of standards of proof, especially in criminal cases, 
should reflect's people sentiments toward the harms and 
benefits that the trial system brings about. 

We will now explore different assignments 
of utility for correct and incorretc trial decisions. 
These are assignment might be traced back to different political ideaologies, or more generally, 
to different sentiments about the role of the trial systems. 

Let's start with what we might call a politically neutral assignment of utilities. Consider the loss resulting from convicting an innocent defendant. This loss includes: the inappropriate assignment of culpability (-1); damaged reputation (-1); loss of income (-1); severance from family members (-1); putting other citizens at risk of victimization by failing to  convict the actual perpetrator (-1); weakening the deterrence function of the trial system by failing to apprehend the perpetrator (-1). This is a total utility loss of -6. What about the correct acquittal of an innocent? There woud be no inappropriate assignment of culpability (0); no loss of income (0); no severance from familiy members (0). The actual perpetrator, however, could still victimize others (-1) and the trial system would have failed to apprehend the actual perpetrator (-1). The innocent defendant who is acquitted could still experience damaged reputation and other harms, but we shall leave these details aside. All in all, trying an innocent defendant, no matter the final decision, carries the baseline costs of failing to identify the true perpetrator (-1) and putting other citizens at risk of victimization (-1). This baseline cost increases when an innocent person is wrongly convicted. So, by adding everything up, $\ut(AI)-\ut(CI)=-2+6=+4$. Next, consider the loss resulting from acquitting a guilty defendant. This will include putting other citizens at risk of victimization by the perpetrator who is not convicted (-1) and possibly weaken the deterrence function of the trial system (-1). There would be no damaged reputation (0); no loss of income (0); no severance from family members (0). What about the conviction of a guilty defendant? The defendant would still experience damaged reputation (-1); loss of income (-1); severance from family members (-1). However, citizens would enjoy a lower risk of victimization (+1) and the deterrence function of the trial system would be reaffirmed (+1). So, all things considered, $\ut(CG)-\ut(AG)=-1+2=+1$. So, assuming the utilities are correctly assigned, the threshold for a criminal conviction should be $\frac{1}{1+\frac{1}{4}}=0.8$, lower than the more common $90\%$  threshold. 

The above analysis is a politically neutral assignment of utilities because it takes into account the costs of a conviction, even for those who are factually guilty, such as loss of income and severance from familiy members. These are issues often emphasized by those on the left who are wary of the costs of criminalization and punitiveness, even for those who, strcitly, did committ a crime. At the same time, the above analysis also takes into account the negative consequences that result from failing to apprehend the actula perpetrato, such as hightened crime victimization, a point
often made by people on the right who are concerned with so-called "law and order". 

The threshold can be made more or less stringent 
depending on how utilities are assigned.
Say someone is extremely concerned about risk of victimization for different reasons, 
...
lower risk of of victimizatio associated with a correct conviction are also icreased 
from +1 to +3. Then, $\ut(AI)-\ut(CI)=-3+7=+4$, as before, but $\ut(CG)-\ut(AG)=-1+4=+3$. The threshodl would be $\frac{1}{1+\frac{3}{4}}=0.57$, significanly lower than before. Unsurprsingly, someone who is very concerned with the  risk of victimization will favor a lower threshold for conviction in criminal cases. (CITE LAUDAN). 

Let's now explore the view of someone who is 
more concerned about negative effects of criminalization, such as loss of income 
and severance from family members. Suppose the utility loss is increaed from -1 to -2 for each of these items.
Then, $\ut(AI)-\ut(CI)=-5+9=+4$, as before, but $\ut(CG)-\ut(AG)=-3+2=-1$. The threshodl would be $\frac{1}{1+\frac{-1}{4}}=1.33$. Interestingly, what this shows is that, under this assignment of utilities, convciting a defendant never maximizes expected utility unless the evidence establishes guilt with 133% probability. Which is clearly impossible. In other words, if the costs of criminalization are so high, convicting 
anyone is never justified, not even someone we are 100\% sure to be guilty. This conclusion could be take as a reductio ad absurdum of the utility assignments ijn question, or alternative, as an argument that the trial system and the 
practise of convicting people shoud be abolished. The latter is a view that agree with radical 
proposal about prison abolition. (CITE APPROPRIATE REFERENCES ABOUT PRISON ABOLITION).

Claims about cost ratios and their magnitude, and claims 
about differences between criminal and civil cases, 
can of course be contested.  As the above discussion shows, 
...
standards---in criminal as well as civil trials. This analytical framework allows for even 
