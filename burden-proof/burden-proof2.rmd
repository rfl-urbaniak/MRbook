...
\noindent
This analysis only considers the costs of mistaken decisions, 
but leaves out the benefits associated with correct decisions. 
More comprehensive analyses would consider both, but the basic insight would remain the same [@Dekay1996; @laudan2016law]. Trial decision-making is viewed as one instrument among others for 
maximizing overall social welfare [@Posner1973].

for the sake of completeness, however, consider costs and benefits of trial decisions. In a criminal trial, 
let the costs be$\cost(CI)$ and $\cost(AG)$, and let the benefits be $\benefit(CG)$ and $\benefit(AI)$.
On this more complex analysis, a conviction is justified provided the expected utility of a conviction---that is,
$\pr{I | E } \cdot \cost(CI) + \pr{G | E} \cdot \benefit(CG)$---exceeds the expected utility of an acquittal---that is, $\pr{G | E} \cdot \cost(AG) + \pr{I | E} \cdot \benefit(AI)$. The 
threshold is identified by the equation:

\[ \pr{G | E} > \frac{1}{1+\frac{\benefit(CG)-\cost(AG)}{\benefit(AI)-\cost(CI)}}\]

For simplicity, we are supposing here that costs stand for positive numbers, 
while costs stand for negative number. If the benefits of convictions and acquittals are zero, this equation would boil down to the earlier equation. Say, if the costs of a mistaken conviction is nine times the cost of a mistaken acquittal, that is, $\frac{\cost{CI}}{\cost{AG}}=9$, while the benefits are zero,
the decision threhshold should be a guilt probability of $\frac{1}{(1+(0-1)/(0-9)}=0.9$, as before.
Or suppose the magnitude of the benefits resulting from acquitting an innocent defendant is the same as the magnitude of the costs resulting from convicting an innocent. Say the benefit is +9 units of utility (9 units of utility are gained), while the cost is -9 units of utility (9 units of utility are lost).  Similarly, suppose
the magnitude of the benefits resulting from convicting a guilty defendant is the same as the magnitude 
of the cost resulting from acquitting a guilty defendant. Say the benefit is the acquisition 
...
NEED  TO EXPLORE WHAT HAPPEN IF COST AND BENEFITS ARE NOT THE SAME. HOW DOES 
...
Consider the loss resulting from convicting an innocent defendant. This loss may include: the 
inappropriate assignment of culpability; loss of income; severance from family members; putting other citizens at higher risk of victimization by the actual perpetrator. What about the correct acquittal of an innocent? There woud be no inappropriate assignment of culpability; no loss of income; no severance from familiy members. The actual perpetrator, however, would still potentially victimize others. What this shows is that there is a significant cost of trying an innocent defendant, no mater the dfinal decision. It is the cost of failing to identify the true perpetrator and thus putting other citizens at potential risk of victimization. This baseline cost of trying an innocent defedant become even graver when an innocent defendant unjustly convicted. The costs of trying an innocent at significant, but they become even higher whne an innocent person is convicted. 
