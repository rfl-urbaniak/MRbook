...
denies the accumulation of risk. If one is justifiably very sure about the truth of 
each claim considered seperataly, one should not be equally sure of their conjunction. Say you have checked 
each page of a book and found no error whatsover. So, for each page, you are sure there is no error. Can you, on this basis, be sure that the book as a whole contains no error whatsover? Not really. As the number of pages grows, the risk you might gave missed a mistake increases. It becomes virtually certain that there must be at least one error in the book as a whole, although for each page you are justifiably sure there is no error. The same applies to product quality checks. You may be very sure, for each prodoct you chcked, that they are free from defects. But you cannot, on this basis, be sure that all items you chcked are free from defects. In fact, since the risk of error accumulates, you should be very sure to have missed at least one defective item. The conjunction principle, insted, would have us deny this basic principle of the accumulation of risk.


The conjunction principle, however, is not so clearly implausible. The example pertaining to risk accumulation all have in common the fact that, as the number conjuncts increases, the possibility of error becomes more likely and at some point it becomes certain. But in trial proceedings the possibility of error need not become certain as the numbere of claims increases. It is perhpas best to understand the process of proof as follows. Say there is a piece of eviodence $E_1$ that defesibly suport a claim $C_1$; $E_2$ defesibly suport claims $C_2$; and so up to $E_k$ and $C_k$.  Each of these relationshio of eviodenetial support could defeated by defeaters, $D_i$.
Does the conjunction of $E_1 \wedge E_2 \wedge \dots \wedge E_k$ supports the conjuction $C_k \wedge C_2 \wedge \dots wedge C_k$? Presumably yes, so long as there is defeater $D_i$ that undermines one of the relatioship of evidential support. This picture seems to capture what intuitvely goes on at trial. However, note that while each individual claim $C_i$ could be mistaken in virtue of a defeater $D_i$, the conjuction $C_1 \wedge C_2 \wedge \dots \wedge C_k$ could be mistaken in virtue of a much larger numbere of defeater. So $C_1 \wedge C_2 \wedge \dots \wedge C_k$ is more suspectible to be provej wrong than any individual claim $C_i$. On other hand, 
$C_1 \wedge C_2 \wedge \dots \wedge C_k$ is suported by a much larger body of evidence $E_1 \wedge E_2 \wedge \dots \wedge E_k$ then each idnividual claim $C_i$. 

Now, for teh sake of argument, suppose legal probabilists reject the conjunction principle. How should they then define the standard of proof? They have two options. Each of these options is intuitively 
