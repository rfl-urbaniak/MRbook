...
cases in which individual Bayes factors are above one, and yet
distribution fails. That is, even when the joint Bayes factor is above a
given threshold, the individual Bayes factors may not meet the threshold
(Figure \ref{fig:BFdistr}). Likelihood ratios follow a similar pattern
...
ones (Figure \ref{tab:CPTconjunctionBNL}). Again, distribution fails in
a large class of cases as with the Bayes factor. Finally, we identify
cases in which the joint Bayes factor or likelihood ratio can be below
the individual ones even when these are greater than one. This can
happen when there is a direct dependence between the pieces of evidence
...
\label{fig:DAG1BF} \label{fig:BFind2} \label{fig:BFdistr}
