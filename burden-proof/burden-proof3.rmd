...
equals $\frac{\pr{a |A}}{\pr{a}} \times \frac{\pr{b |B}}{\pr{b}}$ (if A and B are independent) 
or $\frac{\pr{a |A}}{\pr{a}} \times \frac{\pr{b |B}}{\pr{b \vert a}}$ (if A and B are dependent).
Aggregation could not be justified using posterior probabilities $\pr{A | a}$ and $\pr{B | b}$. 
So it is an advantage of the Bayes factor that it can justify 
...
\todo{Not always true; just give a specific numerical counterexample. M: I added 'often'. Is this enough?} (which should not happen if $b$ positively supports $B$). So, curiously, there would be cases in which, even though the conjunction $A\et B$ is established to the desired standard of proof, the individual claims still fail to meet the standard. This is odd. More specifically, the following extrapolation principle fails in some cases:

\[\text{If S[$a \wedge b, A\wedge B$], then S[$a, A$] and S[$b, B$].} \tag{EXT1}\]

\noindent
This principle might not be as essential as we think. 
Since the evidence is not held constant, 
the support supplied by $a\wedge b$ could be stronger than 
...
But now consider a less controversial version of 
extrapolation. If the evidence is held constant, 
...
the standard of proof using the Bayes factor. The intuitive reason for this---perhaps surprising---result is that 
...
to a threshold $t$ above one. In criminal trials, for example, the rule of decision would be: Guilt is proven beyond a reasonable doubt if and only if the evidential support in favor of $G$---as measured by the likelihood ratio 
$\frac{\pr{E \vert  G}}{\pr{E \vert \neg G}}$---meets a suitably high threshold $t$ above one.
The greater the threshold, the more stringent the standard. Setting aside the problem of how ot identify 
...
called extrapolation. So the extrapolation 
