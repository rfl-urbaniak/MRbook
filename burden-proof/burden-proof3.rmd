...
in favor of $A$ and $B$? The combined support should be 
measured by the combined Bayes factor $\pr{a \wedge b| A\wedge B}/\pr{a \wedge b}$, and the latter,
under suitable independence assumptions, equals the product of the individual 
...
pieces of evidence whether or not $A$ and $A$ are probabilistically independent. This result can be generalized 
beyond two pieces of evidence and holds generally.\footnote{M: DOES THIS NEED A FORMAL PROOF OR IS GOOD ENOUGH?} 
...
\todo{Not always true; just give a specific numerical counterexample. M: I added 'often'. Is this enough?} (which should not happen if $b$ positively supports $B$). So, curiously, there would be cases in which, even though the conjunction $A\et B$ is established to the desired standard of proof, the individual claims still fail to meet the standard. This is odd. Call this the extrapolation paradox. 

We have seen that if the Bayes factor measures evidential suppport, the following 
extrapolation principle fails in a large class of cases:

\[\text{If S[$a \wedge b, A\wedge B$], then S[$a, A$] and S[$b, B$].} \tag{EXT1}\]

\noindent
But perhpas this principle is not as essential as we might think. 
Since the body of evidence is not held constant, 
the combined support supplied by $a\wedge b$ could be stronger than 
the support supplied by $a$ and $b$ individually. So even when the conjunction 
...
standard (given evidence $a$) nor does $B$ (given evidence $b$). But if the evidence is held constant, 
the extrapolation principle would become: 

 \[\text{If S[$a \wedge b, A\wedge B$], then S[$a \wedge b, A$] and S[$b \wedge b, B$].} \tag{EXT2}\]

\noindent
This principle is much harder to deny. That is, one would not want 
...
measure of evidential support. On this account, the standard of proof is again formalized 
as a certain threshold $t$ above one. In criminal trials, for example, the rule of decision would be: Guilt is proven beyond a reasonable doubt if and only if the evidential support in favor of $G$---as measured by the likelihood ratio 
$\frac{\pr{E \vert  G}}{\pr{E \vert \neg G}}$---meets a suitably high threshold $t$ above one.
The greater $t$ the more stringent the threshold. We will see that this approach is able to justify one direction of the conjunction principle---what we called aggregation---but still fails 
to justify the other direction---what we called extrapolation. So the extrapolation 
...
Consider aggregation first. Say both individual likelihood 
 ...
 threshold $t$ for meeting the standard of proof. Will the combined likelihood ratio $\frac{\pr{ a \et b |A \et B}}{\pr{a \et b | \neg (A \et B)}}$ also be above the threshold? The answre is affirmative. To see why, consider numerator and denominator separately. The numerator can be computed easily:
 \begin{align*}
...
\noindent
Given these assumptions, the graph of the combined likelihood ratio 
can now be easily plotted against the single likelihood ratios. As Figure \ref{fig:jointLRMarcello} shows, the combined likelihood  ratio varies depeding on the prior probabilities $\pr{A}$ and $\pr{B}$, but 
 it always execeeds the individual likelihood ratios so long as 
...
Likelihood ratios still fail to capture the other 
direction of the conjunction principle, what 
we called extrapolation. For suppose evidence $a \et b$ supports $A \et B$ to 
...
\textbf{SEE PROOF IN EARLIER CHAPTERS.}
 ...
 $S[a\wedge b, A]=\frac{\pr{a \vert A}}{\pr{a \vert \neg A}}=S[a, A]$. Since $S[a, A]< S[a\wedge b, A\wedge B]$ (see Figure \ref{fig:jointLRMarcello}), it follows $S[a\wedge b, A]<S[a\wedge b, A\wedge B]$} So likelihood ratios fail to justify 
 (EXT2). Even this second, seemingly unobjectionable version of extrapolation fails. The same counterintuitive consequences that arose with Bayes factor manifest themselves here. Even with likelihood ratios 
 instead the Bayes factor, the extrapolation paradox persists. 
...
Instead of thinking in terms of absolute thresholds---whether threshold on posterior prbabilities, teh Baye factor or the likelihood ratio---the standard of proof can be understood comparatively [@cheng2012reconceptualizing]. 
...
On this approach, rather than directly evaluating the posterior probability or the evidential 
...
Let's first work out this comparative strategy 
