...
The trouble----at least for the legal probabilist---is that the conjunction principle conflicts 
with the threshold-based probabilistic interpretation 
...
How could legal probabilists respond? The conjunction paradox is a difficult problem, as the vast literature on the topic attests. Before we offer what we take to be the most promising answer, we will explore other probability-based solutions, and understand why these solutions, albeit promising, ultimately fail. This is what we set out to do in the following sections. 


## A closer look at the conjunction principle


The conjunction paradox arises because of the conjunction principle. Could legal probabilists reject 
...
--> 
In current discussions in epistemology, this principle has been contested because 
it appears to deny the fact that risks accumulate (CITE). \todo{Hey, we can quote a paper that's out by Alicja!:) Also, I guess you want me to find the right refs? M: Yes, if you can} If one is justifiably sure about the truth of 
...
How should they define standards of proof? The legal probabilist has two immediate options. Neither of them is without problems.  One option stipulates that, in order to establish the defendant's guilt beyond a reasonable doubt, the party making the accusation should establish each claim, separately, to the requisite probability, say at least 95\%, without establishing the conjunction to the requisite probability. Call this the \textit{atomistic account}.  On this view, the prosecution could be in a position to establish guilt beyond a reasonable doubt without estalishing guilt---that is, the conjunction of different claims---with a sufficiently high probability. This account would allow convictions in cases in which the probability of the defendant's guilt, call it $G$, is low, just because $G$ is a conjunction of several independent claims that separately satisfy the standard of proof. This move runs counter to legal probabilism, since it would allow convictions when the defendant is most likely innocent.  
...
difficulties of its own. Legal probabilists should 
...
constitute the allegation of wrongdoing in a criminal or civil trial. 
This is, however, a simplification. It is instructive to 
distinguish the evidence and the hypotheses that the evidence support. 

Say evidence $a$ supports $A$ and evidence 
$b$ supports $B$. In an aggraveted assault case, evidence $a$ could be a witness testimony that the defendant 
physically harmed the victim (claim $A$), and $b$ evidence that the defendant 
knew that the victim was a firefighter (claim $B$), for example, another testimony that the 
defendant earlier called the firefighther for help. The question now is whether the combination of $a$ and $b$ support 
the conjunction $A \et B$ and viceversa. 
...
differs from the earlier one in that it is formulated 
...
not always hold, but they do sometimes. Consider our aggravated 
assault example.  Presumably, $\pr{A \vert a}=\pr{A \vert a \wedge b}$
...
still arises in a number of circumstamces.  Suppose evidence $a\et b$ establishes claim $A$ and also claim $B$, separately, right above above the probability threshold $t$. Since $\pr{A \wedge  B | a \wedge b} =\pr{A |a \wedge b} \times \pr{B | a \wedge b \wedge A}$, it follows that $\pr{A \wedge  B | a \wedge b}$ would be below $t$ so long as $\pr{B | a \wedge b \wedge A}$ is below 100\%, which would often be the case since evidence is fallible. So even though $A$ and $B$ were established to the required probability, the conjunction is not. 
...
while keeping this in mind. In fact, when confronted with the difficulty about conjunction, 
...
Not quite. As we will now show, the difficulty about conjuntion will not go away even 
...
s_{AB}& =  s_{A}\times s_{B} 
 ...
 \noindent 
The only controversial step is marked by an asterisk. These independence assumptions nedded for this step 
are codified in the Bayesian network in Figure \ref{network-conjunction}. \todo{more work on explaining independence in BNs is needed at some point, not sure if here. } \todo{How about without independence? Can we find a counterexample?}
...
The structure of the nextwork in Figure \ref{network-conjunction}
is extremely natural except for the incoming 
arrows into node $A \wedge B$. An alternative network, and equally plausible network, is depicted in Figure
\ref{network-conjunction-alternative}. The arrows are outgoing from 
node $A \wedge B$ rather than incoming. Given this other network, 
the following holds:
...
s_{AB}& =  s_{A}\times s^{*}_{B} 
 ...
 \noindent 
Presumably, $s^{*}_{B}$ is lower than $s_{B}$ because
$\pr{b | a} > \pr{b}$ (assuming $a$ and $b$ are convergent pieces 
of evidence) but is till grater than one. If that is the case, the combined support $s_{AB}$ would still execeed the individual support of the individual piece of evidence. 
...
the difficulty about conjunction now evaporate, as Dawid seems to think? To some extent 
 the paradox does go away. The paradox was about whether the conjunction satisfies a proof standard just in case so do its conjuncts. The Bayes factor strategy will work against this challenge  only if the standard of proof is no longer formalized as a probability threshold, but rather, as a threshold about evidential support. That is, for this to work, the threshold should no longer apply to quantities such as
...
There is an obvious question this approach poses. How do we identify the suitably high threshold $t$?\todo{good question, will think about it, will need to take a look at "Bayesian Choice", also need to think about a counterexample} \footnote{QUESTION: CAN DECISION THEORY BE APPLIED TO THE BAYESIAN FACTOR OR IS IT ALWAYS ABOUT POSTERIOR PROBABILITIES? IF THERE IS NO DECISION THEORY APPLICABLE HERE, THAT WOULD BE A FURTHER DIFFICULTY.} We will not discuss this question for now. As it turns out, even if this question can be satisfactorily answered, this  approach gives rise to a complication that proves fatal. At issue here is 
...
the threshold. This implies that the individual support $\frac{\pr{a |A}}{\pr{a}}$ (and the same applies to $\frac{\pr{b |B}}{\pr{b}}$) would be below the threshold unless $\frac{\pr{b |B}}{\pr{b}}=1$ \todo{Not always true; just give a specific numerical counterexample} (which should not happen if $b$ positively supports $B$). So, curiously, even though the conjunction $A\et B$ would be established 
...
measure of evidential support. The conjunction paradox redux arises again. Say both individual likelihood 
 ...
 threshold $t$ for meeting the standard of proof. How does the combined likelihood ratio $\frac{\pr{ a \et b |A \et B}}{\pr{a \et b | \neg (A \et B)}}$ compare? 
 
 The numerator can be computed easily:\todo{Again, you are a bit hasty with your independence assumptions.} 
 \begin{align*}
...
& = \pr{a | A} \times \pr{b | B} 
 ...
 \noindent
 The equality requires the indepedence assumptions invoked earlier. The asterisk marks the step that requires the indepedence assumptions.\todo{This is misleading; all further steps based on this step also require the intependence assumptions, to be fair.} Let $\pr{a |A}=x$ and $\pr{b |B}=y$, so the numerator simply equals $xy$. 
...
invoked before are needed here again,\todo{I am not convinced, we should be more careful here.} marked by the asterisk in the derivation. For simplicity, \todo{You're a bit repetitive with the introduction of $x$ and $y$ here.} assume that $\pr{a |A}=x$ and $\pr{a |\neg A}=1-x$ \todo{But why?? These would be substantial, non-definitional assumptions! Ok, I stop reading here, we need to discuss this before we even get into the further details.} and also
$\pr{b |B}=y$ and $\pr{b | \neg B}=1-y$. Let $\pr{A}=k$ and $\pr{b}=t$.\todo{You meant P(B) here?} Thus,
  \begin{align*}
...
\noindent
Let's make a further simplification  assuming $x=y$, and plot individual and  combined likelihood ratios against $x$.  \eqref{eq:combinedLRMarcello} reduces to:
  \begin{align*}
...
The graph of the combined likelihood ratio 
 ...
 $y/(1-y)$ for $\frac{\pr{b |B}}{\pr{b | \neg B}}$.\todo{Make clear the assumption that sensitivity equals specificity here is relied on.} The combined ratio 
