...
in Figure \ref{fig:conjunctionDAG2}? Well, the claim still holds, with weaker independence assumptions in the background. \ali{double check, are these d-sep in the second network?}
...
Given the second Bayesian network, 
...
supports $B$ even conditional on  $a$.\ali{check if d-sep in this fn holds in the second bn}\todo{checked and rewrote the footnote}

Note that 
$\frac{\pr{b \vert B}}{\pr{b\vert a}}=\frac{\pr{b \vert B \wedge a}}{\pr{b\vert a}}$, by independence \eqref{eq:I6}. Now, observe that by Bayes' theorem 
...
\noindent  \eqref{eq:jointLR2} is easily obtained from \eqref{eq:jointLR}  by  \label{eq:indAB} and  \label{eq:indAB2}. 
\textbf{Argument ends.}
...
This also entails that any likelihood ratio threshold decision standard still satisfies aggregation. After all, it both individual LRs are above a threshold, then so will be the composite LR.
