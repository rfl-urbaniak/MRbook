...
If the standard of proof is formalized using a fixed  threshold, the conjunction principle  would boil down to one of these:
...
But (DS2)  is less controversial, as it  holds the evidence constant. This principle is harder to deny: one would not want 
...
(DIS2).  Yet, (DIS2) fails both for  Bayes factor and likelihood ratio. After all, if  $A$ and $B$ are probabilistically independent, (DIS1) and (DIS2) are in fact equivalent, so the counterexamples to (DIS1) work also against (DS2).
