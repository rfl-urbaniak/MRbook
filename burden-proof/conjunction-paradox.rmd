...
The revised version of the conjunction principle---at least in this case---conflicts with a threshold-based interpretation of standards of proof. The difficulty with conjunction persists.  

There are some loose ends in this discussion. Why should one subscribe to the independence assumptions in the aggravated assault case?  Further, do these assumptions hold in other cases? The argument should be made both more rigorous and more general. To this end, we will represent formally the relationship between claims $A$, $B$ and their conjunction $A\wedge B$, as well as the supporting evidence $a$, $b$ and their conjunction $a\wedge b$. A natural way to do that is to use Bayesian networks. 
...
With this set-up in place, we can see that the conjunction paradox arises here. By the theory of Bayesian networks, the directed graph in Figure \ref{tab:CPTconjunction}
...
Does the difficulty with conjunction arise even without the independence of hypotheses $A$ and $B$? It does in a number of circumstances.  \raf{M: Can you check the derivation here?} Suppose evidence $a\et b$ establishes claim $A$ and also claim $B$, separately, right above the probability threshold $t$. The graph in Figure \ref{fig:conjunctionDAG2} ensures that:^[ \begin{align*}
\pr{A \wedge  B \vert a \wedge b}& =\pr{A \vert a \wedge b} \times \pr{B \vert  a \wedge b \wedge A}\\
 & = \pr{A \vert a} \pr{B \vert  b \wedge A}
 \end{align*}]
 \begin{align*}
...
<!---
Since $\pr{A \wedge  B \vert  a \wedge b} =\pr{A \vert a \wedge b} \times$

$\pr{B \vert a \wedge b \wedge A}$, it follows that $\pr{A \wedge  B \vert a \wedge b}$ might be below $t$ if the factors on the right-hand side are below 1 and separate posteriors are very close to $t$.
--->

## Indepedencies more generally 

In what follows, we will refer to the directed graphs in Figure \ref{fig:conjunctionDAG}
 and \ref{fig:conjunctionDAG2}. They provide a compact representation of two plausible scenarios  one might have in mind in formulating the conjunction paradox. In one scenario, hypotheses $A$ and $B$, as well as items of evidence $a$ and $b$, are unconditionally independent. In another scenario, $A$ and $b$ are no longer independent, and $a$ and $b$ are only conditionally independent. In fact, the two directed graphs encode several relationships of probabilistic independence which it would be hard to derive without the theory of Bayesian networks. We list some of them below as we will appeal to them in the rest of the chapter. The fist graoh encodes all the relationship below and the second graph a subset of them (those marked by *). The symbols $\indep$ stands for independence. 
