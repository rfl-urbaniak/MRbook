...
There is some disagreement among legal scholars about the tenability of conjunction principle (\textbf{CITE}). \mar{R: Dunno, I prefer to think of this as a way of validating it! After all, it holds once you accept the narration, no?} We will ultimately reject this principle. For the time being, however, let us assume the principle correctly captures features of the standard of proof.  The principle has some degree of plausibility and is consistent with the case law. For example, the United States Supreme Court writes 
...
who also endorses the conjunction principle is in fact committed to a contradiction in many clearly admissible scenarios.
...
the $.95$ threshold unless $\pr{A \vert B}=1$. For example, that someone premeditated a harmful act against another (call it \textsf{Premed}) makes it more likely that they did cause harm in the end (call it \textit{Harm}). Since $\pr{\mathsf{Harm} \vert \textsf{Premed}} > \pr{\textsf{Harm}}$, the two claims are not independent. Still, premeditation does not always lead to harm, so $\pr{\textsf{Harm} \vert \textsf{Premed}}$ will often be below $1$. If both claims are established with $.95$,
...
The discussion so far proceeded without mentioning the evidence in support of the claims that constitute the wrongdoing. This a simplification. To be more precise, the conjunction paradox and the conjunction principle must be formulated by including the supporting evidence. For two claims, the conjunction principle can be formulated as follows:
...
To fix ideas, consider an aggravated assault case. The prosecution should establish, first, that the defendant physically injured the victim, and second, that the defendant knew he was interacting with a\ali{R: fix these representations too, convert all abbreviations to lowercase throughout the chapter.} firefighter. Let \textit{Witness} denote a witness testimony supporting the claim that the defendant physically injured the victim, call this claim \textit{Injury}. Let \textit{Call} be evidence that the defendant made a call to an emergency number. This evidence supports the claim that the defendant knew he was dealing with a firefighter, call this claim \textit{Firefighther}. If $\pr{\textit{Injury} \vert \textit{Witness}}$ and $\pr{\textit{Firefighther} \vert \textit{Call} }$ both meet the required probability threshold, does $\pr{\textit{Injury} \wedge \textit{Firefighther} \vert \textit{Witness} \wedge \textit{Call}}$ also meet the threshold? 
...
This  violates  aggregation, and so  the difficulty with conjunction persists.  
...
Does the difficulty with conjunction arise even without the independence of hypotheses $A$ and $B$? It does in a number of circumstances.  Suppose evidence $a\et b$ establishes claim $A$ and also claim $B$, separately, right above the probability threshold $t$. The graph in Figure \ref{fig:conjunctionDAG2} ensures that:\mar{R: looks fin, although moving from P(a \& b|A\& B) to the numerator in the second line requires a bit of comment in a gloss after the derivation.}
 \begin{align*}
...
\noindent Note two things. First, $\pr{B \vert A}$ is the prior probability of  $B$ conditional  on $A$. 
This conditional prior probability  may be greater or lower than the unconditional probability of $B$. 
However, examples of probabilistic dependence hypotheses are usually such that one hypothesis makes the other more likely than it would be otherwise. \mar{R: I don't think this is true, and I don't think we should rely on this assumption assertively.} So typically $\pr{B \vert A} > \pr{B}$.  Second, 
...
\mar{R: no. Because it is not generally the case. But for us it is enough it might happen quite often; I think we need the simulations here to talk about frequency.}

 \raf{M: see comment in boldface. Your brief argument commented out was not comprehensible to me.}

\raf{M: let me try it like this.}

For this to be less than $\pr{A\vert a}$, it is enough that  $\frac{\pr{b \vert B}}{\pr{b\vert a}} \pr{B \vert A}$, or equivalently, that:
\begin{align*}
\frac{\pr{b \vert B}}{\pr{b \vert a}}   &<  \frac{1}{\pr{B\vert A}}
\end{align*}
\noindent This inequality can come out true for a range of probabilities. For instance, if $\pr{b\vert B}=.9$, $\pr{b\vert a} = \pr{B \vert A} = .55$, the left-hand side is $\approx 1.63$ and the right-hand side is $\approx 1.81$.  \mar{R: we still need the simulation here}
...
Hence, $s^{'}_{AB}$ should be greater than either $s_A$   if   $s^{'}_B$ is  
...
\end{align*}
So, symmetrically, we have $s^{'}_{AB} =  s_{B}\times s^{'}_{A}$, and so   $s^{'}_{AB}$ should be greater than either $s_B$   if   $s^{'}_A$ is greater than one.  
