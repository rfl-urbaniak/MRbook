...
\noindent Suppose the prosecution presents evidence \(E\) that
establishes claims \(A\) and \(B\), separately, to the required
probability, say at least 95\%. That is, \(\pr{A \vert E}\geq 95\%\) and
...
Say a piece of evidence \(E_1\) (defeasibly) supports hypothesis \(H_1\)
and \(E_2\) (defeasibly) supports \(H_2\). To illustrate, \(E_1\) could
be evidence that the defendant caused harm to the victim (\(H_1\)), and
\(E_2\) evidence that the defendant did so intentionally (\(H_2\)), for
example, a recorded phone conversation shows that the defendant planned
the attakk against the victim several days in advance. The conjuction
principle for evidence \(E_1\) and \(E_2\), hypotheses \(H_1\) and
\(H_2\), and standard of proof \(S\) can be formulated, as follows:
