...
prosecution has to establish both that the defendant caused harm to the
victim (call it claim \(A\)) and that the harmful act was intentional
(call it claim \(B\)). Cohen (1977) argues (p.~66) that common law
systems subscribe to a conjunction principle, which assumes that if
\(A\) and \(B\) are established according to the governing standard of
proof, so is their conjunction, and vice versa. If the conjunction
principle holds, the following statements must be equivalent, with \(S\)
a generic standard of proof:
...
\noindent The conjunction principle is supported by the case law. For
example, the United States Supreme Court makes it clear that guilt must
be proven beyond a reasonable doubt and also makes clear that
...
\noindent So, to establish guilt beyond a reasonable doubt simply means
to establish each element of the crime according to the requisite
standard. This is an application of the conjunction principle in
criminal proceedings.

The trouble is that the conjunction principle is false given a
probabilistic intepretation of\\
...
probabiliity, say at least 95\%. In other words,
\(\pr{A \vert E}\geq 95\%\) and \(\pr{B \vert E}\geq 95\%\). Has the
prosecution met the burden of proof? Since each claim was established to
the requisite threshold, guilt should also be established according to
the requisite standard, assuming the conjuction principle holds. But
even though each claim necessary to sustain the accusation was
established according to the requisite probability, the probability of
their conjunction -- assuming the two claims are indepedent -- is only
\(95\%\times95\%=90.25\%\), below the required 95\% threshold. So guilt
was not established according to the requisite standard. It remain
unclear whether the prosecution has met the burden of proof or not.

The difficulty about conjunction -- the fact that the conjunction
principle is false assuming a probabilistic interetation of the standard
of proof -- was originally formulated by (Cohen, 1977) and has enjoyed
great popularity every since ({\textbf{???}}-; Allen, 1986; Allen \&
Stein, 2013; Allen \& Pardo, 2019; Haack, 2014; Schwartz \& Sober,
2017).

The difficulty about conjuction does not subside if the number of
constituent claims increases. If anything the difficulty becomes more
apparent. If the prosecution has established three separate claims to
95\% probability, their probability -- again if the claims are
independent-- would be about 85\%, even further below the 95\%
threshold. Nor does the difficulty about conjunction subsides if the
claims are no longer regarded as independent.
...
pages grows, the risk you have missed a mistake increases. It becomes
...
principle, insted, would have us deny the accumulation of risk.

The conjunction principle, however, is not so clearly implausible. In
trial proceedings the possibility of error need not become certain a s
the number of claims under consideration increases. Say there is a piece
of evidence \(E_1\) that defeasibly suports a claim \(C_1\); \(E_2\)
defeasibly supports claim \(C_2\); and so. Each of these relationships
of evidential support could be defeated by defeaters \(D_1\), \(D_2\),
and so on. Now suppose the pieces of evidence are combined toghether
...
there is no defeater \(D_i\) that undermines one of the relatioships of
evidential support, the combined evidence
\(E_1 \wedge E_2 \wedge \dots \wedge E_k\) supports the conjunction
\(C_1 \wedge C_2 \wedge \dots wedge C_k\). On this view, the conjunction
principle holds. Note, however, the difference between each individual
claim \(C_i\) and the conjunction
\(C_1 \wedge C_2 \wedge \dots wedge C_k\). While each claim \(C_i\)
could be mistaken in virtue of a defeater \(D_i\), the conjunction
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) could be mistaken in virtue
of a larger set of defeaters, \(D_1\), \(D_2\), \dots, \(D_k\). So
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) is more susceptible to be
proven wrong than any individual claim \(C_i\). On other hand,
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) is suported by a larger body
of evidence \(E_1 \wedge E_2 \wedge \dots \wedge E_k\) than each
idnividual claim \(C_i\).

All in all, the conjunction principle is not obviously implausible. But,
for the sake of argument, suppose the legal probabilist rejects the
conjunction principle. How should they then define standards of proof?
They have two options. Each of these options is intuitively
unattractive. One option is to require that the prosecution in a
...
separately, to the requisite probability. Call this the
\textit{atomistic account}.

Going with the atomistic account seems a non-starter. On this account,
the prosecution could establish guilt beyond a reasonable doubt without
having estalished guilt with a suffuciently high probability. The
atomistic approach would allow convictions in cases where
\(\pr{G\vert E}\) is quite low, just because \(G\) is a conjunction of
several independent claims that separately satisfy the standard of
proof. This is counterintuitive, and even more clearly against the
spirit of legal probabilism.

Going with the holistic approach is not without problems either. The
proof of \(A\et B\) would impose an importantly higher requirement on
the separate probabilities of the conjuncts. If the conjunction
\(A\et B\) has to be proven with at least 95\% probability, the
individualluy conjunction should be established with probability higher
than the 95\% threshold. This also means that the more conjunctt should
be proven with at least 95\%, the highere their proibability should be.
...
treatment requirement mentioned already in the introduction. So the
holistic option will put seemingly unnecessarily high requirements
sensitive to fairly contingent and irrelevant facts on the prosecution,
and treat various elements to be proven unevenly. Neither option seems
desirable.
...
interetation rather than a interpretation.
