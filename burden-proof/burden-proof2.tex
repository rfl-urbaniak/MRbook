...
of mistaken decisions, but leaves out the benefits associated with
correct decisions. More comprehensive analyses should consider both,
even though the basic insight would remain the same to the extent that
trial decision-making is viewed as one instrument among others for
maximizing overall social welfare (Dekay, 1996; Laudan, 2016; Posner,
1973). But considering the costs of incorrect decisions as well as the
benefits of correct decisions adds nuance to the analysis.

For simplicity, we will quantify costs and benefits by units of utility
(abbreviated \(\ut(...)\)), where benefits will correspond to positive
numbers and costs to negative numbers. In a criminal trial, the
following costs should be weighed: \(\ut(CI)\) and\(\ut(AG)\), as well
as the following benefits: \(\ut(CG)\) and \(\ut(AI)\). Given this
set-up, a conviction is justified provided its expected utility---that
is, \(\pr{G | E} \cdot \ut(CG) + \pr{I | E } \cdot \ut(CI)\)---exceeds
the expected utility of an acquittal---that is,
...
of utility (9 units of utility are gained), while the cost is -9 units
of utility (9 units of utility are lost). Similarly, suppose the
magnitude of the benefits resulting from convicting a guilty defendant
is the same as the magnitude of the cost resulting from acquitting a
guilty defendant. Say the benefit is the acquisition of 1 unit of
utility, while the costs is the loss of 1 unit of utility. Then, once
again, the threshold would be \(\frac{1}{(1+(1+1)/(9+9)}=0.9\), as
before.
...
members (-1); putting other citizens at risk of victimization by failing
to convict the actual perpetrator (-1). This is a total utility loss of
-5. What about the correct acquittal of an innocent? There woud be no
...
could still victimize others (-1). The innocent defendant who is
acquitted could still experience damaged reputation and other harms, but
we shall leave this possibility aside. All in all, trying an innocent
defendant, no mater the final decision, carries the baseline costs of
failing to identify the true perpetrator and thus putting other citizens
at risk of victimization. This baseline cost increases when an innocent
person is wrongly convicted. So, by adding everything up,
\(\ut(AI)-\ut(CI)=-1+5=+4\).

Consider the loss from acquitting a guilty defendant. This will include
...
trial system (-1). There would be no damaged reputation (0); no loss of
income (0); no severance from family members (0). What about the
conviction of a guilty defendant? The defendant would still experience
damaged reputation (-1); loss of income (-1); severance from family
members (-1). However, citizens would enjoy a lower risk of
victimization (+1) and the deterrence function of the trial system would
be reaffirmed (+1). So, all things considered,
\(\ut(CG)-\ut(AG)=-1+2=+1\).

So, assuming the utilities are correctly assigned, the threshold for a
criminal conviction should be \(\frac{1}{1+\frac{1}{4}}=0.8\), somewhat
lower than the more common 0.9 or 0.95 threshold.

But tthe threshold can be made more stronger or even less stringent
depending on how utilities are assigned. Say one is very concerned about
risk of victimization for different reasons, such as more conservative
political ideas or havign grown up in a high crime area. Say the costs
resulting from the risk of victimization resulting from a false
acquittal or the trial of an innoocent is increased from -1 to -2. Then,
\(\ut(AI)-\ut(CI)=-1+5=+4\)

COMPLETE THIS PART WHEN YOU PLAY AROUND WITH THE COSTS OF VICTIMIZATION
AND TEH COSTS OF CONVICTION, BY USING MORE CONSERVATIVE OR MORE
PROGRESSIVE ASSIGNMENTS, AND SEE HOW THE THRESHOLD CHANGES BASED ON
THAT.
