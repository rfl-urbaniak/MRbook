...
\hypertarget{conjuction-paradox-new-chapter-here}{%
\subsection{Conjuction paradox -- NEW CHAPTER
HERE}\label{conjuction-paradox-new-chapter-here}}
...
But suppose the legal probabilist does away with conjunction principle.
...
\(H\) is the Bayes factor \(\pr{E | H}/\pr{E}\). If the Bayesian factor
is greater than 1, \(E\) positively supports \(H\). The greater the
Bayes factor (for values above 1), the stronger the support of \(E\) in
favor of \(H\). The Bayes factor \(\pr{E | H}/\pr{E}\), unlike the
conditional probability \(\pr{H | E}\), offers a way to overcome the
difficulty about conjuction. {[}MAYBE SAY THAT THE BAYES FACTOR IS NOT
THE PREFERRED MEASURE OF EVIDENTIAL VALUE FOR REASONS EXPLAINED IN
EARLIER CHAPTERS.{]}

Say \(a\) supports \(A\) and \(b\) supports \(B\), to degree \(s_A\) and
\(s_B\) greater than 1, that is, \(\pr{a | A}/\pr{a}=s_A\) and
\(\pr{b | B}/\pr{b}=s_B\), where \(s_A, s_B > 1\). Does \(a \wedge b\)
...
\noindent The only controversial step is marked by asteriks. This step
...
as \(s_{A}\) and \(s_{B}\) are greater than one. Under suitable
...
conditional probability \(\pr{H | E}\), but in terms of the Bayes
factor. For this solution to work, however, the standard of proof itself
should no longer be formalized as a probability threshold, but rather,
as a threshold about evidential support. More specifically, the
threhsold would no longer apply to the quantity \(\pr{G | E}\) or
\(\pr{L | E}\), but rather to the quantity
\(\frac{\pr{E_i | C_i}}{\pr{E_i}}\), for each claim \(C_i\) that the
prosecutor or plaintiff should establish according to the governing
standard of proof. QUESTION: CAN DECISION THEORY BE APPLIED TO THE
BAYESIAN FACTOR OR IS IT ALWAYS ABOUT POSTERIOR PROBABILITIES? IF THERE
IS NO DECISION THEORY APPLICABLE HERE, THAT WOULD BE A FURTHER
DIFFICULTY.

Before we go further, note that the same result can be obtained using
the likelihood ratio, although calculations are not as straightfoward as
with thE Bayes factor. Say both individual likelihoo ratios
\(\frac{\pr{a |A}}{\pr{a | \neg A}}\) and
\(\frac{\pr{b |B}}{\pr{b | \neg B}}\) are above one. How does the
combined likelihoo ratio
\(\frac{\pr{ a \et b |A \et B}}{\pr{a \et b | \neg (A \et B)}}\)
compared? The numerator
\(\pr{ a \et b |A \et B}=\pr{ a|A}\times \pr{ b |B}\). This equality
requires the indepedence assumptions invoked earlier (EXPLAIN). Assuming
\(\pr{a |A}=\pr{b |B}=x\), the numerator is simply \(x^2\).
...
\noindent Again,the assumption is that \(\pr{a |A}=\pr{b |B}=x\) and
...
always greater than the single likelihood ratio. THIS WILL NEED TO BE
DRAWN WITH R PROPERLY. JUST TO GIVE THE GENERAL IDEA FOR NOW. IS THIS
CORRECT BY THE WAY?
