...
\item
  A chapter that looks at the two theoretical difficulties (naked stats
  and conjunction paradox, ad also problem of priors) and examines
  whether likelihood ratios are a good way to address these
  difficulties. This follows the earlier chapter. The point of this
  chapter is: yes, LR are promising but not as good we might have
  thought initially, so we need something better. Title: ``Likelihood
  Ratios as Decision Thresholds''
\item
  A chapter that finally addresses the two theoretical difficulties,
...
should address is the difficulty about conjunction. The difficulty about
conjunction was originally formulated by (Cohen, 1977) and has enjoyed a
great deal of scholarly attention every since (Allen, 1986; Allen \&
Stein, 2013; Allen \& Pardo, 2019; Haack, 2014; Schwartz \& Sober, 2017;
Stein, 2005). This difficulty arises when an accusation of wrongdoing,
in a civil or criminal proceeding,
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD is broken down into its constituent elements. Suppose that in order
to prevail the prosecution should establish by the required standard,
first, that the defendant caused harm to the victim (call it claim
\(A\)), and second, that the harmful act was performed with
premeditation (call it claim \(B\)). Cohen (1977) argues that common law
systems subscribe to a conjunction principle, that is, if \(A\) and
\(B\) are established according to the governing standard of proof, so
is their conjunction (and vice versa). If the conjunction principle
holds, the following must be equivalent, where \(S\) is a placeholder
for the standard of proof: ======= is broken down into its constituent
elements. Suppose the prosecution has to establish that the defendant
caused harm to the victim (call it claim \(A\)) and that the harmful act
was intentional (call it claim \(B\)). Cohen (1977) argues that common
law systems subscribe to a conjunction principle, that is, if \(A\) and
\(B\) are established according to the governing standard of proof, so
is their conjunction (and vice versa). If the conjunction principle
holds, the following statements must be equivalent, with \(S\) a generic
standard of proof:
\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}
bd6a36a0bd93cc0ca6c9c5270a22268aa6ff6c96
...
\textbf{Overall}  &   [A and B] are established according to S  \\ 
\bottomrule
\end{tabular}
\end{center}

\noindent In other words, the conjunction principles requires that
\[S[H_1 \wedge H_2 \wedge \dots \wedge H_k] \text{ iff } S[H_1] \wedge S[H_2] \wedge \dots \wedge S[H_k],\]

\noindent
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD where \(S[H]\) means that claim or hypothesis \(H\) is established
according to standard \(S\). The conjunction principle seems consistent
with---perhaps even required by---the case law. For example, the United
States Supreme Court writes that in criminal cases ======= The
conjunction principle is consistent wih the case law. For example, the
United States Supreme Court makes it clear that
\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}
bd6a36a0bd93cc0ca6c9c5270a22268aa6ff6c96
...
\noindent
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD A plausible way to interpret this quotation is to posit the
following identity: to establish beyond a reasonable doubt that someone
is guilty of a crime \textit{just is} to establish each element of the
crime beyond a reasonable doubt. More formally,

\[BARD[H_1 \wedge H_2 \wedge \dots \wedge H_k] \text{ iff } BARD[H_1] \wedge BARD[H_2] \wedge \dots \wedge BARD[H_k],\]

\noindent where the conjuction
\(H_1 \wedge H_2 \wedge \dots \wedge H_k\) comprises all the material
facts that, according to the applicable law, constitute the crime with
with the accused is charged.

The trouble is that the conjunction principle is false given a
probabilistic interpretation of the standard of proof. After all, the
following statements may come apart: ======= A plausible way to
interpret the passage above is to assume that to establish guilt beyond
a reasonable doubt means to establish each element of the crime
according to the requisite standard. This is an application of the
conjunction principle.

The trouble is that the conjunction principle is false given a
probabilistic intepretation of the standards of proof, because the
following two statements may come apart:
\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}
bd6a36a0bd93cc0ca6c9c5270a22268aa6ff6c96
...
{@{}ll@{}}
\toprule
\textbf{Seperate} &    $\pr{A\vert E} \geq 0.95$ and $\pr{B\vert E} \geq 95$\\   
\textbf{Overall}  &   $\pr{B\et B\vert E}\geq  0.95$ \\ 
\bottomrule
\end{tabular}
\end{center}
...
\noindent
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD Suppose the prosecution presents evidence \(E\) that establishes
claims \(A\) and \(B\), separately, to the required probability, say at
least 95\%. That is, \(\pr{A \vert E}\geq 95\%\) and
\(\pr{B \vert E}\geq 95\%\). Has the prosecution met the burden of
proof? Each claim was established to the requisite threshold, and thus
it was established to the requisite standard (assuming a probabilistic
interpretation of the standard). Further, if each claim was established
to the requisite standard, then guilt as a whole was established to the
requisite standard (assuming the conjunction principle). But even though
each claim was established to the requisite probability, the probability
of their conjunction -- assuming the two claims are independent -- is
only \(95\%\times95\%=90.25\%\), below the required 95\% threshold. So
guilt as a whole was \textit{not} established to the requisite standard
(assuming a probabilistic interpretation). This is a contradiction. It
is as though the prosecution has met and not met the burden of proof.
...
claims increases. If anything, the difficulty becomes even more
apparent. Say the prosecution has established three separate claims to
95\% probability, the probability of their conjunction -- again if the
claims are independent-- would be about 85\%, even further below the
95\% threshold. Nor does the difficulty about conjunction subsides if
the claims are no longer regarded as independent. MARCELLO: CAN WE SAY
WHY? HOW DO WE EXPLAIN THIS?

Legal probabilists could reject the conjunction principle outright.
After all, the conjunction principle seems to deny the fact that risks
...
conjunction. Say you have checked each page of a book and found no
error. So, for each page, you are sure there is no error. Having checked
each page and found no error, can you, on this basis, be sure that the
book as a whole contains no error? Not really. As the number of pages
grows, the risk that you have overlooked an error increases. It becomes
virtually certain that there is at least one error in the book as a
whole, although for each page you are justifiably sure there is no
error. The same applies to other contexts, including product quality
control. You may be sure, for each product you checked, that it is free
from defects. But you cannot, on this basis alone, be sure that all
products you checked are free from defects. In fact, since the risk of
error accumulates, you must have missed at least one defective product.

Even if the conjunction principle may be problematic in its own way,
denying it does not make life any easier for the legal probabilist.
Absent the conjunction principle, how should they define standards of
proof? The legal probabilist has two options. Neither of them is without
...
the \textit{atomistic account}. On this account, the prosecution could
establish guilt beyond a reasonable doubt without having estalished
guilt with a sufficiently high probability. The atomistic approach would
allow convictions in cases where \(\pr{G\vert E}\) is quite low, just
because \(G\) is a conjunction of several independent claims that
separately satisfy the standard of proof. This runs counter to legal
probabilism since it would allow convictions when the defendant is most
likely innocent. So the atomistic account is a non-starter. =======
Suppose the prosecution manages to present evidence \(E\) that
establishes claims \(A\) and \(B\), separately, to the required
probabiliity, say at least 95\%. That is, \(\pr{A \vert E}\geq 95\%\)
and \(\pr{B \vert E}\geq 95\%\). Has the prosecution met the burden of
proof? Since each claim was established to the requisite threshold,
guilt should also be established according to the requisite standard,
assuming the conjuction principle holds. But even though each claim
necessary to sustain the accusation was established according to the
requisite probability, the probability of their conjunction -- assuming
the two claims are independent -- is only \(95\%\times95\%=90.25\%\),
below the required 95\% threshold. So guilt was not established
according to the requisite standard. It remain unclear whether the
prosecution has met the burden of proof or not.

The difficulty about conjunction -- the fact that the conjunction
principle is false assuming a probabilistic interpretation of the
standard of proof -- was originally formulated by (Cohen, 1977) and has
enjoyed great popularity every since ({\textbf{???}}-; Allen, 1986;
Allen \& Stein, 2013; Allen \& Pardo, 2019; Haack, 2014; Schwartz \&
Sober, 2017).

The difficulty about conjunction does not subside if the number of
constituent claims increases. If anything, the difficulty becomes even
more apparent. Say the prosecution has established three separate claims
to 95\% probability, the probability of their conjunction -- again if
the claims are independent-- would be about 85\%, even further below the
95\% threshold. Nor does the difficulty about conjunction subsides if
the claims are no longer regarded as independent.
\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}\textgreater{}
bd6a36a0bd93cc0ca6c9c5270a22268aa6ff6c96
...
been achieved for the other conjuncts. If \(\pr{A\vert E}=0.8\), then
\(\pr{B\vert E}\) must be at least \(0.635\) so that
\(\pr{A\et B\vert E}\) is above a 50\% threshold. If, however,
\(\pr{A\vert E}=0.6\), then \(\pr{B\vert E}\) must be at least \(0.85\)
to reach the same threshold. Thus, the holistic account would require
that the elements of an accusation be proven to different probabilities
depeding on how well other claims have been established. This outcome
seems counterintuitive.

Denying the conjunction principle, then, is not the solution for the
legal probabilist. More importantly, the conjunction principle is not
obviously implausible. It is a principle about the aggregation of
evidence and the agregation of hypotheses. In the discussion so far, the
evidence was held fixed while the hypotheses supported by the evidence
were kept distinct. The conditional probabilities \(\pr{A | E}\) and
\(\pr{B | E}\) were compared to \(\pr{A \wedge B | E}\). It is
instructive to break down the evidence into parts, and then see what the
conjunction principle looks like.

\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD Say a piece of evidence \(E_1\) (defeasibly) supports hypothesis
\(H_1\) and \(E_2\) (defeasibly) supports \(H_2\). To illustrate,
\(E_1\) could be evidence that the defendant caused harm to the victim
(\(H_1\)), and \(E_2\) evidence that the defendant did so intentionally
(\(H_2\)), for example, a recorded phone conversation shows that the
defendant planned the attakk against the victim several days in advance.
The conjuction principle for evidence \(E_1\) and \(E_2\), hypotheses
\(H_1\) and \(H_2\), and standard of proof \(S\) can be formulated, as
follows: ======= Legal probabilists could reject the conjunction
principle. After all, the conjunction principle denies the accumulation
of risk. If one is justifiably sure about the truth of each claim
considered seperataly, one should not be equally sure of their
conjunction. Say you have checked each page of a book and found no
error. So, for each page, you are sure there is no error. Can you, on
this basis alone, be sure that the book as a whole contains no error?
Not really. As the number of pages grows, the risk that you have missed
a mistake increases. It becomes virtually certain that there is at least
one error in the book as a whole, although for each page you are
justifiably sure there is no error. The same applies to product quality
checks. You may be sure, for each product you checked, that it is free
from defects. But you cannot, on this basis alone, be sure that all
products you checked are free from defects. In fact, since the risk of
error accumulates, you should be sure to have missed at least one
defective product. The conjunction principle, insted, would have us deny
that risks accumulate in this manner.

Without ignoring that risks accumulate, those who endorse the
conjunction principle can still reject this line of argument. In trial
proceedings the possibility of error does not become certain as the
number of claims under consideration increases. Say a piece of evidence
\(E_1\) defeasibly suports claim \(C_1\); \(E_2\) defeasibly supports
claim \(C_2\); and so. Each of these relationships of evidential support
could be undercut by defeaters \(D_1\), \(D_2\), and so on. Suppose the
pieces of evidence are combined into the conjunction
\(E_1 \wedge E_2 \wedge \dots \wedge E_k\). Do the pieces of evidence
combined support the conjunction
\(C_k \wedge C_2 \wedge \dots \wedge C_k\)? Presumably yes. So long as
no defeater \(D_i\) undercuts any of the relatioships of evidential
support, the combined evidence
\(E_1 \wedge E_2 \wedge \dots \wedge E_k\) supports the conjunction
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\). On this view, the
conjunction principle holds. Note, however, the difference between each
individual claim \(C_i\) and the conjunction
\(C_1 \wedge C_2 \wedge \dots wedge C_k\). While each claim \(C_i\)
could be attacked by a defeater \(D_i\), the conjunction
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) could be attacked by a larger
set of defeaters, \(D_1\), \(D_2\), \dots, \(D_k\). So
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) is more susceptible to be
proven wrong than any individual claim \(C_i\). On other hand,
\(C_1 \wedge C_2 \wedge \dots \wedge C_k\) is supported by a larger body
of evidence \(E_1 \wedge E_2 \wedge \dots \wedge E_k\) than each
idnividual claim \(C_i\).

All in all, the conjunction principle is not clearly implausible. For
the sake of argument, however, suppose the legal probabilist rejects it.
How should they then define standards of proof? The legal probabilist
has two optionsm, and neither is without problems. One option is to
require that the prosecution in a criminal case (or the plantiff in a
civil case) establish the claim as a whole -- say the cojunction of
\(A\) and \(B\) -- to the requisite probability. Call this the
\textit{holistic account}. The other option is that the party making the
accusation should establish each claim, separately, to the requisite
probability without establishing their conjunction to the requisite
probability. Call this the \textit{atomistic account}.

The atomistic account is a non-starter. On this account, the prosecution
could establish guilt beyond a reasonable doubt without having
estalished guilt with a suffuciently high probability. The atomistic
approach would allow convictions in cases where \(\pr{G\vert E}\) is
quite low, just because \(G\) is a conjunction of several independent
claims that separately satisfy the standard of proof. This runs counter
to legal probabilism since it would allow convictions when the defendant
is most likely innocent.

The holistic account is more promising, but not without problems either.
The proof of \(A\et B\) would impose an higher requirement on the
...
should be established with probability higher than the 95\% threshold.
So the more conjuncts, the higher their required probability. Moreover,
the standard that applies to one of the conjuncts would depend on what
has been achieved for other conjuncts. If \(\pr{A\vert E}=0.8\), then
\(\pr{B\vert E}\) must be at least \(0.635\) so that
\(\pr{A\et B\vert E}\) is above a 50\% threshold. If, however,
\(\pr{A\vert E}=0.6\), then \(\pr{B\vert E}\) must be at least \(0.85\)
to reach the same threshold. Thus, the holistic account would require
that the elements of an accusation be proven to different required
probabilities depeding on how well other claims have been established.
This outcome seems counterintuitive.

\hypertarget{specific-narratives-idesa-of-a-solution-to-theoretical-difficulties}{%
\subsection{Specific Narratives {[}IDESA OF A SOLUTION TO THEORETICAL
DIFFICULTIES{]}}\label{specific-narratives-idesa-of-a-solution-to-theoretical-difficulties}}
...
or in other words, \(\frac{\Pr(H_p | E)}{\Pr(H_d | E)}\) should be above
a suitably high threshold. If the threshold is for example 2,
...
\[\text{Support[$E_1, H_2, S$] and Support[$E_2, H_2, S$] iff Support[$E_1 \wedge E_2, H_1\wedge H_2, S$]},\]

\noindent where \(\text{Support}[E, H, S]\) means that evidence \(E\)
supports hypothesis \(H\) by standard \(S\). This principle is plausible
and does not deny the fact that risks accumulate. No doubt the possible
presence of a defeater \(D_1\) would undermine the support of \(E_1\) in
favor of \(H_1\), and another defeater \(D_2\) would undermine the
support of \(E_2\) in favor of \(H_2\). But, absent any defeater \(D_1\)
or \(D_2\), the combination of \(E_1\) and \(E_2\) should jointly
support (to whatever suitable standard) the combined hypothesis
\(H_1 \wedge H_2\).

While each hypothesis \(H_i\) could be attacked by a defeater \(D_i\),
the conjunction \(H_1 \wedge H_2\) could be attacked by a larger set of
defeaters, including \(D_1\) or \(D_2\). So \(H_1 \wedge H_2\) is more
susceptible to attack than any individual claim \(H_i\). In this sense,
there are more reasons to doubt \(H_1 \wedge H_2\) than to doubt \(H_1\)
or \(H_2\). This is another way to capture the thought that risks do
accumulate. On the other hand, \(H_1 \wedge H_2\) is more susceptible to
attack, but it is also supported by a larger body of evidence
\(E_1 \wedge E_2\) than each of the individual claim \(H_1\) and
\(H_2\). The larger body of evidence in support of \(H_1 \wedge H_2\)
balances off its greater susceptibility to attack and risk of error.

The challenge for the legal probabilist, at this point, is to capture
the conjunction principle in the formulation above. To this end, the
legal probabilist can pursue different strategies. One strategy is
clearly a non-starter. Suppose that \(\pr{H_1 | E_1}>t\) and
\(\pr{H_2 | E_2}>t\), for a probability threshold \(t\). It does not
follow that \(\pr{H_1 \wedge H_2 | E_1 \wedge E_2}>t\). This can be
readily seen. By the probability calculus,

\[\pr{H_1 \wedge H_2 | E_1 \wedge E_2}=\pr{H_1 |E_1 \wedge E_2} \times \pr{H_2 | E_1 \wedge E_2 \wedge H_1}.\]

\noindent Assuming the independence of the hypotheses and items of
evidence, the above simplifies to

\[\pr{H_1 \wedge H_2 | E_1 \wedge E_2}=\pr{H_1 |E_1} \times \pr{H_2 | E_2}.\]

\noindent So it follows that

\[\pr{H_1 \wedge H_2 | E_1 \wedge E_2}< \pr{H_i | E_i} \text{ for any i=1,2}.\]

But the support that a piece of evidence provides in favor of a
hypothesis should not be understood as a function of the conditional
probability \(\pr{H | E}\). We discussed this point extensively in
earlier chapters.\footnote{REFER TO EARLIER CHAPTERS}. The degree of
evidential support that \(E\) provides in favor of \(H\) consists in the
extent to which taking into account \(E\) raises the probability that
should be assigned to \(H\). A common probabilistic measure of the
evidential support of evidence \(E\) in favor of hypothesis \(H\) is the
Bayesian factor \(\frac{\pr{E | H}}{\pr{E}}\). If the Bayesian factor is
greater than 1, then \(E\) positively supports\(H\). The greater the
Bayesian factor (for values above 1), the stronger the support that
\(E\) provides in favor of \(H\). The Bayesian factor
\(\frac{\pr{E | H}}{\pr{E}}\), unlike the conditional probability
\(\pr{H | E}\), offers an elegant way to overcome the difficulty about
conjuction.

Say \(E_1\) and \(E_2\) support \(H_1\) and \(H_2\), respectively, to
degree \(d_1\) and \(d_2\) greater than 1, that is,
\(\frac{\pr{E_1 | H_1}}{\pr{E_1}}=d_1\) and
\(\frac{\pr{E_2 | H_2}}{\pr{E_2}}=d_2\), where \(d_1, d_2 >1\). Does
\(E_1 \wedge E_2\) provide at least as support in favor of
\(H_1 \wedge H_2\) as measured by the combined Bayesian factor
\(\frac{\pr{E_1 \wedge E_2| H_1\wedge H_2}}{\pr{E_1 \wedge E_2}}=d_{12}\)?
The answer is yes, for the following hold, at least assuming evidence
and hypotheses are independent (in a sense to be specified later):

\begin{align*}
\frac{\pr{E_1 \wedge E_2| H_1\wedge H_2}}{\pr{E_1 \wedge E_2}} & =  \frac{\pr{E_1| H_1\wedge H_2} \times \pr{E_2 | H_1\wedge H_2 \wedge E_1}}{\pr{E_1} \times \pr{E_2 | E_1}}\\
 & = \frac{\pr{E_1| H_1} \times \pr{E_2 | H_2}}{\pr{E_1} \times \pr{E_2}} \\ 
d_{12}  & =d_1 \times d_2 \\ 
 \end{align*}

\noindent The second equality is justified by assuming that (i) \(E_1\)
is probabilistically indepedent of \(H_2\) and \(E_2\) is
probabilistically indepedent of \(H_1\) and also (ii) that \(E_2\) is
probabilistically independent of \(E_1\). These assumptions may be
contested, but they were the same assumptions used to generate the
difficulty with conjunction. These assumptions are also indepedently
plausibe as they are codified in teh Beayesian network in Figure
\ref{network-conjunction}.

Since the combined evidential support \(d_{12}\) equals
\(d_{1}\times d_2\), it will be higher than the support provided by the
individual pieces of evidence toward the individual hypotheses. Under
the independence assumptions, the difficulty about conjuction disappears
soi long as evidentila support is not formalzied in terms of the
conditional probability \(\pr{H | E}\) but rather in terms of the
Bayesian factor \(\frac{\pr{E | H}}{\pr{E}}\).
...
\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}\textless{}
HEAD SO, BASICALLY, NOW WE HAVE TEH FOLLOWING SAMPLE CHAPTERS: =======

\hypertarget{kaplow-quite-independently-a-similar-approach-to-juridical-decisions-has-been-proposed-by-kaplow2014likelihood-well-call-it-.-it-turns-out-that-chengs-suggestion-is-a-particular-case-of-this-more-general-approach.-let-lreprevert-h_piprevert-h_delta.-in-whole-generality-dtlp-invites-us-to-convict-just-in-case-lrelrstar-where-lrstar-is-some-critical-value-of-the-likelihood-ratio.}{%
\subsection{\texorpdfstring{Kaplow Quite independently, a similar
approach to juridical decisions has been proposed by Kaplow (2014) --
we'll call it \textbf{decision-theoretic legal probabilism (DTLP)}. It
turns out that Cheng's suggestion is a particular case of this more
general approach. Let \(LR(E)=\pr{E\vert H_\Pi}/\pr{E\vert H_\Delta}\).
In whole generality, DTLP invites us to convict just in case
\(LR(E)>LR^\star\), where \(LR^\star\) is some critical value of the
likelihood
ratio.}{Kaplow Quite independently, a similar approach to juridical decisions has been proposed by Kaplow (2014) -- we'll call it . It turns out that Cheng's suggestion is a particular case of this more general approach. Let LR(E)=\textbackslash{}pr\{E\textbackslash{}vert H\_\textbackslash{}Pi\}/\textbackslash{}pr\{E\textbackslash{}vert H\_\textbackslash{}Delta\}. In whole generality, DTLP invites us to convict just in case LR(E)\textgreater{}LR\^{}\textbackslash{}star, where LR\^{}\textbackslash{}star is some critical value of the likelihood ratio.}}\label{kaplow-quite-independently-a-similar-approach-to-juridical-decisions-has-been-proposed-by-kaplow2014likelihood-well-call-it-.-it-turns-out-that-chengs-suggestion-is-a-particular-case-of-this-more-general-approach.-let-lreprevert-h_piprevert-h_delta.-in-whole-generality-dtlp-invites-us-to-convict-just-in-case-lrelrstar-where-lrstar-is-some-critical-value-of-the-likelihood-ratio.}}
...
\hypertarget{the-likelihood-strategy-1}{%
\section{The likelihood strategy}\label{the-likelihood-strategy-1}}
