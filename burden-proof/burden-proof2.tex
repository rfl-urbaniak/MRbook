...
perform, [their] humanizing function} [@tribe71].
\end{quote}
...
should address is the difficulty about conjunction. This difficulty
arises when an accusation of wrongdoing, in a civil or criminal
proceeding, is broken down into its constituent elements. Suppose the
prosecution has to establish that the defendant caused harm to the
victim (call it claim \(A\)) and that the harmful act was intentional
(call it claim \(B\)). Cohen (1977) argues that common law systems
subscribe to a conjunction principle, that is, if \(A\) and \(B\) are
established according to the governing standard of proof, so is their
conjunction (and vice versa). If the conjunction principle holds, the
following statements must be equivalent, with \(S\) a generic standard
of proof:
...
\noindent More generally, the conjuction principles requires that
\[S[C_1 \wedge C_2 \wedge \dots \wedge C_k] \text{ iff } S[C_1] \wedge S[C_2] \wedge \dots \wedge S[C_k]\]

\noindent The conjunction principle is consistent wih the case law. For
example, the United States Supreme Court makes it clear that
...
\noindent A plausible way to interpret the passage above is to assume
that to establish guilt beyond a reasonable doubt means to establish
each element of the crime according to the requisite standard. This is
an application of the conjunction principle.
...
\noindent Suppose the prosecution manages to present evidence \(E\) that
...
MARCELLO: CAN WE SAY WHY? HOW DO WE EXPLAIN THIS?

Legal probabilists could reject the conjunction principle. After all,
the conjunction principle denies the accumulation of risk. If one is
justifiably sure about the truth of each claim considered seperataly,
one should not be equally sure of their conjunction. Say you have
checked each page of a book and found no error. So, for each page, you
are sure there is no error. Can you, on this basis alone, be sure that
the book as a whole contains no error? Not really. As the number of
pages grows, the risk that you have missed a mistake increases. It
becomes virtually certain that there is at least one error in the book
as a whole, although for each page you are justifiably sure there is no
error. The same applies to product quality checks. You may be sure, for
each product you checked, that it is free from defects. But you cannot,
on this basis alone, be sure that all products you checked are free from
defects. In fact, since the risk of error accumulates, you should be
sure to have missed at least one defective product. The conjunction
principle, insted, would have us deny that risks accumulate in this
manner.
...
number of claims under consideration increases. Thus, the conjunction
principle is not clearly implausible. For the sake of argument, however,
suppose the legal probabilist rejects it. How should they then define
standards of proof? The legal probabilist has two optionsm, and neither
is without problems. One option is to require that the prosecution in a
criminal case (or the plantiff in a civil case) establish the claim as a
whole -- say the cojunction of \(A\) and \(B\) -- to the requisite
probability. Call this the \textit{holistic account}. The other option
is that the party making the accusation should establish each claim,
separately, to the requisite probability without establishing their
conjunction to the requisite probability. Call this the
\textit{atomistic account}.
...
What would an account of legal that obeys the conjunction principles
look like? Say a piece of evidence \(E_1\) defeasibly suports claim
\(C_1\); \(E_2\) defeasibly supports claim \(C_2\); and so. Each of
these relationships of evidential support could be undercut by defeaters
\(D_1\), \(D_2\), and so on. Suppose the pieces of evidence are combined
into the conjunction \(E_1 \wedge E_2 \wedge \dots \wedge E_k\). Do the
pieces of evidence combined support the conjunction
...
The probabilistic account is unable to capture any of these evidential
relationships. Evidentila support could be captured in terms of
posterior probability or in term,s of likelihood ratio.

First note that \(\pr{C_1 | E_1}>t\) and \(\pr{C_2 | E_2}>t\), it does
not follow that \(\pr{C_1 \wedge C_2 | E_1 \wedge E_1}>t\). EXAMPLE?

Second note that if \(\frac{\pr{E_1 | C_1}}{\pr{E_1 | \neg C_1}}>1\) and
\(\frac{\pr{E_2 | C_2}}{\pr{E_2 | \neg C_2}}>1\), it does not follow
that
\(\frac{\pr{E_1 \wedge E_2 | C_1 \wedge C_2}}{\pr{E_1 wedge E_2 | \neg (C_1 \wedge C_2)}}>1\).

\hypertarget{specific-narratives-idesa-of-a-solution-to-theoretical-difficulties}{%
\section{Specific Narratives {[}IDESA OF A SOLUTION TO THEORETICAL
