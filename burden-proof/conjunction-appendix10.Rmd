...
\begin{figure}[H]
\hspace{2mm}\scalebox{1}{\begin{subfigure}[!ht]{0.35\textwidth}
```{r,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%", dpi = 300}
coordinates(serial) <- list( x=c(X = 2, Z = 2, Y = 2),
                                            y=c(X = 1, Z = 2, Y = 3) )
drawdag(serial, 
        shapes =  list(X = "c", Z = "c", Y = "c"), radius = 5)
```
\subcaption{\textsf{serial}}
\end{subfigure}} 
\hspace{5mm}\begin{subfigure}[!ht]{0.35\textwidth}
\vspace{1mm}
```{r,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%", dpi = 300}
...
drawdag(converging, 
        shapes =  list(X = "c", Z = "c", Y = "c"), radius = 5)
```
\subcaption{\textsf{converging}}
\end{subfigure}

\normalsize
\caption{Three dags to visualize connections.}
...
\begin{align*}
BF^{'}_{AB} &= BF^{'}_{A} \times BF_{B} \geq BF_{B}
\end{align*}
...
First, let's look at simulations based on \textsf{DAG1}. We  can illustrate the distribution of Bayes factors in the two separate scenarios used as assumptions of the above corollaries. 
...
The picture doesn't change when we move to \textsf{DAG2} (Figure \ref{fig:BFind2}). One reason this is interesting is that this suggests that the additional assumption we used in the proof of Corollary \ref{cor:BFweaker} was not needed. We have failed to derive the claim without it, though. 
...
\mar{R: I spent quite some time trying to prove this falls out of the setup, I checked the assumptions in the simulations though, give it a try!}
What does in fact hold in all \textsf{BNs} build on \textsf{DAG 2}, however, is 
 the assumptions of  Corollary \ref{cor:BFweaker2}: it falls out of the \textsf{DAG 2} setup and the assumption that the individual Bayes factor is greater than 1 (this assumption is essential, the claim fails without it) that  $\pr{a \vert b} \leq \pr{a \vert A}$ and $\pr{b \vert a} \leq \pr{b \vert B}$, and so, still both $BF^{'}_{A}$ and $BF^{'}_{B}$ are above one if the individual Bayes factors are above one, even though the former might be lower the latter (respectively). 
...
\raf{A: Verify if this is what you meant}

## Individual likelihood exceeds the conjunction likelihood
...
\begin{table}[h]
```{r,echo=FALSE,eval=TRUE,fig.align = "center",cache=TRUE, fig.show = "hold", out.width = "100%"}
CPkable2("conjunctionBNL","AB") %>%   
                          kable_styling(latex_options=c("striped","HOLD_position")) 
```
\normalsize
\caption{Conditional probability table for the individual likelihood above the conjunction.}
