...
individual claims meet the applicable threhsold, so does the composite
claim. This verifies the aggregation. Conversely, whenever the composite
claim, say \(A \wedge B\), meets the appliacable threshold, the
individual claims also meet the applicable threhsold so long as the
threshold is 75\%. This verifies the other direction of the conjuction
principle, what we called distribution, to some approximation in a
...
cannot be the only factor at play, or else the process of equalization
of the prior probabilities would have elimited the paradox entirely. So
...
This result agrees with the claim defened in the previous section. That
is, even when \(a\wedge b\) strongly supports \(A \wedge B\), the same
evidence need \textit{not} strongly support \(A\). Formally,
...
contrast class. So, in this sense, the expression
\(\frac{\pr{a\wedge b \vert A}}{\pr{a\wedge b \vert A \wedge B}}\) is
more straightforward.

But no matter how we state the claim we reach the same conclusion. The
exact same body of evidence supports \(A\wedge B\) more than it supports
the weaker claim \(A\), even assuming that \(A\) and \(B\) are
independent of one another and thus not mutually reinforcing,. This
seems paradoxical. At first, we thought the paradox could be due to
prior dependency since the combined likelihood ratio
\(\frac{\pr{a\wedge b \vert A \wedge B}}{\pr{a\wedge b \vert \neg (A \wedge B)}}\)
varies depending on the priors of \(A\) and \(B\). But this argument no
longer goes through in case of
\(\frac{\pr{a\wedge b \vert A}}{\pr{a\wedge b \vert A \wedge B}}\) where
any prior dependency has been eliminated. What else is going on here?
...
sensitivity of the evidence, the lower its evidential strength. This is
hard to dispute. Let's try to understand, intuitively, why the
sensitivity of \(a\wedge b\) is worse relative to \(A\) than
...
\(A\) and \(b\) picks up on \(B\). So the sensitivity of \(a\wedge b\)
