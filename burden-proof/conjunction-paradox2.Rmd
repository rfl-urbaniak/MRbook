...
point holds for likelihood ratio threshold.^[Say $A$ and $B$ have  prior probabilities of 
...
The approach consists in fixing the evidential strength 
...
Still, it is worth noting that, if this question can be 
satisfactorily answered, the fixed threshold approach manages 
to vindicate aggregation. 
...
thinks of $H_d$ as a substantial account of what has happened. The defense may elect to challenge 
...
using posterior probabilities. More specifically, given a body of evidence $E$ and two competing hypotheses $H_p$ and $H_d$, the probability $\pr{H_p \vert E}$ should be suitably higher 
than $\pr{H_d \vert E}$, or in other words, the ratio $\nicefrac{\pr{H_p \vert E}}{\pr{H_d \vert E}}$ should be above a suitable threshold. Presumably, the ratio threshold shoud be higher 
...
INSTEAD OF COMPARING POSTERIOR PROBABILITIES, COMPARE LIKELIHOODS OF EVIDENCE GIVEN HYPOTHESIS. DOES THE PARADOX STILL REMAIN?
