...
of evidential support (discussed earlier in Chapter XX \todo{REFER TO EARLIER CHAPTER}, 
...
is that $BF_B=\frac{\pr{b |B}}{\pr{b}}$ was replaced by \mar{R: use vert for conditional probabilities, please.}
 $BF^{'}_B=\frac{\pr{b |B}}{\pr{b\vert a}}$. Since $b$ need not be probabilistically independent of $a$,  there is no guarantee that $\pr{b \vert a}=\pr{b}$. However, if $BF_B$ is  greater than 1, so is $BF^{'}_B$ as soon as  $\pr{b\vert a} \leq \pr{b}$. In many cases  of interest, when pieces of evidence converge, this is a plausible assumption to make, unless a piece of evidence for $A$ actually makes the occurrence of a piece of evidence for $B$ more likely (which in turn would decrease the Bayes factor for $B$ by increasing the denominator). \todo{REFER TO EARLIER CHAPTER ON CROSS-EXAMINATION AND CORROBORATION}  If these assumptions are satisfied, \eqref{eq:BFdep} entails that the joint Bayes factor, $BF_{AB}$, is going to be greater than any of the individual Bayes factors (again, see the appendix for proof).
...
Let us start with observing that  given the independence assumptions entailed even by the more complex Bayesian network, we have (the proof is in the appendix):
...
The complexity in the numerator arises from conditioning on a negation of a conjunction, and it makes proving a general claim about the behavior of a conjunction tricky. However, the formula above allows us to  look at an illustration obtained by looking at a simplified picture. 
