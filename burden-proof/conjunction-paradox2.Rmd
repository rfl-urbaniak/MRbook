...
\noindent where $a$ and $b$ denote the evidence for claims $A$ and $B$ respectively, and $S$ denote the standard by which the evidence establishes the claim in question. In the case of more than two claims, the formulation of the conjunction principle should be extended accordingly. 
...
Consider now what happens if the probabilistic independence between claims $A$ and $B$ is dispensed with. To this end, it is enough to draw an arrow between $A$ and $B$ in our directed graph. The result is displayed in Figure \ref{fig:conjunctionDAG2}. There is an open path between  hypothesis nodes $A$ and $B$ and  evidence nodes $a$ and $b$. Thus, the new graph no longer guarantees the probabilistic independence of $A$ and $B$, or that of $a$ and $b$.  Items of evidence $a$ and $b$ are still probabilistically independent of one another \textit{conditional} on their respective hypothesis. That is, $\pr{a \vert A}=\pr{a \vert A \wedge b}$ and $\pr{b \vert B}=\pr{b \vert B  \wedge a}$.
...
by multiple pieces of evidence (e.g. $a\wedge b$) for a conjunctive claim (e.g. $A\wedge B$) typically exceeds the individual support supplied by individual pieces of evidence for individual claims. This claim holds for both the Bayes factor and the likelihood ratio. We start with the Bayes factor $\pr{E \vert H}/\pr{E}$ as our measure of the support of $E$ in 
...
\raf{M: Argument here is too cryptic. Need to say more. Will revise later. See also my next comment}

\noindent(See the appendix for a proof.) The difference from the case of independent hypotheses 
is that $BF_B=\frac{\pr{b \vert B}}{\pr{b}}$ was replaced by  $BF^{'}_B=\frac{\pr{b \vert B}}{\pr{b\vert a}}$. Since $b$ need not be probabilistically independent of $a$,  there is no guarantee that $\pr{b \vert a}=\pr{b}$. However, if $BF_B$ is  greater than 1, so is $BF^{'}_B$ as soon as  $\pr{b\vert a} \leq \pr{b}$. In many cases  of interest, when pieces of evidence converge, this is a plausible assumption to make, unless a piece of evidence for $A$ actually makes the occurrence of a piece of evidence for $B$ more likely (which in turn would decrease the Bayes factor for $B$ by increasing the denominator). \raf{M: Not sure this argument is right. I  expect evidence $a$ (favoring $A$, which converges with $B$) to increase the probability of $b$, hence $\pr{b \vert a} \geq \pr{a}$.} \todo{REFER TO EARLIER CHAPTER ON CROSS-EXAMINATION AND CORROBORATION}  If these assumptions are satisfied, \eqref{eq:BFdep} entails that the joint Bayes factor, $BF_{AB}$, is going to be greater than any of the individual Bayes factors (again, see the appendix for proof).
...
\noindent The equality is sufficiently generally as it holds whether or not hypotheses $A$ and $B$ are probabilistically independent. (See the appendix for a proof.) The numerator is involved because conditioning on the negation of a conjunction makes proving a general claim about the behavior of the conjunction tricky. 
...
### Same sensitivity and specificity along with independence 

For illustrative purposes, we first consider a simplified picture. Let the sensitivity and specificity of the items of evidence be the same and equal $x$.  Let also $A$ and $B$ be probabilistically independent in agreement with the directed graph in Figure \ref{fig:conjunctionDAG}.
...
### Different sensitivity and specificity along with depedence  -- NEEDS SIMULATION

If the items of evidence have different levels of sensitivity and specificity, the combined likelihood ratio never goes below the lower of the two individual likelihood ratios, but can be lower than the higher one. The simplified situation visualised in the previous situation doesn't contradict this. In fact, it follows from this more general fact---after all, in the simplified situation both individual likelihood ratios were identical, so whenever the joint \textsf{LR} was higher than the minimum of the invidual \textsf{LRs}, it was higher than the both of them. \raf{M: Need to add simulation results to make this argument 
fully general and drop all the simplifying assumptions (.e.g. independence or equiprobability)}
If the composite likelihood ratio behaves differently than the Bayes factor in that it is greater than the lower of the individual likelihood ratios, rather than being greater than both of them,  Dawid's claim that  `the support supplied by the conjunction of several independent testimonies exceeds that supplied by any of its constituents' still holds.  \mar{R: I don't see how it holds; after all it fails to be higher than one of them!! Rethink this.}
...
According to the Bayes factor and the likelihood ratio, the combined evidential support for the conjunction usually exceeds the individual evidential support for individual claims. This fact can be used to justify aggregation. The argument, however, is not immediate. Aggregation is a principle about the standard of proof. The Bayes factor and the likelihood ratio are instead measures of evidential support. 
The standard of proof must somehow be connected to these measures of evidential strength.  To this end, the decision criterion can be formulated in terms of evidential strength instead of posterior probabilities. Consequently, the standard of proof will no longer be formalized as a posterior probability threshold, but rather, as a threshold for the Bayes factor or the likelihood ratio. The new threshold will no longer be a probability between 0 and 1, but a number somewhere above one. The greater this number, the more stringent the standard of proof, for any value above one. In criminal trials, for example, the rule of decision could be: guilt is proven beyond a reasonable doubt if and only if the evidential support in favor of $H$---as measured by the Bayes factor $\nicefrac{\pr{E \vert H}}{\pr{E}}$ or the likelihood ratio $\nicefrac{\pr{E\vert H}}{\pr{E \vert \n H}}$---meets a suitably high threshold $t_{BF}$ or $t_{LR}$.  The question at this point is, how do we identify 
the appropriate evidential strength threshold? The answer to this question is not obvious. \todo{will at some point take a look at "Bayesian Choice"} Below we examine two possible approaches. 
 
### Variable threhsold

The first approach we consider will soon turn out to be inadequate. It is still useful to understand why this approach does not work to formulate one that is more promising. A threshold on evidential strength can be derived from the threshold on posterior probability. The advantage of the posterior probability threshold is that its stringency can be determined in a decision-theoretic manner via the minimization of expected costs (see Chapter XX \todo{REFER TO ERALIER CHAPPER}). So how should one derive a threshold for the Bayes factor and the likelihood ratio, call these thresholds $t_{BF}$ or $t_{LR}$, from a thresholds $t$ on the posterior probability? 
...
greater than the individual evidential support, whether it is measured 
in terms of the Bayes factor or the likelihood ratio. So whenever $BF_A$ 
...
using a variable evidential strength threshold, this result militates in favor of the fixed evidential strength threshold  approach.  
...
Focusing on a posisble defense hypothesis, $\n S\et \n C$, the following holds:^[NEED PROOF]
We have
...
probabilists simply reject this principle?  So far we have not challenged it, but it is time to scrutinize it more closely. In this section, we provide an epistemic argument and a legal argument to question the conjunction principle. But we also caution that a simple rejection of the conjunction will not automatically dissolve the difficulty with conjunction.  More work needs to be done.
