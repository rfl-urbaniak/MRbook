...
individual support by \(a\) and \(b\) in favor of \(A\) and \(B\)? The
combined support should be measured by the combined Bayes factor
\(\pr{a \wedge b| A\wedge B}/\pr{a \wedge b}\), and the latter, under
suitable independence assumptions, equals the product of the individual
supports \(s_{A}\) and \(s_{B}\). By the probability calculus,
...
evidence bears on its own hypothesis. One might wonder, however, why the
arrows go from \(A\) and \(B\) into the node representing the
conjunction \(A\wedge B\). This setting ensures that \(A\) and \(B\) are
probabilistically independent, an assumption often made in the
formulation of the conjunction
...
result can be generalized beyond two pieces of evidence and holds
...
We have seen that if the Bayes factor measures evidential suppport, the
following extrapolation principle fails in a large class of cases:

\[\text{If S[$a \wedge b, A\wedge B$], then S[$a, A$] and S[$b, B$].} \tag{EXT1}\]

\noindent But perhpas this principle is not as essential as we might
think. Since the body of evidence is not held constant, the combined
support supplied by \(a\wedge b\) could be stronger than the support
supplied by \(a\) and \(b\) individually. So even when the conjunction
...
counterintuitive consequences, what we called the extrapolation paradox.
For suppose the prosecution provided evidence for claim \(A\), but this
evidence still falls short of the threshold \(t\) (a certain number
above 1). Just by tagging an additional claim \(B\) and without doing
any further evidentiary work, the prosecution could provide sufficiently
strong evidence (which meets the threshold \(t\)) in favor of claim
\(A \wedge B\). So, it could well happen that, while the prosecution
failed to prove beyond a reasonable doubt that the defendant injured the
victim, the prosecution could nevertheless prove beyond a reasonable
doubt that the defendant injured the victim and did so intentionally.
This is odd.
...
probabilistic measure of evidential support. On this account, the
standard of proof is again formalized as a certain threshold \(t\) above
one. In criminal trials, for example, the rule of decision would be:
Guilt is proven beyond a reasonable doubt if and only if the evidential
support in favor of \(G\)---as measured by the likelihood ratio
\(\frac{\pr{E \vert G}}{\pr{E \vert \neg G}}\)---meets a suitably high
threshold \(t\) above one. The greater \(t\) the more stringent the
threshold. We will see that this approach is able to justify one
direction of the conjunction principle---what we called
aggregation---but still fails to justify the other direction---what we
called extrapolation. So the extrapolation paradox arises here again.
...
also be above the threshold? The answre is affirmative. To see why,
...
\noindent Given these assumptions, the graph of the combined likelihood
ratio can now be easily plotted against the single likelihood ratios. As
Figure \ref{fig:jointLRMarcello} shows, the combined likelihood ratio
varies depeding on the prior probabilities \(\pr{A}\) and \(\pr{B}\),
but it always execeeds the individual likelihood ratios so long as they
...
conjunction principle, what we called extrapolation. For suppose
evidence \(a \et b\) supports \(A \et B\) to the required threshold
\(t\). If evidential support is measured by the likelihood ratio, the
threshold in this case should be some order of magnitude greater than
one. If the combined likelihood ratio meets the threshold \(t\), one of
the individual likelihood ratios may well be below the threshold.
...
\textbf{SEE PROOF IN EARLIER CHAPTERS.}
 If $A$ and $A$ are assumed to be probabilistically independent, the numerator and the denominator will be the same, 
 so $\frac{\pr{b \vert A}}{\pr{b \vert \neg A}}=1$. Thus, 
 $S[a\wedge b, A]=\frac{\pr{a \vert A}}{\pr{a \vert \neg A}}=S[a, A]$. Since $S[a, A]< S[a\wedge b, A\wedge B]$ (see Figure \ref{fig:jointLRMarcello}), it follows $S[a\wedge b, A]<S[a\wedge b, A\wedge B]$}
So likelihood ratios fail to justify (EXT2). Even this second, seemingly
unobjectionable version of extrapolation fails. The same
counterintuitive consequences that arose with Bayes factor manifest
themselves here. Even with likelihood ratios instead the Bayes factor,
the extrapolation paradox persists.
...
Instead of thinking in terms of absolute thresholds---whether threshold
on posterior prbabilities, teh Baye factor or the likelihood ratio---the
standard of proof can be understood comparatively (Cheng, 2012). Say the
...
trials. On this approach, rather than directly evaluating the posterior
probability or the evidential support of \(H_p\) given the evidence and
comparing it to a threshold, we compare the support that the evidence
provides for two competing hypotheses \(H_p\) and \(H_d\), and decide
for the one for which the evidence provides better support.

Let's first work out this comparative strategy using posterior
probabilities. More specifically, given a body of evidence \(E\) and two
