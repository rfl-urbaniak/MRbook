...
conjunction principle conflicts with the threshold-based probabilistic
...
to the required probability, say at least 95\%. Has the prosecution met
the burden of proof? Each claim was established to the requisite
probability threshold, and thus it was established to the requisite
standard (assuming the threshold-based interpretation of the standard of
proof). And if each claim was established to the requisite standard,
then (i) guilt as a whole was established to the requisite standard
(assuming the conjunction principle). But even though each claim was
established to the requisite probability threshold, the probability of
their conjunction---assuming the two claims are independent---is only
...
interpretation of the standard of proof is in the above sense conclifts
with the conjunction principle---does not subside when the number of
...
the claims are no longer regarded as independent. For suppose claim
\(A\) and \(B\) have each been established to 95\% probability, enough
to meet the probability threshold. The probabilty of the conjunction
\(A \et B\), without the assumption of independence, equals
\(\pr{A | B} \times \pr{B}\). This probability could still be below the
95\% threshold unless \(\pr{A | B}=100\%\), and the latter is not always
the case. For example, that someone premediated a harmful act against
another (claim \(B\)) makes it more likely that they did cause harm in
the end (claim \(A\)). That is, \(\pr{A | B} > \pr{A}\). But
premeditation of harm does not always lead to harm. Thus, \(\pr{A | B}\)
should be below 100\%. In that case, the probability of the conjunction
\(A \et B\) would still be below the 95\% probability
...
we offer what we take to be the most promising answer, we will explore
other probability-based solutions, and understand why these solutions,
albeit promising, ultimately fail. This is what we set out to do in the
following sections.
...
disappear? In current discussions in epistemology, this principle has
been contested because it appears to deny the fact that risks accumulate
(CITE).
...
probability, say at least 95\%, without establishing the conjunction to
the requisite probability. Call this the \textit{atomistic account}. On
this view, the prosecution could be in a position to establish guilt
...
relate to larger claims in the process of legal proof. While we think
that denying the conjunction principle is ultimately the right strategy,
...
the allegation of wrongdoing in a criminal or civil trial. This is,
however, a simplification. It is instructive to distinguish the evidence
and the hypotheses that the evidence support.

Say evidence \(a\) supports \(A\) and evidence \(b\) supports \(B\). In
an aggraveted assault case, evidence \(a\) could be a witness testimony
that the defendant physically harmed the victim (claim \(A\)), and \(b\)
...
called the firefighther for help. The question now is whether the
combination of \(a\) and \(b\) support the conjunction \(A \et B\) and
viceversa. In other words, the question is whether the following
...
from the earlier one in that it is formulated in terms of claims \(A\)
and \(B\) accompanied by their supporting evidence \(a\) and \(b\).
...
that \(\pr{A | a}>t\) and \(\pr{B | b}>t\), for a threshold \(t\)---in
other words, that given the supporting evidence \(a\) and \(b\), both
\(A\) and \(B\) are sufficiently probable (for a fixed threshold). It
does not generally follow that \(A \et B\) is sufficienlty probable
given the combined evidence \(a\et b\), or in symbols,
\(\pr{A \wedge B | a \wedge b}>t\). By the probability calculus,
\begin{align*}
...
of independence do not always hold, but they do sometimes. Consider our
aggravated assault example. Presumably,
...
\(B\), separately, right above above the probability threshold \(t\).
Since
\(\pr{A \wedge B | a \wedge b} =\pr{A |a \wedge b} \times \pr{B | a \wedge b \wedge A}\),
it follows that \(\pr{A \wedge B | a \wedge b}\) would be below \(t\) so
long as \(\pr{B | a \wedge b \wedge A}\) is below 100\%, which would
often be the case since evidence is fallible. So even though \(A\) and
\(B\) were established to the required probability, the conjunction is
not.
...
discussed this point extensively in earlier chapters (REFER TO EARLIER
CHAPTERS). We should therefore examine the difficulty about conjunction
while keeping this in mind. In fact, when confronted with the difficulty
about conjunction, legal probabilist will often invoke measures of
evidential support, the Bayes factor and the likelihood ratio being the
most common ones. For example, here is a remark by (Dawid, 1987) in one
of the earlier attempts to solve the difficult about conjunction using
...
factor or the likelihood ratio. Does this strategy work? Not quite. As
we will now show, the difficulty about conjuntion will not go away even
...
\noindent  The only controversial step is marked by an asterisk. These
independence assumptions nedded for this step are codified in the
Bayesian network in Figure \ref{network-conjunction}.
\todo{more work on explaining independence in BNs is needed at some point, not sure if here. }
\todo{How about without independence? Can we find a counterexample?}
Now, if the combined support \(s_{AB}\) equals \(s_{A}\times s_{B}\), it
...
The structure of the nextwork in Figure \ref{network-conjunction} is
extremely natural except for the incoming arrows into node
\(A \wedge B\). An alternative network, and equally plausible network,
is depicted in Figure \ref{network-conjunction-alternative}. The arrows
are outgoing from node \(A \wedge B\) rather than incoming. Given this
other network, the following holds:
...
\noindent  Presumably, \(s^{*}_{B}\) is lower than \(s_{B}\) because
\(\pr{b | a} > \pr{b}\) (assuming \(a\) and \(b\) are convergent pieces
of evidence) but is till grater than one. If that is the case, the
combined support \(s_{AB}\) would still execeed the individual support
of the individual piece of evidence.

So, if the combined support \(s_{AB}\) equals \(s_{A}\times s_{B}\) or
\(s_{A}\times s^{*}_{B}\)---where the individual support are all greater
than one---does the difficulty about conjunction now evaporate, as Dawid
seems to think? To some extent the paradox does go away. The paradox was
about whether the conjunction satisfies a proof standard just in case so
do its conjuncts. The Bayes factor strategy will work against this
challenge only if the standard of proof is no longer formalized as a
probability threshold, but rather, as a threshold about evidential
support. That is, for this to work, the threshold should no longer apply
to quantities such as the posterior probability \(\pr{G | E}\), but
rather to quantities such as the Bayes factor
\(\frac{\pr{E | G}}{\pr{E}}\). In criminal trials, the rule of decision
would be as follows:
...
and \(\pr{b}=t\).\todo{You meant P(B) here?} Thus, \begin{align*}
...
\noindent Let's make a further simplification assuming \(x=y\), and plot
individual and combined likelihood ratios against \(x\).
\eqref{eq:combinedLRMarcello} reduces to: \begin{align*}
