...
support because \(\frac{\pr{a \et b | A \et B}}{\pr{a \et b}}\) equals
\(\frac{\pr{a |A}}{\pr{a}} \times \frac{\pr{b |B}}{\pr{b}}\) (if A and B
are independent) or
\(\frac{\pr{a |A}}{\pr{a}} \times \frac{\pr{b |B}}{\pr{b \vert a}}\) (if
A and B are dependent). Aggregation could not be justified using
posterior probabilities \(\pr{A | a}\) and \(\pr{B | b}\). So it is an
...
individual claims still fail to meet the standard. This is odd. More
specifically, the following extrapolation principle fails in some cases:

\[\text{If S[$a \wedge b, A\wedge B$], then S[$a, A$] and S[$b, B$].} \tag{EXT1}\]

\noindent This principle might not be as essential as we think. Since
the evidence is not held constant, the support supplied by \(a\wedge b\)
could be stronger than that supplied by \(a\) and \(b\) individually. So
even when the conjunction \(A \wedge B\) is established to the requisite
standard given evidence \(a\wedge b\), it might still be that \(A\) does
not meet the requisite standard (given \(a\)) nor does \(B\) (given
\(b\)).

But now consider a less controversial version of extrapolation. If the
evidence is held constant, the extrapolation principle would become:
...
equating the standard to a threshold \(t\) above one. In criminal
...
of \(G\)---as measured by the likelihood ratio
\(\frac{\pr{E \vert G}}{\pr{E \vert \neg G}}\)---meets a suitably high
threshold \(t\) above one. The greater the threshold, the more stringent
the standard. Setting aside the problem of how ot identify the
appropriate threshold, we will see that this approach is able to justify
one direction of the conjunction principle---what we called
