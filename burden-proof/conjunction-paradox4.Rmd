...
The standard of proof in a civil or criminal trial is a decision criterion. It answers the question, what characteristics should the evidence presented at trial possess to judge the defendant civilly or criminally liable of whatever they were accused?
According to some legal probabilists, a verdict against the defendant is warranted in case the evidence establishes the defendant's liability with a probability above a suitable  threshold [see @Bernoulli1713Ars-conjectandi; @Laplace1814;  @Kaplan1968decision;  @kaye79; @Dekay1996; @laudan2006truth]. In criminal cases, the bar is  high, and so the threshold will be, say "at least .95 probability". In civil cases,  the bar is lower, and so the threshold will be, say "at least .5 probability".  


This  threshold interpretation of the standard of proof is simple and elegant. Chapters XX \todo{REFER TO EARLIER CHAPTER} shows how the .95 and .5 thresholds can be formally justified. But, unfortunately,  this interpretation is not without problems. Chapter Chapters ZZ \todo{REFER TO EARLIER CHAPTER} examines a first theoretical problem: the paradox of naked statistical evidence. The present chapter examines a second theoretical problem: what we will \textbf{the difficulty with conjunction}, often called \textbf{the conjunction paradox}. The chapter is structured as follows. First, we describe the difficulty with conjunction. Next, we detail several strategies that legal probabilists have pursued as a response to this difficulty. These strategies are promising and worth examining, but we show that they are ultimately unsatisfactory. Finally, we put forward our own proposal. 
...
\noindent In the case of more than two claims, the formulation of the conjunction principle can be extended accordingly.  Note that  $a$ and $b$ denote the evidence for claims $A$ and $B$ respectively, and $S$ denotes the standard by which the evidence establishes the claim in question. So, for example, the expression S[$a, A$] should be read as 'evidence $a$ supports claim $A$ by standard $S$'. 
...
as evidence for the proposition that the defendant knew he was dealing with a firefighter  (call this proposition \textsf{firefighther}). If $\pr{\textsf{injury} \vert \textsf{witness}}$ and $\pr{\textsf{firefighter} \vert \textsf{call} }$ both meet the required probability threshold, does $\pr{\textsf{injury} \wedge \textsf{firefighter} \vert \textsf{witness} \wedge \textsf{call}}$ also necessarily meet the threshold? 
...
To address this question, we will represent formally---specifically, using Bayesian networks---the relationship between generic claims $A$, $B$ and the conjunction $A\wedge B$, as well as the supporting evidence $a$, $b$ and the conjunction $a\wedge b$. 


##  Independent hypotheses 

 We already studied Bayesian networks in Chapter ZZ. \todo{REF TO OTHER CHAPETR} Here we only sketch the essential ideas.  A Bayesian network is a formal model that consists 
