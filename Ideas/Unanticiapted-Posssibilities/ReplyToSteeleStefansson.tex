...
algebra models the agent's awareness state, the propositions under
consideration by the agent as live possibilities. The algebra is fixed
once and for all, and the learning experience does not modify it. So
Bayesian conditionalization makes it impossible for an agent to learn
something they have never thought about. This forces a great deal of
rigidity on the learning process. Even before learning about \(E\), the
agent must already have assigned a degree of belief in any proposition
conditional on \(E\). This picture commits the agent to the
...
Reverse Bayesianism. Still, we share Steele and Stefánsson's skepticism
and provide a better counterexample. At the same time, we carve out a
class of cases in which Reverse Bayesianism still holds, and these
include not just cases of expansion but also some cases of refinement.
Our critique of Reverse Bayesianism is thus more fine-grained than
Steele and Stefánsson's. We conjecture that the problem of awareness
growth cannot be tackled in an algorithmic manner. Subject-matter
structural assumptions are necessary. We rely on the theory of Bayesian
...
friend met one night secretively without telling you
(\textit{Secretive}). Given this evidence, \textit{Affair} is more
probable than \textit{Friends/acquaintances}:
...
conclude the level of linguistic difficulty must be low and the movie
...
then you add a further category, level of difficulty. So the proposition
which picks out the entire possibility space should be the same before
and after awareness growth, for example,
...
How strong of a counterexample is this? Steele and Stefánsson consider
an objection:
...
Suppose the counterexample did only work in tandem with an episode of
...
theories work when awareness growth is not intertwined in complex ways
with learning. So, Steele and Stefánsson's critique of these theories
sits now on much firmer ground. At the same time, the scope and
applicability of this critique should not be exaggerated either. As we
shall now see, there are cases of refinement in which Reverse
Bayesianism and Awareness Rigidity are perfectly fine in their place.
...
and Stefánsson's critique only targets a subclass of refinement cases.
Its scope is therefore somewhat limited. This does not mean, however,
that Reverse Bayesianism is a defensible theory. In that, we agree with
Steele and Stefánsson. We conjecture that there is a deeper reason why
Reverse Bayesianism cannot work as a theory of awareness growth. That
is, it seeks to provide a formal, almost algorithmic solution, but this
formal aspiration might very well be wrong.

\hypertarget{material-not-formal-constraints}{%
\section{Material, not formal
constraints}\label{material-not-formal-constraints}}

We have seen that Reverse Bayesianism is perfectly fine in its place in
scenarios like \textsc{Veracity}, but fails in scenarios like
\textsc{Lighting}. What is that so? The reason is that two scenarios are
structurally different, and this different can be appreciated by looking
at how the Bayesian network evolve. In one scenario, the new node is
added downstream in the networks. Because of the formal properties of
the network, the conditional probabilities that defined the upstream
nodes remain unaffected. This vindicates Reverse Bayesianism. By
contrast, in another scenario, the new node is added upstream. Because
of of the formal properties of the network, this upstream addition of a
new node affects the conditional probabilities that are associated with
the downstream nodes. Reverse Bayesianisma fails here. The difference
between downstream refinement like \textsc{Veracity} and upstream
refinement like \textsc{Lighting} cannot be captured formally by Reverse
Bayesianism, too coarse of a theory of awareness growth.

Those that sympathize with a formal, algorithmic solution might offer
the following reply. Granted, Reverse Bayesian (or its close cousins,
Awareness Rigidity) are not general enough formal constraints. Could it
be that a weaker formal constraint is in fact satisfied in cases of
upstream refinement like \textsc{Lighting}? We now explore what this
