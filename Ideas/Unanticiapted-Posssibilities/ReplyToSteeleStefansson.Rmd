...
This algebra models the agent's awareness state, the propositions under consideration by the agent as live possibilities. The algebra is fixed once and for all, and the learning experience does not modify it.
So Bayesian conditionalization makes it impossible for an agent to learn something they have never thought about. This forces a great deal of rigidity on the learning process. Even before learning about $E$, the agent must already have assigned a degree of belief in any proposition conditional on $E$. This picture commits the agent to the specification of their 'total possible future experience' (Howson 1976, The Development of Logical Probability), as though learning was confined to an 'initial prison' (Lakatos, 1968, Changes in the Problem of Inductive Logic).  
...
We contend, however, that their counterexamples have limited applicability and thus constitute an overall weak argument against Reverse Bayesianism. Still, we share Steele and Stefánsson's skepticism and provide a better counterexample. At the same time, we carve out a class of cases in which Reverse Bayesianism still holds, and these include not just cases of expansion but also some cases of refinement. Our critique of Reverse Bayesianism is thus more fine-grained than Steele and Stefánsson's. We conjecture that the problem of awareness growth cannot be tackled in an algorithmic manner. Subject-matter structural assumptions are necessary. We rely on the theory of Bayesian networks to illustrate this point. 
...
<!---The algebra contained the propositions \textit{French} and \textit{German}, as well as \textit{Thriller} and \textit{Comedy}. <!---What you are concerned about is whether your will enjoy the movie or not (\textit{Enjoy Movie}), depending on its language and genre. ---> <!---Then, you realized another variable might be at play, namely the level of difficulty of the language of the movie.---> <!---, \textit{Difficult} and \textit{Easy}.---> But learning that the owner is simple-minded made you conclude the level of linguistic difficulty must be low and the movie most likely a thriller rather than a comedy (perhaps because thrillers are simpler---linguistically---than comedies). So, against Reverse Bayesianism, \textsc{Movies} violates the condition $\frac{\pr{\textit{Thriller}}}{\pr{\textit{Comedy}}}=\frac{\ppr{+}{\textit{Thriller}}}{\ppr{+}{\textit{Comedy}}}$. 
...
How strong of a counterexample is this? Steele and Stefánsson consider an objection:
...
can no longer claim that their theories work when awareness growth is not intertwined in complex ways 
with learning. So, Steele and Stefánsson's critique of these theories 
sits now on much firmer ground. At the same time, the scope and applicability of this critique should not be exaggerated either. As we shall now see, there are cases of refinement in which Reverse Bayesianism and Awareness Rigidity are perfectly fine in their place. 
...
which Steele and Stefánsson deploy in their analysis of awareness growth is too coarse.--->  Bayesian networks helped to distinguish these two forms of refinement, and there may be other, more fine-grained distinctions to be made. So, ultimately, Steele and Stefánsson's critique only targets a subclass of refinement cases. Its scope is therefore somewhat limited. This does not mean, however, that Reverse Bayesianism is a defensible theory. In that, we agree with Steele and Stefánsson. We conjecture that there is a deeper reason why Reverse Bayesianism cannot work as a theory of awareness growth. That is, it seeks to provide a formal, almost algorithmic solution, but this formal aspiration might very well be wrong. 

# Material, not formal constraints

We have seen that Reverse Bayesianism is perfectly fine in its place in scenarios like \textsc{Veracity}, but fails in scenarios like \textsc{Lighting}. What is that so? The reason is that two scenarios are structurally different, and this different can be appreciated by looking at how the Bayesian network evolve. In one scenario, the new node is added downstream in the networks. Because of the formal properties of the network, the conditional probabilities that defined the upstream nodes remain unaffected. This vindicates 
Reverse Bayesianism. By contrast, in another scenario, the new node is added upstream. Because of 
of the formal properties of the network, this upstream addition of a new node affects the conditional probabilities that are associated with the downstream nodes. Reverse Bayesianisma fails here.  The difference between downstream refinement like \textsc{Veracity} and upstream refinement like \textsc{Lighting} cannot be captured formally by Reverse Bayesianism, too coarse of a theory of awareness growth.

Those that sympathize with a formal, algorithmic solution might offer the following reply. Granted, Reverse Bayesian (or its close cousins, Awareness Rigidity) are not general enough formal constraints. Could it be that a weaker formal constraint is in fact satisfied in cases 
of upstream refinement like \textsc{Lighting}? We now explore what this 
weaker formal constraint might look like. 
...
The difference with condition ($C^*$) is that here two propositions, not just one, are conditioned on. So ($C^*$) should be be generalized, accordingly, but the general idea remains the same.^[A generalzied version iof ($C^*) wouldbe as follows. $Let $E$ be a node with several incoming arrows departing from a finite set of nodes, $X_1, X_2, \dots X_k$ that are shared before and after awareness growth. Then:
...
We argued that the case against Reverse Bayesianism is much 
weaker than one might think. The scenario \textsc{Movies}---which is Steele and Stefánsson's key counterexample to Reverse Bayesianism---is unconvincing since it mixes learning and refinement. To avoid this, we constructed a more clear-cut case of refinement, \textsc{Lighting}, in which both Awareness Rigidity and Reverse Bayesianism fail unequivocally. However, we showed that even in cases like this a modest conservative constraint can still be carved out, vindicating to some extent the spirit 
of Reverse Bayesianism. We also showed that there are cases of refinement like \textsc{Veracity}
in which Reverse Bayesianism and Awareness Rigidity are perfectly 
fine in their place. <!---So the distinction between refinement and  expansion 
that Steele and Stefánsson draw, albeit a good first approximation, 
is too coarse and needs to be made more precise.--->


We conclude with a few programmatic observations. We think that the awareness of agents grows while holding fixed certain material structural assumptions, based on commonsense, semantic stipulations or causal dependency. To model awareness growth, we need a formalism that can express these material structural assumptions. This can done using Bayesian networks, and we offered some illustrations of this strategy, for example, by distinguish two forms of refinement on the basis of different structural assumptions. These material assumptions also guide us in formulating the adequate conservative constraints, and these will inevitably vary on a case-by-case basis.  Our approach stands in stark contrast with much of the literature on awareness growth from a Bayesian perspective. This literature is primarily concerned with a formal, almost algorithmic solution to the problem. We suspect that seeking such formal solution is doomed to fail. Insofar as Reverse Bayesianism is an expression of of this formalistic aspiration, we agree with Steele and Stefánsson that we are better off looking elsewhere.
