...
Bayesianism is not the right theory of awareness growth, what theory (if
...
\hypertarget{new-explanations}{%
\section{New explanations}\label{new-explanations}}

Perhaps the most common example of awareness growth occurs when you
...
inquiry: medicine, law, science, even in everyday affairs Here is a
...
considered for the fact that your partner and your best friend met one
night without telling you (\textit{Secretive}). Given this evidence,
\textit{Affair} is more probable than \textit{Friends/acquaintances}:
...
As Steele \& Stefánsson (2021) note, the conjunction of (\(>\)) and
(\(<\)) violates Reverse Bayesianism since
\textit{Friends/acquaintances} and \textit{Affair} are basic
propositions that do not contain any connectives. But, they also note, a
quick fix is available. For consider the following condition, called
...
\(T^*\). After all, before awareness growth, the eventuality that your
partner and best friend could be organizing a surprise for had been
tacitly ruled out. Conditional on \(\neg\textit{Surprise}\), no
probability assignment should change, including the probability of
\textit{Affair}. Thus,
...
does not hold in all cases. We will soon see examples of its failure.
Before that, it is important to understand why Awareness Rigidity is an
adequate principle to govern awareness growth in some cases, commonly
known in the literature as cases of \emph{expansion}. A precise
definition can be tricky to provide, but a rough characterization will
...
possibility space. An algebra of propositions thus interpreted induces a
partition of the possibility space. Expansion occurs when the new
proposition is inconsistent with the disjunction of all the existing
ones, thus making the old partition no longer exhaustive.
...
values are meant to be exhaustive. This means that \textit{Affair}
...
two values are not longer considered exhaustive, but a hird value is
added, \textit{Surprise}. Now the disjunction of
\textit{Friends/acquaintances} and \textit{Affair} functions as the
negation of \textit{Surprise}.

Since you trust the evidence, you think that the evidence is more likely
under the hypothesis that the defendant was present at the crime scene
...
role of subject-matter assumptions in theorizing about awareness growth.
...
been good, perhaps you should just stay put until you learn soemthing
more specific.

In what follows, we illustrate how Bayesian networks helps to model what
is going on in \textsc{Lighting} and conclude that you should not stay
put. You should probably revise downward your confidence in the
hypothesis that the defendant was around the crime scene.
...
graph, which is the usual hypothesis-evidence idiom (Fenton et al.,
2013):
...
Before we offer another example of how Bayesian network can capture the
intuitive causal structure that underlies awareness growth, we tackle
the question whether you should assess the evidence at your
disposal---that the witness saw the defendant at the crime scene---any
differently than before. Without a clear model of the scenario, it might
seem that you should simply stay put. After all, besides the sensory
experience of the witness, you have gained no novel information about
the lighting conditions. Should you thus conclude that the evidence has
the same value before and after the realization that lighting could have
been bad?
...
evidence is likely to appear less strong.\footnote{By the law of total
  ...
  assignments.} If this is correct, this outcome violates Reverse
...
we now return to the more general lesson to be learned here: the
importance of formalizing structural assumptions and the role of
Bayesian networks in modeling awareness growth. To strengthen this
point, consider this variation of the \textsc{Lighting} scenario:
...
Bayesianism is perfectly fine in scenarios such as \textsc{Veracity}.
