...
\author{}
\date{\vspace{-2.5em}February 26, 2023}
...
(refinement). So we should consider each cae in turn.

Suppose awareness growth is modeled by adding states to an existing node
\(X\) in the network. Then, the existing probability tables in which
\(X\) occurs should be modified to accommodate the new value \(x^*\) of
\(X\). Specifically, the probability table for \(X\) should be modified,
as well as the probability tables for the children nodes of \(X\).
Constraint (C) will guide on how to change the probability tables in
question and define the new probability distribution \(\ppr{+}{}\). No
other changes to the probability tables are necessary because the theory
of Bayesian networks ensure that a variable \(X\) is independent from
all other variables conditional on its so-called Markov blanket (which
include the parent and children nodes of \(X\), as well as the parents
of the children of \(X\)).

Suppose instead awareness growth is modeled by adding a new node \(X\)
somewhere in the network. There are several cases to distinguish here.
The easiest cases are ones in which the new node is added without
breaking any existing connections between nodes. The addition of the new
nodes only requires drawing additional arrows that connects to the new
nodes to an existing nodes. Both \textsc{Veracity} and \textsc{Lighting}
are such cases. A slightly more complicated case is one in which an
alternative path between two variables is hypothesized. Say initially
you thought that gender had an effect on graduate admission decision.
The aggregate data available indicate that women are less likely to be
admitted to graduate schools. So the initial graph looks like this:
\(G \rightarrow A\). You then hypothesize that, since departments or
units make decision about admission, not the university as a whole,
there might is an alternative path from \(G\) to \(A\). Perhaps, women
happen to prefer departments that have lower admission rates. So a new
path must be added to the graph: \(G \rightarrow D \rightarrow A\).
