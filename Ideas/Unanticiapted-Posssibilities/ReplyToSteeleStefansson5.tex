...
\author{}
\date{\vspace{-2.5em}July 13, 2023}
...
explicit. So no adequate theory of awareness growth can be a purely
formal theory. This does not mean, however, we should give up on
probability theory altogether. Thanks to their ability to express
probabilistic dependencies, Bayesian networks can help to model
...
to the old probability distribution. The changes are usually localized
to the probability tables of the node \(X\) and the children of node
\(X\). Following well-known properties of Bayesian networks, these
localized changes are justified on the basis of the dependencies
structure among the nodes in the old and new network.\footnote{The
  theory of Bayesian networks ensure that a variable \(X\) is
  independent from all other variables conditional on its so-called
  Markov blanket (which includes the parent and children nodes of \(X\),
  as well as the parents of the children of \(X\)).}
