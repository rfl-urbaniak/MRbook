...
We share Steele and Stefánsson's skepticism toward Reverse Bayesianism, but also believe their counterexamples have limited applicability. To remedy that, we strengthen their argument by providing a simpler counterexample that is less prone to objections (\S \ \ref{sec:counterexamples}) <!---(\S \ \ref{sec:better})--->. At the same time, we conjecture that the problem of awareness growth cannot be tackled in an algorithmic manner because subject-matter structural assumptions need to be made explicit. Thanks to its ability to model probabilistic dependencies, we think that the theory of Bayesian networks can help to theorize about awareness growth in the Bayesian framework. We offer two illustrations of this claim. First, we provide an example of awareness growth refinement that it is structurally different from other cases of refinement (\S \ \ref{sec:downstream}). Second, we model two scenarios from Anna Mathani, both intended to challenge Reverse Bayesianism (\S \ \ref{sec:mathani}). As we will see, Bayesian networks allow us to see more clearly which probability assignments should be retained during awareness growth and which ones should be modified. The choice is guided by how each scenario is modeled, not by general formal constraints. 
...
sits now on firmer ground. But there is a more general lesson to be learned here. 
