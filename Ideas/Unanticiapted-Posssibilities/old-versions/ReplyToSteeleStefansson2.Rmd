...
the distinction between refinement and expansion that Steele and Stefánsson rely on is actually more fine-grained. The second illustration draws on some recent scenarios formulated by Anna Mathani. 
...
Before awareness growth, the node $Image$ has two states: $lions$ and \textit{heads-image}.^[The heads side must have some imagine, not specified in the scenario.] You assume that $Image=lions$ is true if and only if $Outcome=tails$ is true. <!---This assumption can be represented probabilistically, as shown in the table below.--->
