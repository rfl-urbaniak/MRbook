...
concerned with. Assuming that for each data point RMP\(=\gamma\) is held
constant, and that random matches with different data points
\(\mathsf{match_1, match_2, \dots, match_d}\) exclude each other, the
probability of there being at least one random match is the same as the
probability of their disjunction could be calculated by the additivity
axiom:
...
This calculation would result in the outcome recommended by NRC II. The
question is, whether this model should be applied to database matches.
...
clearly negative. Assuming the matches are independence, of
\(\mathsf{no match}\) for the members of the database, this probability
should rather be
...
Multiplication by database size would make sense if we thought of it as
addition of individual match probabilities, provided matches exclude
each other and so, are (extremely) not independent. Here is a coin
...
Another reason why DMP is problematic can be seen by taking an extreme
...
guilt, since everybody would excluded as a suspect except one matching
individual. But if RMP were to be multiplied by the size of the
database, the probative value of the match as measured by DMP should be
extremely low. This is highly counter-intuitive.
...
As mentioned in passing, perhaps a somewhat more sensible answer is
...
I could still use \(1-\pr(\mathsf{no match})\) to calculate the same
probability, but I can't calculate \(\pr(\textrm{no match})\) by taking
it to be
