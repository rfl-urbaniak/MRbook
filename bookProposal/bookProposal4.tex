...
normic support (Smith), argumentation theory (Sartor and Prakken), the
story model (Pennington and Hastie; van Koppen and Mackor) and to some
extent relative plausibility (Allen and Pardo). The second point we make
is that other accounts, in particular knowledge accounts of legal proof,
borrow ideas from contemporary analytic epistemology and apply them to
the legal domain. These accounts can be criticized for failing to take
seriously into account the values and objectives that are integral to
decision-making in the justice system.

We might ourselves be accused of unquestionably taking probability
theory as a paradigm of rationality for legal-decision making.
Admittedly, there is no empirical evidence that a probabilistic turn
would improve trial decisions, if only because what it would mean to
`improve' them is unclear. A set of criteria are needed for assessing
the desired improvements. `Probability on Trial' tackles this normative
question in the final part and shows how probability theory can be of
service here.
