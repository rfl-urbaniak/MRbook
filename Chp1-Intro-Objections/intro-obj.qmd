...
The metaphor of a scale tilting on one side or othe other depeding on the evidence presented is good only up to a point. How do we move past the metaphor? Numbers should be provided to fill in the relevant conditional probabilities such as $P(H_A\ vert E)$. The problem is that it is difficult to assign such numbers. Here legal probabilism faces the first major challange: \textit{Where do the numbers come from?}
...
The generic $L$ could be replaced by the more specific accuasation theory $H_A$, but to keep the notation easier to read, 
we will stick to $L$ for now.  The probability 
$P(H_A\ vert E)$ is a function of the prior probability $P(H_A)$. It is also a fucntion of the likelihood probabilities $P(E \vert H_A)$ and $P(E \vert \neg H_A)$. So numbers must be assigned to these probabilities in order to assign a number to $P(H_A\ vert E)$. But it is hard to find the relevant data, and often guesses are the best that can done.

## The challenge more in detail 

This challenge comes in two parts, first the problem of priors and second the problme of likelihoods. Let us start with assessing **prior probabilities**. The prior probability $P(L)$ must be set somewhere, but where? Should the prior be $1/n$, where $n$ is the number of individuals who could have committed the unlawful acts in question? Setting $P(L)=1/n$ makes sense in a criminal case in which the identity of the perpertrator is disputed. Absent information for distinguishing $n$ possible perpetrators---prior to considering the trial evidence $E$---it is natural to set the prior probability $P(L)$ to $1/n$ since any of them could be the perpetrator.  But $1/n$ does not make sense in other contexts, criminal or civil, in which the identity of the person who committed the acts isn't disputed. What is disputed, rather, is how the acts exactly unfolded and so whether they amount to illegal conduct after all. In such cases, the prior could be $l/s$, where $s$ is the number of possible ways the events could have unfolded and $l$ is the number of legally prohibited ways, where of course $l<s$. But while it is clear how to count $n$ possible suspects,  it is less clear how to count $s$ or $l$ possible ways the events could have unfolded.

Even if the problem of priors can be addressed, another problem looms. The task of assessing the likelihood probabilities $P(E \vert L)$ and $P(E \vert \neg L)$ is daunting, especially because the statements $L$ and $E$ are complex propositions. How is one supposed to find the right numbers? 

As a first step, it is helpful to break down $L$ into smaller level statements, say, whether the defendant visited the crime scene or left a blood stain at the scene. It is also be helpful to break down $E$, which refers to the total body of evidence presented at trial, into smaller components: fingerprint evidence, witness testimonies, genetic matches, expert reports, etc. Once the statements are broken down this way, assigning probabilities to them becomes more manageable. 
...
Forget about setting the priors for $L$, and only focus on the ratio of the other two probabilities, $\frac{P(E \vert L)}{P(E \vert \neg L)}$, often called the likelihood ratio. This ratio measures how strongly the evidence supports the liability claim $L$. The greater the ratio (for values above one), the stronger the support $E$ lends in favor of $L$. So, what numbers should be assigned to $P(E \vert L)$ and $P(E \vert \neg L)$, sometimes called likelihood probabilities? As we've see, the answer is far from clear. But, instead of assigng probabilitis between 0 and 1, to both numerator and denominator, we could assign a ratio, without specifying the full probabilities. Call this the **ratio apoproach**. Still, it is doubtful that ratios alone will be enough to model an entire legal case. They might serve to model uncertainty for small level propositions, but not much beyond that. 
...
Another problemas with sensitivity analysis or the interval approach is that any probability in the interval has the same weight as any other, so extreme values are going to play too strong a role. If the interval is suffuciently large, nearly any probability value will count. More generally, imprecise legal probabilism will suffer from problems similar to imprecise probabilism in formal epistemology, such as belief inertia. So the approach rests on a shaky therethical foundation. 
