...
Witnesses are called to testify in court about questions relevant to the defendant's civil or criminal liability. They can be lay people testifying about what they saw or heard. They can be experts testifying about laboratory testing or general scientific knowledge. As they testify, 
...
For legal probabilism, the rule of decision is that the defendant should be found liable if the probability of liability meets a suitable threshold, say, in civil cases $>.5$ How do we arrive at the probability of liability? There is broad agreement that the underlying inference engine is Bayes' theorem, but numbers must be provided to arrive at the probability of liability, formally modeled as the conditional probability $P(H_A \vert E)$. The problem is that it is difficult to assign such numbers. Here legal probabilism faces the first major challenge: \textit{Where do the numbers come from?}
...
For examples of "logical structure of reasoning" see the next challenge. 
So the open question here is, does probability theory give us the correct and most adequate structure of reasoning under uncertainty? This is by no means obvious.  See the next challenge. 



# Challenge II: Not just probability as the guiding criterion


## The challenge in a nutshell

Even if the numerical challenge could be addressed in one of the ways outlined earlier, other difficulties exist for legal probabilism. Why ascertain the probability of liability? Is this the right metric to focus on? Intuitively, if the probability of liability were low---assuming this probability can be established---that would be a good reason not to find the defendant liable. But this intuition only shows that a high enough probability of liability is a \textit{necessary} condition for a finding of liability. Is it also a sufficient condition? This is less obvious. 

We can think of cases in which---intuitively---the probability of liability is high enough, yet the supporting evidence is weak by some other measure, and thus we think it insufficient to sustain a judgment against the defendant. Examples below.


## Stylized case A: high probability, but no liability judgment 

A civil suit is brought against someone to recover a certain amount of money. This is a case of civil theft and the governing standard is preponderance or balance of probabilities. The evidence is that a rumor suggests the defendant embezzled money from the plaintiff. The accusation theory $H_A$ is this: the defendant was working in the plaintiff's company as an administrator and in this capacity embezzled USD 50,000 of the company's funds. That the money is gone is unquestionable. It is also unquestionable that only two people had access to the money; one of them was the defendant. Without any more specific evidence about what happened, the starting point is equipoise between $H_A$ and its negation, where $H_A$ is the accusation theory that the defendant embezzled the money.  That is, $P(H_A)=P(\neg H_A)$. As the rumor $R$ is added as evidence, the balance tilts towards $H_A$, if only so slightly. So, $P(H_A \vert R)>P(\neg H_A \vert R)$. But it would be odd---on such tenuous evidence---to conclude that the defendant is liable for embezzlement. A case like this should not even be litigated.

## Stylized case B: meta-uncertainty 
...
Consider a variation of the case. Instead of a rumor, the key evidence consists of a recording of a phone conversation. In the recording, the speaker says 'I will provide you with the banking details for transferring USD 50,000 tomorrow'. The phone conversation took place a few days before the money disappeared from the company's accounts. So the speaker must be the one who embezzled the company's funds. Who is the speaker, then? 

A voice recognition expert testifies at trial that the voice profile in the call matches the defendant's voice profile. In addition, the expert testifies that, based on a database of 100 voice profiles, the matching profile occurs with 20 percent frequency. Is this enough to find the defendant liable of civil theft?

The starting point is $P(H_A)=P(H_D)$ given the setup of the case (only two people had access to the company's accounts). What needs to be determined is the ratio 
...
and $P(P(PM \vert H_D))=.2$.^[\textbf{!!!Possible error!!!} Not sure this number is correct. If 20 percent is the frequency of the voice profile in the general population, this figure does not apply to the other person who had access to the company's accounts. This other person is not the general population. So, for all we know, $P(PM \vert H_D)=1$, as well. It is correct to say, P(PM \ vert \neg H_A)=.2, but it does not follow that P(PM \vert H_D)=.2. \textit{So the comparative approach makes some numbers we have, like random match probabilities, unusable?}] So, $\frac{P(H_A \vert PM)}{P(H_D \vert PM)}=1/.2=5$, largerly above the ratio threshold of 1. So should the defendant be found liable?
...
A more sophisticated version of legal probabilism, then, should be able to do at least two things: first, formally model these additional dimensions using the language of probability (or determine to what extent they fall outside the scope of probability theory and cognate theories). In particular, two subchallenges arise. 

## Sub-challenge II: Modeling adversarial scrutiny and evidential weight

Trials are adversarial events. Evidence is examined and cross-examined. How can this adversarial process be modeled probabilistically? Part of this challenge is discussed later under "Subchallenge III: Modeling argument patterns".

There seems to be a connection between probability and adversarial scrutiny, but this connection must explored more in detail.  Take a body of evidence $E$ which slightly tilts in favor of $A$ over $B$, say a rumor favors $A$ over $B$ as discussed above.  Now, after a fierce process of examination and cross-examination, a body of evidence $E'$ also tilts slightly in favor of $A$ over $B$. Clearly, the support that $A$ receives by $E$ is much weaker than the support it receives by $E'$, but the probabilities of $A$ compared to $B$ are the same. For the evidence in both cases just slightly tilts in favor of $A$. How can this idea be captured with probability theory? The contrast here can be put in terms of *balance* versus *weight* of the evidence.  


## Sub-challenge II: Modeling specificity and other narrative-related notions

Aiming to establish that the defendant is liable with a sufficiently having probability is certainly important, but it is also important to establish the facts with some degree of specificity.  The plaintiff or prosecutor cannot simply establish that the defendant did something or other, or that no matter what they did exactly, they are liable. This line of argument would be unconvincing. 

Interestingly, probability and specificity are inversely proportional. The more specific a story, the less probable it is going to be all else being equal.  After all, the statement $A \wedge B$ is more specific than $A$ or $B$, but less probable all else being equal. How are specificity and probability related? How should specificity be modeled? If specificity lowers probability, shouldn't we aim for less specific but more probable narratives and this way maximize accuracy?

Besides specificity, another narrative-related notion is coherence. See later under "Subchallange III: Modelign coherence boost and fitting together"


# Challenge III: How to aggregate evidence and evaluate hypotheses?


## The challenge in a nutshell 

Legal probabilism has been successful in modeling, using probability theory, the uncertainty for small-level propositions, say the conditional probability $P(S \vert M)$, where $S$ stands for 'source' and $M$ for 'match.' But what about general-level propositions, sometimes also called activity-level propositions? What is the probability that the defendant visited the crime scene? Or what is the probability they committed a specific act? Factual questions are often framed at different levels of analysis, and these different levels must be integrated. The question is how. 

A related problem is that the evidence presented in a case often consists of several pieces dependent on one another in complex ways. Some intermediate hypotheses could play the role of evidence. So, how are multiple pieces of evidence aggregated and how are complex hypotheses built out of their simpler components? This is---in a broad and rough outline---the challenge, call it the **aggregation challenge**. 

## Subchallenge III: Modeling Conjunction 

A simple way in which this challenge can be 
formulated is this (inspired by the literature on the conjunction paradox; see later chapters). Suppose two propositions must be established in a case to prove liability for the defendant, $A$ and $B$. Suppose $P(A \vert E_A)=p_A$ and and $P(B \vert E_B)=p_B$. Then, what is the aggregate probability  $P(A \wedge B \vert E_A \wedge E_B)$? The answer is not to multiply $p_A$ and $p_B$, but depends on the relationship between $A$ and $B$, as well as $E_A$ and $E_B$. The challenge, then, is to offer a systematic method to analyze these dependencies and offer a recipe for the aggregation of evidence and hypotheses.


## Bayesian networks as a common response
...
## Subchallenge II: Modeling argument patterns 
...
Hopefully, Bayesian networks will not simply validate familiar argument patterns but also offer further nuance and insight. Bayesian networks could also show that some reasoning patterns are invalid, or they could validate new reasoning patterns that informally are hard to see.


### Example: Linear chain of uncertain inferences
...
In the argumentation framework,  each reasoning step could be defeated. If no reasoning step is defeated, then the entire chain of inferences stands. If any one of the reasoning steps is defeated, the entire chain breaks down. This is a simple approach, but it is binary and does not allow for modeling the strength of each reasoning step.   
 ...
 How to model this using Bayesian networks? Suppose we know the uncertainty of the inference from $A$ to $B$, namely in terms of $P(B \vert A)$ and we also know the uncertainty of the inference from $B$ to $C$, namely in terms of $P(C \vert B)$. Then, what is the aggregate uncertainty of the inference from $A$ to $C$ via $B$? Is uncertainty aggregation compositional? In other words, let $P(B \vert A) = u_{AB}$ and $P(C \vert B) = u_{BC}$. What is the overall uncertainty $P(C \vert A) = u_{AC}$?
...
## Subchallenge II: Modeling coherence boost and fitting together 
...
## Ratios of probabilities, not priors or posterior probabilities 
...
Bayes' theorem, in its ratio version, now becomes:
...
## Ratio thresholds as decision rules 

That the posterior ratio cannot be used to deduce the posterior probability $P(H_A \vert E)$ should not be a problem, however.  On the comparative approach, the goal should be to assess the ratio, not the posterior probability $P(H_A \vert E)$. So the rule of decision, the burden of proof, should also be understood in this comparative fashion (call it the **comparative formulation**): 
...
## Found liable, yet unlikely to be liable 
...
If we focus on the third task only, the question remains whether legal probabilism and the theory of relative plausibility are in fact in broad agreement or differences still exist. 

## Response B: Going beyond Bayesian updating 

- Legal probabilists can extend the framework beyond mere updating and cover tasks (i) assessing and interpreting the evidence and (ii) formulating hypotheses in light of the evidence. There is updating within the model (standard Bayesian updating) and there is updating of the model (space of possibilities) outside the model. The latter seems an integral part of reasoning with evidence and hypotheses. 

\textit{See the chapter on cross-examination and arguments should address this challenge.} 
...
\textit{The chapter on story coherence should address this challenge.} 


# Challenge V: No evidence that probability reduces errors 
It is clear that people make probabilistic mistakes in reasoning, but does this show that mistaken convictions are caused by these probabilistic mistakes? There is no evidence of that. In what way does probability improve the accuracy of legal decisions? \textit{Discussion about accuracy and fairness should address this challenge}

# Structure 

So we can envision  four central chapters: 

Chapter: Higher-order probability See the existing chapter and paper on higher-order legal probabilism.
