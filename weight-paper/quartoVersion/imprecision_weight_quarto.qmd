...
The @fig-weightDemo1 demonstrates the issue described above. We have created $3$ beta distributions representing seeing equal amounts of colors of beans. The first one is the prior, the second one is the posterior after seeing $2$ beans, and the third one is the posterior after seeing $1000$ beans. The weight of evidence is represented with the narrown shape of the distributions, where the last one has much more weight as the number of observations increases.

```{python}
#| echo: false
#| label: fig-weightDemo1
#| fig-cap: "An example of how the weight of evidence can change with the sample size."
...
plt.plot(x, prior, label='prior (Beta(1, 1))', linestyle='-', color='black')  # Solid line
plt.plot(x, posterior_2, label='posterior (Beta(2, 2))', linestyle='--', color='black')  # Dashed line
plt.plot(x, posterior_1000, label='posterior (Beta(501, 501))', linestyle=':', color='black')  # Dotted line

plt.title('Bayesian inference with beta distribution (0, 10 and 1000 coin tosses)', fontsize=16)
plt.xlabel('coin bias', fontsize=16)
...
The information contained within a distribution can be quantified using its entropy, given by $H = -\sum p \log(p)$. To normalize this measure and make it insensitive to the grid size choice (as long as it is kept fixed), and to scale it between 0 and 1 for interpretability, we use the formula:
$$ W(p) = 1 - \frac{H(p)}{H(\text{uniform})} $$
...
priorWeight = weight(prior)
posterior_2Weight = weight(posterior_2)
posterior_1000Weight = weight(posterior_1000)
...
#translating the values to R
priorWeightR <- py$priorWeight
posterior_2WeightR <- py$posterior_2Weight
posterior_1000WeightR <- py$posterior_1000Weight
```

\normalsize

The calculations of the weights for the distributions in @fig-weightDemo1 are as follows (rounded to two decimal places):

\begin{align*}
&W(\text{uniform}) = \text{`r round(priorWeightR, 2)`} \\
&W(post_{2}) = \text{`r round(posterior_2WeightR, 2)`} \\
&W(post_{1000}) = \text{`r round(posterior_1000WeightR, 2)`}
\end{align*}
...
after obtaining the evidence as compared to our point of departure. Notably, the prior doesn't have to be uniform. Here, one conceptualization (which has the feature of being on the same scale) is simply the difference in weights of the distributions: $\Delta W = W(\text{posterior}) - W(\text{prior})$.

\begin{align*}
&\Delta W(post_{1000}, post_{2}) = \text{`r round(posterior_1000WeightR, 2)`} - \text{`r round(posterior_2WeightR, 2)`} = `r round(posterior_1000WeightR, 2) - round(posterior_2WeightR, 2)`\\
\end{align*}
...
Another approach is to calculate the proportionate difference between the weights of the prior and the posterior, though this makes the scale harder to interpret:
$\frac{W(\text{posterior})}{W(\text{prior})}$

\begin{align*}
&\Delta W(post_{1000}, post_{2}) = \frac{\text{`r round(posterior_1000WeightR, 2)`}}{\text{`r round(posterior_2WeightR, 2)`}} = `r round(posterior_1000WeightR, 2) / round(posterior_2WeightR, 2)`\\
\end{align*}
...
#| fig-cap: "Joyce's $w$ (the lower it is, the higher the weight) for various observed successes in 10 Bernoulli trials. Three chance chypotheses: $.4, .5, .6$, and two sets of priors: equal and $.5, .3, .2$ respectively. Note how the weightiest evidence is obtained with five observed successes and how its drop is fourteen-fold, if the observed frequency is 0."
#| label: fig-joyce1
...
The behavior of $w$ is even more unusual if the sample size is higher. In Figure @fig-joyce2 we illustrate what happens with $n=100$, for various possible outcomes of 100 Bernoulli trials.
...
In Figure @fig-joyce3 we illustrate two other phenomena, which  might come up when the observed frequencies are kept fixed, but the sample size increases.
...
#| fig-cap: "Joyce's $w$ (the lower it is, the higher the weight) for two fixed success ratio across various observed successes in  Bernoulli trials (lines are used for smoothing).  Note large shifts with possible decrease in the beginning, and a flattening afterwards."
#| label: fig-joyce2
...
From this perspective, the various scenarios we discussed (which \textsf{IP} has hard time distinguishing between) can be easily represented in the manner illustrated in Figure @fig-evidenceResponse.  
...
```{r figEvidenceResponse,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%"}
#| fig-cap: "Examples of RA's distributions responding to various types of evidence for typical cases brought up in the literature."
...
```
 ...
 \(B \geq A\), and that the agent considers
...
```{r figInertia2, echo=FALSE, eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
#| fig-cap: "As observations of green, green and black come in, extreme parameter values drop out of the picture and the posterior is shaped by the evidence."
#| label: fig-intertia2
#| fig-pos: H
...
```{r figRinard2,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
#| fig-pos: H
#| label: fig-rinard2
#| fig-cap: "Learning in Rinard's example of belief inertia."
...
one, your choices are visualized in Figure @fig-entDAG.
...
#| fig-cap: "You want to navigate from $A$ to $D$ with the uninformed prior."
#| label: fig-entDAG
#| fig-pos: H
...
distributions look like, inspect Figure @fig-betas. Remember  we're working with a grid approximation ($n=1k$).


```{r figBetas,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
n <- 1000 #parameters 
s <- 1e5  #sample size
ps <- seq(from=0 , to=1 , length.out=n)
a <- seq(1,100,1)
...
```{r figBetas2,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
...
```{r figBetas3,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
#| fig-pos: H
#| label: fig-betas
#| fig-cap: "Examples of beta distributions with entropies and their weights with grid approximation ($n=1000$). Note that distribution weight does not strongly depend on its expected value."
...
Now let us get back to the example we used to inspect the behavior of Joyce's proposal. Figure @fig-entropyJoyceExamplePlot illustrates dependency on frequency and on priors for sample size $10$, and @fig-entropyJoyceExamplePlot100 for sample size 100.
...
#| fig-cap: "Entropy-based weight for for various observed successes in 10 Bernoulli trials. Three chance chypotheses: $.4, .5, .6$, and two sets of priors: equal and $.5, .3, .2$ respectively."
...
#| label: fig-entropyJoyceExamplePlot100
#| fig-cap: "Entropy-based weight for for various observed successes in 100 Bernoulli trials. Three chance chypotheses: $.4, .5, .6$, and two sets of priors: equal and $.5, .3, .2$ respectively."
...
We also should inspect the behavior of $w$ when the frequency is kept fixed, but the sample size increases (Figure @fig-entropyJoyceExampleSampleSize).
...
#| fig-cap: "Entropy-based weight for two observed frequencies for various sample sizes (lines used instead of points for smoothing). Three chance chypotheses: $.4, .5, .6$, and two sets of priors: equal and $.5, .3, .2$ respectively."
...
Now, let us generalize and see how the measure behaves with respect to beta distributions. Two phenomena are as expected. First, the entropy decreases with the number of observations, and second, it decreases faster if the proportions are closer to the extremes. This is mirrored by the corresponding weights (Figure @fig-weights).
...
#| fig-pos: H
#| label: fig-weights
#| fig-cap: "Weights of beta likelihoods for $a,b$ ranging from $0$ to $100$, versus the number of observations   and versus absolute distance of the proportion from .5."
...
Moreover, the framework is capable of sensible comparison of weights for distributions of various shapes, including those involving all weights focused on a particular point (strictly speaking, a single bin in the grid approximation). Here are some  examples of shapes  worth looking at (Figure @fig-weightsWeird).
...
```{r figWeightsWeird,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
#| fig-pos: H
#| label: fig-weightsWeird
#| fig-cap: "Examples of various distributions with their entropies and weights, ordered by weights. (1) beta(4,4), (2) uniform starting from .5 to 1, (3), uniform strating from .6 to 1, (4) two normal distributions centered around .4 and .6 with standard deviation .05, glued at .5. (5) normal centered around .5 with the same standard deviation, (6) one that assigns .5 to each of .4  and .6, (7) One that assigns .3 to .4 and .7 to .6., (8) one that assigns all weight to .5, and (9) one that assigns all weight to .7."
...
Consider the (simplified) BN developed by CITE FENTON to illustrate how conviction was unjustified in the Clark case (Figure @fig-scBNplot). 
...
#| label: fig-scBNplot
#| fig-cap: "The BN developed by FENTON ET AL., with marginal prior probabilities."
...
The point to be illustrated was that with a sensible choice of probabilities for the conditional probability tables in the BN, conviction was not justified at any of the major stages (Table @fig-SCfentonTable). 
...
#| fig-cap: "The prior and posterior probabilities for Fenton's Sally Clark BN."
...
Some of these concerns are at least dampened when we deploy the higher order probabilities in the BN. For instance, your conditional probabilities might look as illustrated in Figure  @fig-SCwithHOPa. \todo{describe the sampling approach}
...
#| fig-cap: "Example of approxmiated uncertainties about conditional probabilities in the Sally Clark case."
...
#| label: fig-SCwithHOP
#| fig-cap: "Example of a HOP approach tor the Sally Clark Case  approximated by sampling probabilities  and constructing 10k BNs."
...
Using these we can investigate the impact of incoming evidence as it arrives (Figure @fig-SCwithHOP2)
...
And we can use this tool to gauge our uncertainty about the likelihood ratios (Figure @fig-SClrs).
...
#| label: fig-abuse1
#| fig-cap: "Shifts from prior $beta(2,4)$ to the posteriors, given various conditional probabilities."
#| fig-pos: H
...
```{r figEntropies19,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
#source("../../scripts/SCplotDistros.R")
source("../../scripts/SCfunctions.R")
...
\begin{figure}
```{r figEntropies2_19,echo=FALSE,eval=TRUE,fig.align = "center", cache=TRUE, fig.show = "hold", out.width = "100%",   message = FALSE, warning = FALSE, results = FALSE}
...
One use of weight is to use expected weight of still missing pieces of evidence to guide evidence gathering, especially with limited resources. Since we don't want to possible impacts of evidence to cancel each other, we use absolute distance of proportional change in weight from 1 in the expectancy calculations and in looking at possible outcomes, to which we should also be sensitive (Figure @fig-SCexpectedWeights2).
...
#| label: fig-SCexpectedWeights2
#| fig-cap: "Expected size (absolute value) of the proportional weight change calculations. Illustrated for bruising in child A, calculated for bruising and signs of disease in child A."
...
#| label: fig-EMC
#| fig-cap: "Three (out of many) candidate distributions for a Schoenfield-inspired example. All distributions are built from normal  distributions with standard deviation $.5$, the bimodal ones are 'glued' in the middle."
#| fig-pos: H
...
For the three distributions we're discussing in this chapter, the inaccuracies calculated using CRPS  and KL divergence with respect to various potential true probability distributions look as in Figure @fig-inaccuracies2.
...
#| fig-cap: "KL divergence inaccuracies vs (omniscient functions corresponding to) $n$ true probability hypotheses for the three distributions discussed in this section."
